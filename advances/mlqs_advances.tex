...
\newdefinition{dfn}[thm]{Definition}
...
%\shorttitle{MLQs with spectral parameters}

\author[upenn]{Erik Aas}
\ead{eaas@kth.se}

\author[umn]{Darij Grinberg}
\ead{darijgrinberg@gmail.com}
\ead[url]{http://www.cip.ifi.lmu.de/~grinberg/}

\author[uq]{Travis Scrimshaw\corref{cor1}\fnref{ts}}
\ead{tcscrims@gmail.com}
\ead[url]{https://people.smp.uq.edu.au/TravisScrimshaw/}

\address[upenn]{Department of Mathematics, Pennsylvania State University, McAllister Building, State College, PA 116802, USA}
\address[umn]{School of Mathematics, University of Minnesota, 206 Church St. SE, Minneapolis, MN 55455}
\address[uq]{School of Mathematics and Physics, The University of Queensland, St.\ Lucia, QLD 4072, Australia}

\cortext[cor1]{Corresponding author}
\fntext[ts]{TS was partially supported by the Australian Research Council DP170102648 and the National Science Foundation RTG grant DMS-1148634.}

\begin{keyword}
multiline queue \sep TASEP \sep $R$-matrix \sep symmetric function
\MSC[2010]{
...
For statistical mechanics, the ASEP is a model for gas particles in a lattice with an induced current, where the exclusion mimics the short-range interactions among the particles.
...
\item spontaneous symmetry breaking with possibly multiple broken symmetry phases~\cite{AHR98,AHR99,CEM01,EPSZ05,EFGM95,GLEMSS95,PK07},
...
\end{itemize}
We also refer the reader to~\cite{PEM09,SZ95,Schutz01,TJHJ16} and references therein.
...
\item (deformed) Knizhnik--Zamolodchikov (KZ) equations, (non)symmetric Macdonald polynomials, and Koornwinder polynomials~\cite{CdGW15,KT07},
...
Hence, we have $q(346613321) = 277344511$, which is a word of type $(2,1,1,2,1,0,2,\ldots)$.
\end{example}
...
\begin{dfn}
For a permutation $\sigma \in \SymGp{\ell-1}$, a \defn{$\sigma$-twisted multiline queue (MLQ) of type $\mm = \tup{m_1, m_2, \ldots, m_\ell, 0, 0, \ldots}$}, with $\ell$ classes, is a sequence of queues $\qq = (q_1, \dotsc, q_{\ell-1})$ such that $q_i$ is a $p_{\sigma(i)}(\mm)$-queue and $m_{\ell} = n - p_{\ell-1}(\mm)$ (that is, $p_\ell(\mm) = n$).
...
\begin{proof}
  ...
  \]
  which is a $(s_{i-1} \dotsm s_2 s_1)$-twisted MLQ of type $\mm$ with weight
...
\begin{proof}
Recall that the permutation $w_0 \in \SymGp{r}$ is identified with the word $u(v_{w_0}) \in \mcW_n$;
...
This also proves~\cite[Conj.~3.6]{AasLin17} (which is a generalization of~\cite[Conj.~3.4]{AasLin17}).
...
We will be relying on the \defn{Lindstr\"om--Gessel--Viennot (LGV) Lemma}~\cite{GV85,Lindstrom73} and on a re-interpretation of MLQs as a certain kind of semistandard tableaux (of non-partition shape).
...
If $\tup{A_1, A_2, \dotsc, A_k}$ and $\tup{B_1, B_2, \dotsc, B_k}$ are two $k$-vertices, then we define a \defn{non-intersecting lattice path tuple (NILP)} from $\tup{A_1, A_2, \dotsc, A_k}$ to $\tup{B_1, B_2, \dotsc, B_k}$ to be a $k$-tuple $\tup{p_1, p_2, \dotsc, p_k}$ of paths such that
\begin{itemize}
...
Consider a strictly increasing sequence of elements $s = \tup{s_1, s_2, \dotsc, s_k}$ with $s_i \in \ive{n}$ for all $i \in \ive{k}$.
Then,
\[
...
\]
\end{enumerate}
(For the border case of $\ell = 1$, we interpret $q_0$ as the empty set whenever $\qq = \tup{q_1, q_2, \dotsc, q_{\ell-1}}$ is an MLQ.)
\end{lemma}
...
\item[(b)] For each $h \in \ive{t-1}$, we have $A \succeq B \gg C$, where
\begin{align*}
...
Then, the word $u(v)$ has type $\mm$ by the construction of $u(v)$.
...
Recall that we can write any permutation in $\SymGp{\ell-1}$ as a product of simple transpositions $s_1, s_2, \ldots, s_{\ell-2}$.
...
\appendix
\renewcommand*{\thesection}{\Alph{section}} % Fix for the ToC formatting
