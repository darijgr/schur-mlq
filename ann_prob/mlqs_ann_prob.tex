...
\usepackage{tikz,mathrsfs,listings,multicol}
...
\renewcommand{\arxiv}[1]{\href{http://arxiv.org/abs/#1}{\texttt{arXiv:#1}}}
...
\runtitle{MLQs with spectral parameters}

\begin{aug}
\author{\fnms{Erik} \snm{Aas}\thanksref{m1}\ead[label=e1]{eaas@kth.se}},
\author{\fnms{Darij} \snm{Grinberg}\thanksref{m2}\ead[label=e2]{darijgrinberg@gmail.com}\ead[label=u2,url]{http://www.cip.ifi.lmu.de/~grinberg/}}
\and
\author{\fnms{Travis} \snm{Scrimshaw}\ead[label=e3]{tcscrims@gmail.com}\thanksref{t1,m3}\ead[label=u3,url]{https://people.smp.uq.edu.au/TravisScrimshaw/}}

\affiliation{Pennsylvania State University\thanksmark{m1}, University of Minnesota\thanksmark{m2}, and The University of Queensland\thanksmark{m3}}

\address{Department of Mathematics,\\ Pennsylvania State University,\\ McAllister Building, State College, PA 116802, USA\\\printead{e1}}
\address{School of Mathematics,\\ University of Minnesota,\\ 206 Church St. SE, Minneapolis, MN 55455\\\printead{e2}\\\printead{u2}}
\address{School of Mathematics and Physics,\\ The University of Queensland,\\ St.\ Lucia, QLD 4072, Australia\\\printead{e3}\\\printead{u3}}

\thankstext{t1}{TS was partially supported by the Australian Research Council DP170102648 and the National Science Foundation RTG grant DMS-1148634.}
\end{aug}
...
\kwd[primary ]{60C05}  % Combinatorial probability
\kwd{05A19}  % Combinatorial identities, bijective combinatorics
\kwd[; secondary ]{16T25}  % Yang--Baxter equations
\kwd{05E05}  % Symmetric functions
\end{keyword}
...
One of the fundamental models of particles moving in a 1-dimensional lattice is given by the asymmetric simple exclusion process (ASEP), and it has received broad attention in many different variations.
...
\begin{itemize}
\item transportation processes in capillary vessels~\cite{Levitt73} or in proteins within the cells along actin filaments~\cite{KNL05},
...
\item (deformed) Knizhnik--Zamolodchikov (KZ) equations, Koornwinder polynomials, and (non)symmetric Macdonald polynomials~\cite{CdGW15,KT07},
...
\]
Therefore, it remains to prove $\vee^{(k)} q(u) = q(\vee^{(k)} u)$.
Note that the permutation $\tup{i_1, i_2, \dotsc, i_n}$ in the construction of $q(u)$
...
\begin{proof}
  ...
  \]
  the result of which is a $(s_{i-1} \dotsm s_2 s_1)$-twisted MLQ of type $\mm$ with weight
...
The paper~\cite{AAMP}, building on work by Ferrari and Martin~\cite{FM06,FM07}, introduced opposite operators $\Psi_i \colon \mcW_S \to \mcW_{S \setminus \{i\}}$ given by $\Psi_i(\epsilon_u) = \sum_{q} \epsilon_{q(u)}$, where the sum is taken over all $i$-queues $q$.
Ayyer \textit{et al.}\ showed that $\Psi_i M_S = M_{S \setminus \{i\}} \Psi_i$.
...
Our arguments rely on the \defn{Lindstr\"om--Gessel--Viennot (LGV) Lemma}~\cite{GV85,Lindstrom73} and on a re-interpretation of MLQs as a certain kind of semistandard tableaux (of non-partition shape).
...
For a detailed derivation of Proposition~\ref{prop.LGV.concrete}, see the proof of Corollary~4 in \url{https://math.stackexchange.com/questions/2870640} (applied to $\omega_{a} = \wt(a)$).}

\begin{prop}
\label{prop.LGV.concrete}
Let $k \in \NN$.
Consider two $k$-vertices $\uu = \tup{A_1, A_2, \dotsc, A_k}$ and $\vv = \tup{B_1,B_2, \dotsc, B_k}$ such that
\begin{align*}
...
\[
\bigl\{ \underbrace{a_1 > \cdots > a_{p_1}}_{=q_i^{(1)}} > \underbrace{a_{p_1+1} > \cdots > a_{p_2}}_{=q_i^{(2)}} > \cdots > \underbrace{a_{p_{i-1}+1} > \cdots > a_{p_i}}_{=q_i^{(i)}} \bigr\},
\]
where $p_i = p_i(\mm)$.
...
\[
\overline{P} \colon \set{\text{interlacing MLQs of type } \mm}  \to \set{  \text{semistd.\ tableaux of shape } \lambda^{\mm}}.
\]
\end{enumerate}
...
Therefore, Lemma~\ref{lem:determinant_form.interl-A}(a) (applied to $u'$, $q_\ell$ and $\ell$ instead of $u$, $q$ and $t$) shows that $u'$ is weakly decreasing up to level $\ell-1$
...
Let $\zeta \colon \mcW_n \to \mcW_n$ be the cyclic shift map that sends each word $w_1 \cdots w_n$ to $w_2 w_3 \cdots w_n w_1$.
...
Since $\word \colon \set{\text{MLQs}} \to A^*$ is injective,
...
\begin{proof}
Choose a permutation $(i_1, i_2, \dotsc, i_n)$ of $(1, 2, \dotsc, n)$ such that we have $u_{i_1} \leq u_{i_2} \leq \cdots \leq u_{i_n}$.
...
merges (each merge sends a word $u$ to $\vee^{(k)} u$ for some
...
Thus, the MLQ corresponding to this Kohnert diagram is $\tup{\set{2}, \set{1,2,3}, \set{2,3,4}}$;
