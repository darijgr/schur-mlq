...
\begin{enumerate}
 ...
 \item If $A$ and $B$ are two balanced sets, then the four sets
       $A \cap B$, $A \cup B$, $A \setminus B$ and $B \setminus A$ are
       balanced.\footnote{This can be proven as follows: First argue
       that $A \cap B$ is balanced when $A$ and $B$ are balanced intervals,
       hence also when $A$ and $B$ are balanced sets.
       Then, use $A \setminus B = A \setminus \tup{A \cap B}$ to
       prove the same for $A \setminus B$, thus also for
       $B \setminus A$. Finally, prove it for $A \cup B$
       using $A \cup B = \tup{A \cap B} \sqcup \tup{A \setminus B} \sqcup \tup{B \setminus A}$.}
