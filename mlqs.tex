...
Namely, for $k \in \set{p_i(\mm) \mid i \geq 0}$, let $S_k(w)$ denote the set of the indices $i \in \ive{n}$
corresponding to the $k$ smallest letters $w_i$ of $w$.
\Travis{This is bad notation as it looks like $S_k$ and $L_k$ are queues acting on the word $w$ and so $S_k(w)$ looks like a word.}
...
Thus, we have $S_k(w) \setminus S_{k-p_j(\mm)}(w) = S_k(w') \setminus S_{k-p_j(\mm)}(w')$ defined, which we denote by $P_j$.
...
Note that every balanced block for $(q_1, q_2)$ corresponds to a balanced block of $(q_2^*, q_1^*)$, where $q_i^*$ is the queue from Lemma~\ref{le:dual} of $q_i$.
Hence, we have $(q^*_2,q^*_1)' = ((q'_2)^*, (q'_1)^*)$ if and only if $(q_1,q_2)' = (q'_1, q'_2)$.
Also from Lemma~\ref{le:dual}, we have $q(u) = (q^*(u^*))^*$ (recall that $q^*(u^*)$ has $\ell+1$ classes).

We have $S_k(w)^* = S_{n-k}(w^*)$, where $S_{n-k}(w^*)$ are the largest $n-k$ letters of $w$ but where their positions have been reversed.
Now we have $\abs{q^*} < n-k$, and so from a similar argument as above using the largest letters and Phase~I, we have $\tup{ q^*, S_{n-k}(w^*) }' = \tup{S_{n-k}\bigl( q^*(w^*) \bigr), q^{\dagger*} }$.
Applying the $\ast$-duality, we obtain
\[
\tup{ q^{\dagger}, S_{n-k}\bigl( q^*(w^*) \bigr)^* } = \tup{ q^{\dagger}, S_k\bigl( q^*(w^*)^* \bigr) } = \tup{ q^{\dagger}, S_k\bigl( q(w) \bigr) }.
\]
\end{proof}
