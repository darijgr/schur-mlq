...
$\nn$ of the word $q'(1 \dotsm 1)$ satisfies $p_1(\nn) = k$.
Hence, any word obtained by actions of queues on $q'(1 \dotsm 1)$
will have its type $\nn$ satisfy
$k \in \set{ p_j(\nn) \mid j \geq 1 } $.
...
\[
\begin{tikzpicture}
  \draw[densely dotted] (0,0) grid (7,7);
  % axes:
  \draw[->] (0,0) -- (0,7);
  \draw[->] (0,0) -- (7,0);
  ...
  \begin{scope}[thick,>=stealth,red]
      ...
      \draw (2,3.6) edge[->] (2,4);
      \draw (2,4) edge (2.5,4);
      \draw (2.5,4) node[anchor=south] {$x_4$};
      \draw (2.5,4) edge[->] (3,4);
      \draw (3,4) edge (3.5,4);
      \draw (3.5,4) node[anchor=north] {$x_4$};
      \draw (3.5,4) edge[->] (4,4);
      \draw (4,4) edge (4,4.5);
      \draw (4,4.5) node[anchor=east] {$1$};
      \draw (4,4.5) edge[->] (4,5);
      \draw (4,5) edge (4,5.4);
      \draw (4,5.4) node[anchor=east] {$1$};
      \draw (4,5.4) edge[->] (4,5.7);
      % $p_2$:
      \draw (3,1) edge (3.6,1);
      ...
      \draw (4,2.5) edge[->] (4,3);
      \draw (4,3) edge (4.5,3);
      \draw (4.5,3) node[anchor=south] {$x_3$};
      \draw (4.5,3) edge[->] (5,3);
      \draw (5,3) edge (5,3.5);
      \draw (5,3.5) node[anchor=west] {$1$};
      \draw (5,3.5) edge[->] (5,4);
      \draw (5,4) edge (5,4.5);
      ...
      \draw (6,3.4) edge[->] (6,3.7);
  \end{scope}
  
  \node[circle,fill=white,draw=black,text=blue,inner sep=1pt] at (2,3) {$A_3$};
  \node[circle,fill=white,draw=black,text=blue,inner sep=1pt] at (3,1) {$A_2$};
  \node[circle,fill=white,draw=black,text=blue,inner sep=1pt] at (5,1) {$A_1$};
  \node[circle,fill=white,draw=black,text=black,inner sep=1pt] at (4,6) {$B_3$};
  \node[circle,fill=white,draw=black,text=black,inner sep=1pt] at (5,6) {$B_2$};
  \node[circle,fill=white,draw=black,text=black,inner sep=1pt] at (6,4) {$B_1$};
\end{tikzpicture}
\]
\caption{A NILP from the $3$-vertex $\tup{A_1, A_2, A_3}$ to the $3$-vertex $\tup{B_1, B_2, B_3}$. Here, $A_1 = \tup{5,1}, A_2 = \tup{3,1}$, and so on. The weights of the edges of the paths are written atop the edges. The weight of this NILP is $x_1 x_2 x_3 x_4^2$.}
