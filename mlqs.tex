...
Next, define the near standardization of $u$ to be the word $\std{u} := \std{u}_1 \cdots \std{u}_{n}$ obtained as follows. Let $k$ be minimal such that $\lvert \{ i \in \ive{n} \mid u_i < k \} \rvert = r$. Note that $k$ may not appear in $u$. Denote
\[
...
\end{enumerate}
In each of these cases, it is easy to see that all letters in $u$ starting in unbalanced positions in $C$ or $C'$ simply are assigned to the same column two rows down, and that the labeling procedure inside each balanced interval is the same in $C$ and $C'$.
...
\Travis{We might want to lift up some stuff from the final remarks into here.}
...
We say a word $w$ of type $\mm = (m_1, \ldots)$ is \defn{standard} if $m_i \leq 1$ for all $i$.
...
Next, we assume $\bigl( [w]_k \bigr)_{k=0}^n = \bigl( [w']_k \bigr)_{k=0}^n$.
