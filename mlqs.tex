...
\Travis{Just work through the cases.}
...
Next, define the \defn{$k$-near standardization} of a word $u \in \mcW_n$ of type $\mm$ to be the word $\std{u} := \std{u}_1 \cdots \std{u}_{n}$ obtained as follows.
Define
\[
...
\end{align*}
Note that the near standardization is order preserving in the sense
that $1 \leq i < j \leq n$ satisfy $u_i \geq u_j$ if and only if
either $\std{u}_i > \std{u}_j$ or $\std{u}_i = \std{u}_j = r+1$.
Additionally, note that if $u$ is a word with $\ell$ classes, for $k = \ell - 1$ the type $\widetilde{\mm} = (\widetilde{m}_1, \widetilde{m}_2, \ldots, \widetilde{m}_{r+1}, 0, 0, \ldots)$
of $\std{u}$ is given by $\widetilde{m}_i = 1$ for $i \leq r$ and $\widetilde{m}_{r+1} = m_k$.
For our case, we will only consider $k = \ell$ and simply say near standardization, and we have $W = B$ (but in the reverse order).
...
This is the set $\set{i, i+1, \ldots, j}$ when $i \leq j$ and the set $\set{i, i+1, \ldots, n, 1, 2, \ldots, j}$ when $i > j$.\footnote{We trivially consider an empty interval to be balanced, and note that if $i = j$, we have $\inter[i,j] = \ive{n}$.}
\darij{No, I don't think we should have $\inter[i,j] = \ive{n}$ for $i = j$. The case when $r_1 = r_2$ fucks up too much other stuff to keep it in; in particular, it breaks the cycling argument in the proof because we can't cycle to an unbalanced site anymore. Also, we need $\inter[i,j]$ to be $\set{i}$ when $i = j$, because how else could we get $\set{i}$ ?}
...
\begin{enumerate}
 ...
 % The set $\ive{n}$ of all sites is balanced if and only if $r_1 = r_2$.
 \item If $A$ and $B$ are two balanced intervals, then the three intervals
       $A \cap B$, $A \setminus B$ and $B \setminus A$ are balanced, and $A \cup B \neq \ive{n}$;
       moreover, the union $A \cup B$ is also balanced if it is an interval.
 \item Thus, the union of all balanced intervals (\textit{i.e.}, the set of all balanced
       $i \in \ive{n}$) is also the disjoint union of all inclusion-maximal balanced intervals.
...
\Travis{Quite possibly it should be a $5$. I think it is only used in the old proof. Probably should be moved to the appendix.}
...
Let $\zeta : \mcW_n \to \mcW_n$ be the ``cyclic shift'' map that sends each word $w_1 w_2 \cdots w_n$ to $w_2 w_3 \cdots w_n w_1$.
We also use the notation $\zeta$ for the map that sends each queue $q$ to the queue $\zeta q = \set{ i - 1 \mid i \in q }$ (recall that $0 = n$ as sites).
...
Thus, let $\qq = (q_1, q_2)$. We want to prove $\word(\fraks_1 \qq) = \sigma_1\bigl( \word(\qq) \bigr)$.
If $\abs{q_1} = \abs{q_2}$, then $\fraks_1 \qq = \qq$ (by the definition of $\fraks_1$) and $\sigma_1\bigl( \word(\qq) \bigr) = \word(\qq)$ (since the word $\word(\qq)$ has as many letters $1$ as it has letters $2$, but the map $\sigma_1$ leaves such words unchanged), so this claim is true.
Thus, we WLOG assume that $\abs{q_1} \neq \abs{q_2}$. Hence, in the construction of the dual configuration of $\qq = (q_1, q_2)$, at least one unbalanced site exists.

But the operator $\fraks_1$ commutes with the cyclic shift map $\zeta$ on MLQs (because $\zeta$ merely shifts the balanced intervals), and the operator $\sigma_1$ commutes with the cyclic shift map $\zeta$ on words in $A^*$ (this is~\cite[Prop.~5.6.1]{Loth}). Hence (and because of \eqref{pf.prop:braid.word-zeta}), we can apply $\zeta$ to $\qq$ any number of times without changing our goal. We thus WLOG assume that the site $1$ is unbalanced in the construction of the dual configuration of $\qq = (q_1, q_2)$ (since at least one unbalanced site exists, and we can force it to be $1$ by cyclic shifting). Thus, no balanced intervals ``wrap around the circle'' (\textit{i.e.}, each balanced interval has the form $\inter[i,j]$ with $i \leq j$).
...
Additionally, every matched pair of parentheses from sites $j \leq j'$ under the standard matching algorithm corresponds to the endpoints of a balanced interval $\inter[j,j']$. (This is easily proven by induction on the time at which the parentheses got matched: At this time, all the parentheses inbetween have already been matched, thus forming balanced intervals, and the newly matched pair merely extends it to both sides.)
...
Thus, if the algorithm would leave both a `)' and a `(' unmatched, then the (cyclic) interval between the rightmost unmatched `)' and the leftmost unmatched `(' would also be a balanced interval, which would contradict the fact that no balanced intervals ``wrap around the circle''.
...
\begin{remark}
Our letters $1, 2, \ldots, k$ correspond to the letters
$a_k, a_{k-1}, \ldots, a_1$ in~\cite{Loth},
...
Let $w \in \mcW_n$ be a word of type $\mm$.
...
Indeed, Theorem~\ref{thm:permutation} follows by induction on length, \textit{i.e.} the minimal number of simple transpositions needed to write $\sigma$.
...
Let $k \in \set{0, 1, \ldots, n}$.
Since $u$ is standard, we have $k \in \set{ p_i(\mm) \mid i \geq 0 }$,
where $\mm$ is the type of $u$.
Hence, $[u]_k$ is well-defined.
