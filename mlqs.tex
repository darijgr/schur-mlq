...
Hence, our proof is reduced to constructing the \defn{dual}\footnote{This is unrelated to the contragredient duality of a queue given by Lemma~\ref{le:dual}.} $(r_2,r_1)$-configuration to $C$, which we denote by $C'$, such that $C'$ satisfies $C(u) = C'(u)$, $\wt(C) = \wt(C')$, and $C'' = C$ (\textit{i.e.} the dual of $C'$ equals $C$).
...
Then for $u \in \set{1,2}^n$, we have $C(u)_i = C^*(u^*)^*_{n+1-i}$, where we treat $C^*(u^*)$ as a word with $4$ classes.
...
Note that every balanced interval $\mcI$ for $(q_1, q_2)$ corresponds to a balanced interval $\mcI^* = \set{ n+1-i \mid i \in \mcI }$ of $(q_2^*, q_1^*)$, where we recall that $q_i^*$ is the contragredient dual queue of $q_i$.
Hence, we have $(q_1,q_2)' = (q'_1, q'_2)$ if and only if $(q^*_2,q^*_1)' = \bigl( (q'_2)^*, (q'_1)^* \bigr)$\travis{Rectify this with Remark~\ref{rmk:balanced-dual-let}.}.
Also from Lemma~\ref{le:dual}, we have $q(u) = \bigl( q^*(u^*) \bigr)^*$ (recall that $q^*(u^*)$ has $\ell+1$ classes).

We have $[w]_k^* = [w^*]_{n-k}$, and note that $[w^*]_{n-k}$ are the reversed indices\travis{This part is awkward. Might need a term or more detail.} of the largest $n-k$ letters of $w$.
Now we have $\abs{q^*} = n - \abs{q} < n-k$, and so from a similar argument using Phase~I, we have $\tup{ q^*, [w^*]_{n-k} }' = \tup{ [q^*(w^*)]_{n-k}, q^{\dagger*} }$.
Applying contragredient duality, we obtain
\[
\tup{ q^{\dagger}, [q^*(w^*)]_{n-k}^* } = \tup{ q^{\dagger}, [q^*(w^*)^*]_k } = \tup{ q^{\dagger}, [q(w)]_k }
\]
as desired.
\end{proof}
