...
We have $j_{\kappa} \geq i_{\kappa}$ for all $\kappa \leq \abs{q}$ satisfying $u_{i_{\kappa}} \leq t-1$ (equivalently, for all for all $\kappa \leq \abs{q} - m_t$).
\end{claim}
...
Then either $j_{\kappa} = \min q$ or $\min q$ has already been set on a previous step of Phase~II, and in either case, we have $q(u)_{\min q} < t$.
Since $q(u)$ contains at least one $t$, there exists an $h > \kappa$ such that $q(u)_{j_h} = t$ with $j_h > \min q$.
