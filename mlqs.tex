...
\Darij{Clarify this definition a bit.}
...
\begin{subproof}
Suppose there exists a $\kappa \leq \abs{q} - m_t$ such that $j_{\kappa} < i_{\kappa}$ (\textit{i.e.}, the path of $u_{i_{\kappa}}$ ``wraps around the cycle'').
Without loss of generality, assume $\kappa$ is minimal.
Note that $u_{i_{\kappa}} < t$ since $\kappa \leq \abs{q} - m_t$.
Moreover, there does not exist an $h < \kappa$ such that $j_h = \min q$,
as otherwise this would violate the weakly decreasing up to level $t$ condition as $u_{i_h} < t$ as well.
