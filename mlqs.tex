...
\begin{description}
\item[Phase I]
  Go through all $i$ such that $u_i > t$ in such order that if $u_p > u_q$, then $p$ is processed before $q$.\footnote{The order in which equal letters are processed does not matter, as a simple argument shows.}
...
\begin{remark}
\label{rem:mod_alg}
In the algorithm above, we can consider Phase~III as part of Phase~I and Phase~II by having $t$ be added to every remaining unset $j \notin q$ and $j \in q$ respectively.
\end{remark}

There is an natural duality in the modified algorithm above.
...
We use the modified algorithm to compute $q \tup{u}$ and $q' \tup{v}$ from Remark~\ref{rem:mod_alg}, where we call the phases by Phase~I' and Phase~II'.
