...
\begin{enumerate}
 ...
 \Darij{It is not clear what it means for $\ive{n}$ to be balanced. The definition depends on the choices of starting and ending points.}
...
We want to prove that $(q'_1, q'_2)$ has the form $\tup{q^{\dagger} , [q(w)]_k }$.
In other words, we want to prove that $q'_2 = [q(w)]_k$.
If $k = \abs{q}$, then this is obvious (because in this case, the two queues in the configuration $C$ have the same size, so that its dual configuration $C'$ equals $C$, and thus $q'_2 = q$; but the assumption $k = \abs{q}$ also yields $[q(w)]_k = q$ because of the construction of $q(w)$, and therefore we obtain $q'_2 = q = [q(w)]_k$).
...
(in Phase~II of the algorithm computing $q(w)$) as the first site $j \in q$ weakly to the right of $i_p$ such that $q(w)_j$ is not set yet.
In other words, $j_p$ is the first site $j \in q$ weakly to the right of $i_p$ that is not one of $j_1, j_2, \ldots, j_{p-1}$
(since the sites $j \in q$ such that $q(w)_j$ has already been set are $j_1, j_2, \ldots, j_{p-1}$).
...
From $[w]_k = \set{i_1, i_2, \ldots, i_k}$, we obtain
\[
\inter[a,b] \cap [w]_k = \inter[a,b] \cap \set{i_1, i_2, \ldots, i_k} = \set{i_p \mid p \in R} .
\]
...
\]
Thus, for each $\ell \in \inter[a,b]$, the number $c^\uparrow[a,\ell] = \abs{\inter[a,\ell] \cap [w]_k}$ counts all of the $i_p$ with $p \in R$ that fall into the interval $\inter[a,\ell]$, while the number $c^\downarrow[a,\ell] = \abs{\inter[a,\ell] \cap q}$ counts all of the $j_p$ with $p \in R$ that fall into this interval.
Hence, the former number is at least as large as the latter number (because if $p \in R$ is such that $j_p$ falls into $\inter[a,\ell]$, then $i_p$ must also fall into $\inter[a,\ell]$, since $\inter[i_p, j_p] = I_p$ is a subinterval of $\inter[a,b]$).\footnote{Here we are tacitly using the fact that $\inter[a,b] \neq \ive{n}$, which follows from $U \neq \ive{n}$.}
...
Now, consider again the connected components of $U$. Each of them is top-heavy (as we have just shown), and thus contains at least as many elements of $[w]_k$ as it contains elements of $q$. Hence, the set $U$ altogether contains at least as many elements of $[w]_k$ as it contains elements of $q$, and this inequality becomes an equality only if each connected component of $U$ is balanced.
But this inequality does become an equality, because the set $U$ contains all $k$ elements of $[w]_k$ (since $[w]_k = \set{i_1, i_2, \ldots, i_k} \subseteq U$) and contains exactly $k$ elements of $q$ (since $U \cap q = \set{j_1, j_2, \ldots, j_k}$).
Thus, each connected component of $U$ is balanced. In other words, $U$ is balanced.
Hence, each element of $[q(w)]_k$ is balanced (since the construction of $q(w)$ yields $[q(w)]_k = \set{j_1, j_2, \ldots, j_k} = U \cap q \subseteq U$).
Due to how the dual configuration $(q'_1, q'_2)$ was defined, we thus conclude that each element of $[q(w)]_k$ is in $q'_2$. In other words, $[q(w)]_k \subseteq q'_2$.
Combining this with $\abs{q'_2} = \abs{[w]_k} = k = \abs{[q(w)]_k}$, we obtain $q'_2 = [q(w)]_k$. Thus, Proposition~\ref{prop:SL.dual} is proven in the case when $k < \abs{q}$.
