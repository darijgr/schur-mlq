...
We define \defn{$\SP(q_1, q_2)$} to be the following data: the sequence of parentheses obtained from $q_1$ and $q_2$; the matching obtained by the algorithm; and the correspondence between sites and \emph{closing} parentheses (\textit{i.e.}, which site contributes which closing parenthesis).
...
Hence, the parenthesis sequence $\SP(q_1, q_2)$ between $\alpha$ and $\gamma$ (inclusive) looks like this:
\[
\begin{array}{cccccc}
( & ( & ( & \cdots & ( & ) \\
\alpha & & & & \beta & \gamma ,
...
Denote $q_1^{(p)} := q_i \setminus \{i_1, i_2, \dotsc, i_p\}$ and $q_1^{(p)} := q_i \setminus \{j_1, j_2, \dotsc, j_p\}$.

\begin{statement}
\textbf{Claim:}
Let $p \in \set{0, 1, \ldots, k}$.
Then, the parenthesis sequence $\SP(q_1^{(p)}, q_2^{(p)})$ is obtained from $\SP(q_1 , q_2)$ by removing $p$ pairs of matched parentheses.
...
The site $j_p$ is chosen in Phase~II (since $p \leq k \leq \abs{q}$), and therefore $j_p$ is the first site weakly to the right of $i_p$ that belongs to $q_2 \setminus \set{j_1, j_2, \ldots, j_{p-1}}$.
Hence, Lemma~\ref{lem:SL.dual.1}
%(applied to $q_1 \setminus \set{i_1, i_2, \ldots, i_{p-1}}$, $q_2 \setminus \set{j_1, j_2, \ldots, j_{p-1}}$, $i_p$ and $j_p$ instead of $q_1$, $q_2$, $i$ and $j$)
...
The claim applied to $p = k$ shows that $\SP(q_1^{(k)}, q_2^{(k)})$ is obtained from $\SP(q_1 , q_2)$ by removing $k$ pairs of matched parentheses, and that the closing parentheses of these $k$ pairs are contributed by $j_1, j_2, \ldots, j_k$.
