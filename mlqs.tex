...
Then the type of the word $\qq(1 \dotsm 1)$ is the type of $\qq$ from Remark~\ref{rmk:t-splitting}.
\end{remark}
...
%\Travis{This is really just repeated applications of~\eqref{eq:queue_type_change} and induction on the number of queues. I really think this is completely obvious from how queues act on types. If you really need to ``prove'' a remark, which you do not here, it should not be a remark.}

\begin{remark}
Let $\ell \geq 1$ and $\sigma \in \SymGp{\ell-1}$.
Let $u$ be a packed word with $\ell$ classes and type $\mm$.
Then, there exists at least once MLQ $\qq$ of type $\mm$ satisfying $\qq(1 \dotsm 1) = u$.
In other words, $\swt{u}_{\sigma} \neq 0$ (as a polynomial over $\ZZ$).
\end{remark}
...
Here, we understand our sequence of parentheses as being written on a circle -- so the last opening parenthesis can be matched with the first closing parenthesis if all parentheses ``between them'' (i.e., to the right of the former and to the left of the latter) are frozen.
...
We define \defn{$\SP\tup{q_1, q_2}$} to be the following data: the sequence of parentheses obtained from $q_1$ and $q_2$; the matching obtained by the algorithm; and the correspondence between sites and \textbf{closing} parentheses (i.e., which site contributes which closing parenthesis).

Note that each site $j$ that belongs to both $q_1$ and $q_2$ contributes a pair of parentheses `()`, which can be immediately matched (to each other) at the beginning of the algorithm.
...
Hence, if we skip the sites $j$ that belong to both $q_1$ and $q_2$ in the definition of our sequence of parentheses, then the outcome of the algorithm (specifically, the set of unmatched parentheses) will be the same.
...
\end{equation}
To see this, we compute the dual configurations of both $C$ and $C^*$ using the reduced parenthesis sequences. The sequence for $C^*$ is obtained from that for $C$ by reflecting both the parentheses (i.e., opening become closing and vice versa) and the sequence itself. Thus, the parenthesis matching algorithm proceeds in the same way.
...
\begin{enumerate}
 \item Treat all letters $i$ in $p$ as opening parentheses `(',
       ...
       (i.e., not written on a circle, but having a beginning
       and an end).
...
Then, $\SP\tup{q_1 \setminus \set{i}, q_2 \setminus \set{j}}$ is obtained from $\SP\tup{q_1, q_2}$ by removing a pair of matched parentheses\footnote{The matching of all other parentheses remains the same, and so does the correspondence between sites and closing parentheses (except for the one we removed).}.
Moreover, the closing parenthesis of that pair is contributed by $j \in q_2$.
\end{lemma}

\begin{proof}
The assumption $\abs{q_1} \leq \abs{q_2}$ shows that all closing parentheses in $\SP\tup{q_1, q_2}$ are matched.
In particular, the closing parenthesis contributed by $j \in q_2$ is matched.
Let $\gamma$ be this closing parenthesis.
...
We know that $\gamma$ is matched; let $\beta$ be the opening parenthesis that is matched to $\gamma$.
Hence, the parenthesis sequence $\SP\tup{q_1, q_2}$ between $\alpha$ and $\gamma$ (inclusive) looks like this:
\[
\begin{array}{cccccc}
( & ( & ( & \cdots & ( & ) \\
\alpha & & & & \beta & \gamma ,
...
The parenthesis sequence $\SP\tup{q_1 \setminus \set{i}, q_2 \setminus \set{j}}$ is obtained from $\SP\tup{q_1, q_2}$ by removing $\alpha$ and $\gamma$.
But this is tantamount to removing $\beta$ and $\gamma$ from $\SP \tup{q_1, q_2}$, since both times we are simply shortening the string of opening parentheses before $\gamma$ by $1$ (and since we do not store the positions of the opening parentheses).
\end{proof}
...
Each site $j \in \ive{q(u)}_k$ contributes a matched closing parenthesis to $\SP\tup{\ive{u}_k, q}$.
\end{lemma}
...
Thus, we only need to prove that each of the sites $j_1, j_2, \ldots, j_k$ contributes a matched closing parenthesis to $\SP\tup{\ive{u}_k, q}$.

We will show a bit more:
\begin{statement}
\textit{Claim 1:} Let $p \in \set{0, 1, \ldots, k}$. Then, $\SP\tup{q_1 \setminus \set{i_1, i_2, \ldots, i_{p}}, q_2 \setminus \set{j_1, j_2, \ldots, j_{p}}}$ is obtained from $\SP\tup{q_1 , q_2 }$ by removing $p$ pairs of matched parentheses. The closing parentheses of these $p$ pairs are contributed by $j_1, j_2, \ldots, j_p$.
\end{statement}
[\textit{Proof of Claim 1:} We shall prove this by induction on $p$.
The induction base ($p = 0$) is obvious.
For the induction step, we assume (as induction hypothesis) that Claim 1 holds for $p-1$.
The site $j_p$ is chosen in Phase~II (since $p \leq k \leq \abs{q}$), and thus $j_p$ is the first site weakly to the right of $i_p$ that belongs to $q_2 \setminus \set{j_1, j_2, \ldots, j_{p-1}}$.
Hence, Lemma~\ref{lem:SL.dual.1} (applied to $q_1 \setminus \set{i_1, i_2, \ldots, i_{p-1}}$, $q_2 \setminus \set{j_1, j_2, \ldots, j_{p-1}}$, $i_p$ and $j_p$ instead of $q_1$, $q_2$, $i$ and $j$) shows that $\SP\tup{q_1 \setminus \set{i_1, i_2, \ldots, i_{p}}, q_2 \setminus \set{j_1, j_2, \ldots, j_{p}}}$ is obtained from $\SP\tup{q_1 \setminus \set{i_1, i_2, \ldots, i_{p-1}}, q_2 \setminus \set{j_1, j_2, \ldots, j_{p-1}}}$ by removing a pair of matched parentheses,
and moreover, the closing parenthesis of that pair is contributed by $j_p \in q_2 \setminus \set{j_1, j_2, \ldots, j_{p-1}}$.
Thus, Claim 1 follows from the induction hypothesis. This completes the induction step.]

Claim 1 (applied to $p = k$) shows that $\SP\tup{q_1 \setminus \set{i_1, i_2, \ldots, i_{k}}, q_2 \setminus \set{j_1, j_2, \ldots, j_{k}}}$ is obtained from $\SP\tup{q_1 , q_2}$ by removing $k$ pairs of matched parentheses, and that the closing parentheses of these $k$ pairs are contributed by $j_1, j_2, \ldots, j_k$. Hence, each of the sites $j_1, j_2, \ldots, j_k$ contributes a matched closing parenthesis to $\SP\tup{\ive{u}_k, q}$. This completes the proof.
\end{proof}
...
Suppose $k \leq \abs{q}$.
Then, Lemma~\ref{lem:SL.dual.3} shows that each site $j \in \ive{q(u)}_k$ contributes a matched closing parenthesis to $\SP\tup{\ive{u}_k, q}$.
