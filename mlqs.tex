...
The notations $c^{\uparrow}$ and $c^{\downarrow}$ (and the concept of balanced
intervals) refer to $C$.

Choose a permutation $\tup{i_1, i_2, \ldots, i_n}$ of $\tup{1, 2, \ldots, n}$
such that $w_{i_1} \leq w_{i_2} \leq \cdots \leq w_{i_n}$.
Use this permutation to construct $q(w)$ (as in the definition of $q(w)$).
...
Suppose first that $k \leq \abs{q}$.
Thus, each site in $[w]_k$ is balanced.
But $[w]_k = \set{i_1, i_2, \ldots, i_k}$ by the definition of the permutation.

If $S$ is a set of sites, then the \defn{connected components} of $S$ are the maximal intervals contained in $S$.

We say that a closed cyclic interval $\inter[i,j]$ is \defn{top-heavy} if for each $k \in \inter[i,j]$, we have $c^\uparrow[i,k] \geq c^\downarrow[i,k]$.
...
For each $p \in \set{0, 1, \ldots, k}$, we define a set $\mathcal{G}_p$ of sites
by
$\mathcal{G}_p = \inter[i_1, j_1] \cup \inter[i_2, j_2] \cup \cdots \inter[i_p, j_p]$.
We first claim that $\mathcal{G}_p \cap q = \set{j_1, j_2, \ldots, j_p}$
for each $p \in \set{0, 1, \ldots, k}$.
Indeed, we prove this by induction on $p$, the base case ($p=0)$ being obvious.
For the induction step (from $p-1$ to $p$), we recall that the site $j_p$ is found
(in Phase~II of the algorithm computing $q(w)$) by starting at $i_p$ and
moving weakly to the right in search of the first site $j \in q$ such that $q(w)_j$ is
not set yet.
The sites $j \in q$ such that $q(w)_j$ have already been set are $j_1, j_2, \ldots, j_{p-1}$,
and thus belong to the set
$\inter[i_1, j_1] \cup \inter[i_2, j_2] \cup \cdots \inter[i_{p-1}, j_{p-1}]$,
which (by the induction hypothesis) is top-heavy.


XXX
Hence, for each $p \in \ive{k}$, the site $i_p$ is balanced, so there is a
maximal balanced interval $\mcI_p$ such that $i_p \in \mcI_p$.
Now, we claim that $j_p \in \mcI_p$ for each $p \in \ive{k}$.
We prove this claim by strong induction on $p$:
Assume that $j_a \in \mcI_a$ has already been shown for all $a < p$.
We must show that $j_p \in \mcI_p$.
Assume the contrary. Thus, in particular, $j_p \neq i_p$.
Recall that in Phase~II of computing $q(w)$, the site $j_p$ is computed (by
moving weakly to the right of $i_p$ in search of the first site $j$ such that
$q(w)_j$ is not set yet), and $q(w)_{j_p}$ is set to $w_{i_p}$.
The definition of $j_p$ shows that every site $j \in \inter[i_p, j_p-1] \cap q$
must be one of the $j_a$ with $a < p$ (since otherwise, its value $q(w)_j$
would not be set yet, and $j_p$ would not be first).

For each $a < p$, 
XXXX

a letter $w_j$ moves weakly to the right until it finds the first unset $q(w)_{i_j}$ with $i_j \in q$.
...
Thus, assume that $u$ is standard of type $\mm$.
...
Let $k \in \set{ p_i(\nn) \mid i \geq 0}$.
Thus, $k \in \set{0, 1, \ldots, n} = \set{ p_i(\mm) \mid i \geq 0 }$ (since $u$ is standard).
...
We require a preparatory lemma, for which we need further notation.
...
%%%%%%%%%%
\subsection*{Part I: Reduction to special words}

For a word $u$ and indices $i,j$, we let $u_{i\leftrightarrow j}$ be the result of swapping positions $i$ and $j$ in $u$.

If $i$ and $j$ are two sites, then we let $\inter(i,j) := \inter[i,j] \setminus \set{i,j}$ denote the open (cyclic) interval.
...
%%%%%%%%%%
\subsection*{Part II: finishing the proof}
