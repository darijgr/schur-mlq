...
\begin{proof}
\darij{TODO!}
This follows either from \cite[Chapter 5, (5.6.3)]{Loth}, or
from crystal theory.
...
\begin{itemize}
\item we let $S_k \tup{w}$ denote the set of the indices $i \in \ive{n}$
	  corresponding to the $k$ smallest letters $w_i$ of $w$,
	  if this set is uniquely determined;
	  otherwise, we leave $S_k \tup{w}$ undefined.
\item we let $L_k \tup{w}$ denote the set of the indices $i \in \ive{n}$
	  corresponding to the $n-k$ largest letters $w_i$ of $w$,
	  if this set is uniquely determined;
	  otherwise, we leave $L_k \tup{w}$ undefined.
\end{itemize}
Thus, the queues $S_k \tup{w}$ and $L_k \tup{w}$ are defined exactly
when $k \in \set{ p_i \tup{\mm} \mid i \geq 0 }$ (with $\mm$
being the type of $w$), and otherwise are undefined.
Also, observe that $S_k \tup{w}$ and $L_k \tup{w}$ are two
...
the family of all queues $S_k \tup{w}$ (for the $k$ for which they
...
Let $q$ be any queue.
Then, the dual configuration of $\tup{ S_k \tup{w} , q }$
has the form $\tup{ ? , S_k \tup{q\tup{w}} }$
...
$C \tup{u} = q_2 \tup{ q_1 \tup{u} }$
and
$C' \tup{u} = q'_2 \tup{ q'_1 \tup{u} }$
have the same type;
...
$\tup{S_k \tup{u}, q_1, q_2}$ transforms under the map
...
\tup{?, ?, S_k \tup{ q_2 \tup{ q_1 \tup{u} } }} .
\]
Hence, $ S_k \tup{ q'_2 \tup{ q'_1 \tup{u} }
= S_k \tup{ q_2 \tup{ q_1 \tup{u} } }}$.
In other words,
$S_k \tup{ C' \tup{ u } } = S_k \tup{ C \tup{ u } }$.
