...
\begin{proof}
First, note that if $q$ is an $r$-queue, then $q'$ is an $(n-r)$-queue and that $q'$ is given by reflecting $[n] \setminus q$ across the middle of $[n]$.
For a fixed $k$, let $\ell + 1 - k$ be the dual letter of $k$.
...
\darij{Only if $p_{t-1} < r < p_t$ as strict inequalities. Otherwise, it is the dual letter plusminus 1.}
Therefore, Phase~I (resp.~II) in the construction of $q(u)$ corresponds to Phase~II (resp.~I) in the construction of $q'(v)$ by reversing the resulting word and taking dual letters.
...
But we can define $f_k$ as a sequence of merges on $u$, and so we have $q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j$ for all $k \in \ZZ_{\geq 1}$.
