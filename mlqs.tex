...
Let $\mm$ be the type of $u$, and suppose that $m_1 + \cdots + m_t \leq r < m_{t+1} + \cdots$.
\travis{There does not always exist such a $t$ as the example below shows.}
The output word $v = q(u)$ will have type $(m_1, \dots, m_t, r - (m_1+\dots+m_t), m_{t+1}-r, m_{t+2}, m_{t+3}, \ldots)$.
...
\begin{example}
We consider the $4$-queue $q = \{1, 4, 6, 7\}$ and consider the word $u = 3466121$. Thus, the type of $u$ is $\mm = (2, 1, 1, 1, 0, 2, 0, \ldots)$, and so $t = 3$. To compute $q(w)$:
\[
\begin{tikzpicture}
\node (i1) at (1,1) {$3$};
\node (i2) at (2,1) {$4$};
...
\node[circle,draw=black] (q3) at (6,0) {};
\node[circle,draw=black] (q4) at (7,0) {};
\draw[->,red] (i4) -- (q2);
\draw[->,red] (q2) -- (2,0) -- (t2);
\draw[->,red] (i3) -- (t3);
...
\end{tikzpicture}
\]
where the arrows in red are Phase I and the blue are Phase II.
\Travis{Something seems to be wrong with the definition. I cannot apply Phase II, quite possibly because $t$ is undefined. However, there are no conditions on $q$ to dictate that this is not allowed.}
\end{example}
...
\]
where we sum over all sequences $q_1, \dotsc, q_{r-1}$ of queues such that (i) $q_i$ is a $(m_1 + \cdots + m_{\sigma_i})$-queue and (ii) $u = q_{r-1}(\cdots q_2(q_1(1\dotsm 1)) \cdots )$.
\end{thm}
...
\begin{proof}
  ...
  Define a graph with node set $\{1,\dots,k\} \times \{1, \dots, n\}$ and edges $(i,j) \to (i,j+1)$ with weight $1$ and $(i,j) \to (i-1,j)$ with weight $x_j$. 

  We define $k$ sources $s_i = (k+1-i, 1)$ for $i\in[k]$ and $k$ terminals $t_j = (k-j+\gamma_j,b_j)$ for $j\in[k]$, where $\gamma_j$ is the number of distinct classes in $v_1\dots v_j$.
...
We refer to a pair $(q_1, q_2)$ as an $(r,s)$\emph{-configuration}.
Any $(r,s)$-configuration $C$ gives us a map on words by composition: $C(u) = q_2(q_1(u))$.
Furthermore, we let the \emph{weight} of $C$ simply be the product of weights of $q_1$ and $q_2$: $\wt(C)=\prod_{i\in q_1}x_i\prod_{j\in q_2}x_j$.
Note that to prove Theorem X, it is enough to show that the quantity in (*) does not change when applying an adjacent transposition to $\sigma$.
This we in turn prove by associating to each $(r,s)$-configuration $C$ a \emph{dual} $(s,r)$-configuration $C'$ in a bijective (in fact, involutive) manner, such that $C$ and $C'$ define the same function on words, and furthermore, the weight of $C$ and the weight of $C'$ equal. We split the proof into four parts, as follows.

In part A we describe how to split any configuration into \emph{balanced} and \emph{unbalanced} blocks. In part B we define the involution promised above. In part C we reduce the problem of showing that $C(u) = C'(u)$ for \emph{all} words $u$ to the showing it for a very restricted set of words $u$. Finally in part D we show that for this restricted set of words $u$, we indeed have $C(u) = C'(u)$.


%%%%%%%%%%
\subsection{Part A: splitting into balanced and unbalanced blocks}

Let $C=(q_1,q_2)$ be a configuration. We say that a closed cyclic interval $\inter[i,j]$ is \emph{balanced} if the following condition holds
\begin{itemize}
...
We call a maximal (with respect to inclusion) balanced closed cyclic interval in a \emph{balanced block} of $C$. For $i \in [n]$, we say that $i$ is \emph{balanced} if $i$ belongs to some balanced block, and \emph{unbalanced} otherwise. Note that for a balanced block $B$, we have $|q_1 \cap B| = |q_2 \cap B|$.
...
\begin{proof}
  ...
  If $q_1(u)_j = 1$ then $q_1(u) = q_1(u_{i\leftrightarrow j})$ again by Lemma~\ref{le:orig}. We are thus left with the case $q_1(u)_j = 2$.
  ...
  Then $q_1(u_{i\leftrightarrow i+1}) = q_1(u)_{i\leftrightarrow i+1}$, and for $l \in \{i+1,\dots, i+k\}$, we have $q_1(u)_l = 2$.
  Furthermore, by our assumptions, we see that for $l \in \{i+k+1, \dots, j-1\}$ we have $q_1(u)_l = 1$ for $l \in q_1$ and $q_1(u)_l = 3$ for $l \notin q_1$.
  Finally, $q_1(u)_j = 2$. From this we see that $q_2(q_1(u))_i = 3$.
  Now we apply (A VARIANT) of Lemma~\ref{le:orig} to $q_2, q_1(u), i, i+1$ to conclude that $C(u) = C(u_{i\leftrightarrow j})$.
\end{proof}
...
\begin{dfn}
A \defn{$(\sigma, \mm)$-multiline queue} is an $r \times N$ array with $m_1 + \cdots + m_{\sigma_i}$ boxes in row $i$ for $i = 1, \dotsc, r$.
\end{dfn}

By thinking of each row in a $(\sigma, \mm)$-MLQ as a generalized queue, we get a labeling of the MLQ.


\begin{thm}
  Let $u$ be a word of type $\mm$. The sum of the weights of all $(\sigma, \mm)$-MLQ's whose bottom row is labeled $u$ does not depend on the permutation $\sigma$.
\end{thm}
...
Suppose $|S_1| > |S_2|$ (the case $|S_1| < |S_2|$ is analogous, and the case $|S_1| = |S_2|$ is trivial). Then any such word can be uniquely written as an alternating concatenation $\dotsm UBUB \dotsm$ where $U$ refers to a word consisting of $u$'s only, and $B$ is a \defn{balanced} word. A word is balanced if, for each $k$, the first $k$ columns in the word has at least as many boxes in the upper row as in the bottom row.

\Erik{TODO: description in terms of Dyck paths.}
