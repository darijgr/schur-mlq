...
\begin{proof}
  ...
  \]
  Thus, we have that $\widetilde{\qq}$ is a $(s_{i-1} \dotsm s_1)$-twisted MLQ of type $\mm$ and $\wt(\widetilde{\qq}) = \wt(\qq) \wt(X) $.
  By Lemma~\ref{lemma:queue_merge}, we have
  \[
  v = \qq(1 \dotsm 1) = \merge{i} \qq\bigl( X(1 \dotsm 1) \bigr) = \merge{i} \widetilde{\qq}(1 \dotsm 1).
  \]
  By Theorem~\ref{thm:permutation}, there exists a weight preserving bijection $\omega$ between $(s_{i-1} \dotsm s_1)$-twisted MLQs $\widetilde{\qq}$ and ordinary MLQs $\widetilde{\qq}$ such that $\widetilde{\qq}(1 \dotsm 1) = u = \widetilde{\qq}'(1 \dotsm 1)$.
