...
\begin{lemma}
\label{le:mergeq}
Let $q$ be a queue, and $u \in \mcW_n$.
Let $h \in \set{1,2,\ldots}$.
Then, we have $q \tup{\merge{h} u} = \merge{h} q \tup{u}$ if $h < t$.
Also, $q \tup{\merge{h} u} = \merge{h+1} q\tup{u}$ if $h \geq t$.
\end{lemma}

\darij{todo: Prove this for real. The half-sentence below doesn't seem to do the $h = t-1$ and $h = t$ cases justice.}
