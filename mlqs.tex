...
Hence, we can apply the $k < \abs{q}$ case of Proposition~\ref{prop:SL.dual} to $n-k$, $u^*$ and $q^*$ instead of $k$, $u$ and $q$; we obtain
$\tup{ [u^*]_{n-k} , q^* }' = \tup{ * , [q^*(u^*)]_{n-k} }$, where $*$ denotes some queue we don't need.
In view of $q^*(u^*) = (q(u))^*$ (which follows from Lemma~\ref{le:dual}), this rewrites as 
$\tup{ [u^*]_{n-k} , q^* }' = \tup{ * , [(q(u))^*]_{n-k} }$.
...
we obtain $\bigl( (q'_1)^*, (q'_2)^* \bigr) = \tup{ * , ([q(u)]_k)^* }$,
