...
\begin{lemma}
\label{lem:no_wrap}
Let $u \in \mcW_n$ be a packed word with $\ell-1$ classes.
Let $q$ be a queue.
Assume that $\abs{q}$ is greater than the number of letters of $u$ that are $< \ell-1$.
Assume furthermore that the letters of $q(u)$ that are $< \ell$ are weakly decreasing
(from left to right).
Then, the letters of $u$ that are $< \ell-1$ are weakly decreasing.
\end{lemma}
...
We claim that there is no ``wrap-around'' in Phase II until the steps
when we process the $i$ for which $u_i = \ell-1$
(i.e., every time
...
where $i$ is some site with $u_i < \ell$, and
