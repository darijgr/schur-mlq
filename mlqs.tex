...
\newenvironment{subproof}{}{\hfill$\blacksquare$ \medskip}
...
distinct commuting indeterminates $\ldots,x_{-2},x_{-1},x_{0},x_1%
,x_2,\ldots$ instead of the $n$ indeterminates $x_1,x_2,\ldots,x_n$;
thus, we don't have $x_{n+k}=x_k$ for $k\in\mathbb{Z}$ in this section.


%%%%%%%%%%
\subsection{Lattice paths and the Lindstr\"{o}m--Gessel--Viennot theorem}

Our arguments will rely on the Lindstr\"om--Gessel--Viennot Lemma~\cite{GV85,Lindstrom73} and on a re-interpretation of MLQs as a certain kind of semistandard tableaux (of non-partition shape).
...
arcs are
\begin{align*}
(i,j) & \to (i,j+1) & & \text{for all } (i,j) \in \ZZ, \qquad \text{and} \\
(i,j) & \to (i+1,j) & & \text{for all } (i,j) \in \ZZ.
\end{align*}
...
For each vertex $v = (i,j)$, we set $\operatorname*{x}(v) = i$ and $\operatorname*{y}(v) = j$.
We refer to $\operatorname*{x}(v)$ (resp.~$\operatorname*{y}(v)$) as the \defn{$x$-coordinate} (resp.~\defn{$y$-coordinate}) of $v$.
...
\end{cases}
\]
Thus, all north-steps have weight $1$, while east-steps at $y$-coordinate $j$ have weight $x_j$.

Fix $k \in \NN$.
A \defn{$k$-vertex} shall mean a $k$-tuple of vertices of the lattice.
...
A \defn{path} will simply mean a (directed) path in the lattice.
...
\travis{This is not a (re)definition, but a simple direct substitution. It probably does not even need to be stated unless we assume our readers are completely incompetent.}

\Travis{This is almost certainly well-known and understood and could be an easy excerise for your 4707 students. This is also likely in Stanley (maybe implicitly/left to the reader). I think we should remove it (not even include in the long version).}
...
If $\tup{A_1, A_2, \dotsc, A_k}$ and $\tup{B_1, B_2, \dotsc, B_k}$ are two $k$-vertices, then a \defn{non-intersecting lattice paths (NILP)} from $\tup{A_1, A_2, \dotsc, A_k}$ to $\tup{B_1, B_2, \dotsc, B_k}$ shall mean a $k$-tuple $\tup{p_1, p_2, \dotsc, p_k}$ of paths such that
\begin{itemize}
\item each $p_i$ is a path from $A_i$ to $B_i$;
\item no two of the paths $p_1, p_2, \dotsc, p_k$ have a vertex in common.
\end{itemize}
(Note that the paths must neither cross nor touch.)

The \defn{weight} of a NILP $\pp = \tup{p_1, p_2, \dotsc, p_k}$ is defined as
\[
\wt \pp := \prod_{i=1}^{k} \wt(p_i).
\]
Note that this weight is a monomial.

\Darij{TODO: Picture of a NILP with weight. Don't align the sources or targets on a line.}

If $\uu$ and $\vv$ are two $k$-vertices, then \defn{$N(\uu,\vv)$} denotes the set of all NILPs from $\uu$ to $\vv$.

\begin{verlong}
A pair $\left(  \uu,\vv\right)  $ of two $k$-vertices
$\uu$ and $\vv$ is said to be \defn{nonpermutable} if and only
if every permutation $\sigma\neq\operatorname*{id}$ in $\mathfrak{S}_k$
satisfies $N\left(  \uu,\sigma\left(  \vv\right)  \right) = \emptyset$. Note that we are not requiring that $N\left(  \uu, \vv \right) \neq \emptyset$ here.

The following fact (a particular case of~\cite[Corollary 2]{GesVie89}) is crucial:

\begin{prop}
\label{prop.LGV.nonper}Let $k\in\mathbb{N}$. Let $\left(  \uu%
,\vv\right)  $ be a nonpermutable pair of two $k$-vertices
$\uu=\left(  A_1,A_2,\ldots,A_k\right)  $ and $\vv=\left(
B_1,B_2,\ldots,B_k\right)  $. Then,%
\[
\sum_{\pp\in N\left(  \uu,\vv\right)  }\wt %
\pp=\det\left(  \left(  \sum_{p\in N\left(  A_i,B_j\right)
}\wt p\right)  _{1\leq i\leq k,\ 1\leq j\leq k}\right)  .
\]
...
\begin{lemma}
\label{lem.LGV.hex}Let $A$, $B$, $A'$ and $B'$ be four
vertices such that%
\[
\operatorname*{x}\left(  A'\right)  \leq\operatorname*{x}\left(
A\right)  ,\qquad\operatorname*{y}\left(  A'\right)  \geq
\operatorname*{y}\left(  A\right)  ,\qquad\operatorname*{x}\left(  B^{\prime
}\right)  \leq\operatorname*{x}\left(  B\right)  ,\qquad\operatorname*{y}%
\left(  B'\right)  \geq\operatorname*{y}\left(  B\right)  .
\]
Let $p$ be a path from $A$ to $B'$. Let $p'$ be a path from
$A'$ to $B$. Then, $p$ and $p'$ have a vertex in common.
\end{lemma}
...
+\ell\left(  p'\right)  <N$. We must now prove that Lemma
\ref{lem.LGV.hex} holds when $\ell\left(  p\right)  +\ell\left(  p^{\prime
}\right)  =N$.

So let $A$, $B$, $A'$, $B'$, $p$ and $p'$ be as in
Lemma~\ref{lem.LGV.hex}, and let us assume that $\ell\left(  p\right)
+\ell\left(  p'\right)  =N$. We must prove that $p$ and $p'$
...
from $A$ to $B'$ shows that $\operatorname*{x}\left(  A\right)
\leq\operatorname*{x}\left(  B'\right)  $ and $\operatorname*{y}%
\left(  A\right)  \leq\operatorname*{y}\left(  B'\right)  $.
Similarly, the existence of a path $p'$ from $A'$ to $B$
yields $\operatorname*{x}\left(  A'\right)  \leq\operatorname*{x}%
\left(  B\right)  $ and $\operatorname*{y}\left(  A'\right)
...
$A$ to $B'$). Hence, $\operatorname*{y}\left(  A\right)
=\operatorname*{y}\left(  B'\right)  \geq\operatorname*{y}\left(
B\right)  $, so that $\operatorname*{y}\left(  A'\right)
\geq\operatorname*{y}\left(  A\right)  \geq\operatorname*{y}\left(  B\right)
$. Combining this with $\operatorname*{y}\left(  A'\right)
\leq\operatorname*{y}\left(  B\right)  $, we obtain $\operatorname*{y}\left(
A'\right)  =\operatorname*{y}\left(  B\right)  $. Combining
$\operatorname*{y}\left(  A'\right)  \geq\operatorname*{y}\left(
A\right)  $ with $\operatorname*{y}\left(  A'\right)
=\operatorname*{y}\left(  B\right)  \leq\operatorname*{y}\left(  A\right)  $,
we obtain $\operatorname*{y}\left(  A'\right)  =\operatorname*{y}%
\left(  A\right)  $. Thus, $\operatorname*{y}\left(  A'\right)
...
segment between $A'$ and $B$ (since $\operatorname*{x}\left(
A'\right)  \leq\operatorname*{x}\left(  \underbrace{A}_{=B'%
}\right)  =\operatorname*{x}\left(  B'\right)  \leq\operatorname*{x}%
\left(  B\right)  $).
...
would have $\operatorname*{y}\left(  A'\right)  <\operatorname*{y}%
\left(  B\right)  $ (since $p'$ is a path from $A'$ to $B$).
But this would contradict $\operatorname*{y}\left(  A'\right)
\geq\operatorname*{y}\left(  B\right)  $. Hence, the path $p'$ has no
...
$\operatorname*{x}\left(  A'\right)  =\operatorname*{x}\left(
B\right)  \geq\operatorname*{x}\left(  B'\right)  $, so that
$\operatorname*{x}\left(  A\right)  \geq\operatorname*{x}\left(  A^{\prime
}\right)  \geq\operatorname*{x}\left(  B'\right)  $. Combining this
with $\operatorname*{x}\left(  A\right)  \leq\operatorname*{x}\left(
B'\right)  $, we obtain $\operatorname*{x}\left(  A\right)
=\operatorname*{x}\left(  B'\right)  $. Combining $\operatorname*{x}%
\left(  A\right)  \geq\operatorname*{x}\left(  A'\right)  $ with
$\operatorname*{x}\left(  A'\right)  \geq\operatorname*{x}\left(
B'\right)  =\operatorname*{x}\left(  A\right)  $, we obtain
$\operatorname*{x}\left(  A\right)  =\operatorname*{x}\left(  A^{\prime
}\right)  $. Thus, $\operatorname*{x}\left(  A'\right)
...
on the line segment between $A$ and $B'$ (since $\operatorname*{y}%
\left(  A'\right)  \geq\operatorname*{y}\left(  A\right)  $ and
$\operatorname*{y}\left(  \underbrace{A'}_{=B}\right)
...
$B'$ (namely, on the path $p$). Therefore, $\operatorname*{x}\left(
...
Moreover, $\operatorname*{x}\left(  A'\right)  \leq\operatorname*{x}%
...
we cannot have $\operatorname*{y}\left(  A'\right)  \geq
\operatorname*{y}\left(  P\right)  $. Thus, $\operatorname*{y}\left(
A'\right)  <\operatorname*{y}\left(  P\right)  $. Hence,
$\operatorname*{y}\left(  A'\right)  \leq\operatorname*{y}\left(
P\right)  -1$ (since $\operatorname*{y}\left(  A'\right)  $ and
...
\operatorname*{y}\left(  A'\right)  <\operatorname*{y}\left(
...
A'\right)  \leq\operatorname*{y}\left(  P\right)  -1=\operatorname*{y}%
\left(  A\right)  $ (since $\operatorname*{y}\left(  P\right)
=\operatorname*{y}\left(  A\right)  +1$) with $\operatorname*{y}\left(
A'\right)  \geq\operatorname*{y}\left(  A\right)  $, we obtain
$\operatorname*{y}\left(  A'\right)  =\operatorname*{y}\left(
A\right)  $.
...
$p'$). Therefore, $\operatorname*{x}\left(  A'\right)
\leq\operatorname*{x}\left(  P'\right)  \leq\operatorname*{x}\left(
B\right)  $ (since the $x$-coordinates of the vertices a path are always weakly
increasing) and $\operatorname*{y}\left(  A'\right)  \leq
\operatorname*{y}\left(  P'\right)  \leq\operatorname*{y}\left(
...
Moreover, $\operatorname*{y}\left(  P'\right)  \geq\operatorname*{y}%
\left(  A'\right)  \geq\operatorname*{y}\left(  A\right)  $. If we had
$\operatorname*{x}\left(  P'\right)  \leq\operatorname*{x}\left(
...
P'\right)  \leq\operatorname*{x}\left(  A\right)  $. Thus,
$\operatorname*{x}\left(  P'\right)  >\operatorname*{x}\left(
A\right)  $. Hence, $\operatorname*{x}\left(  P'\right)
\geq\operatorname*{x}\left(  A\right)  +1$ (since $\operatorname*{x}\left(
P'\right)  $ and $\operatorname*{x}\left(  A\right)  $ are integers).

But $P'$ is the next vertex after $A'$ on the path $p^{\prime
}$. Hence, there is an arc from $A'$ to $P'$. If this arc was
a north-step, then we would have $\operatorname*{x}\left(  P'\right)
=\operatorname*{x}\left(  A'\right)  $, which would contradict
$\operatorname*{x}\left(  A'\right)  \leq\operatorname*{x}\left(
A\right)  <\operatorname*{x}\left(  P'\right)  $. Hence, this arc
cannot be a north-step. Thus, this arc must be an east-step. Therefore,
$\operatorname*{y}\left(  P'\right)  =\operatorname*{y}\left(
A'\right)  $ and $\operatorname*{x}\left(  P'\right)
=\operatorname*{x}\left(  A'\right)  +1$. Hence, $\operatorname*{x}%
\left(  A'\right)  +1=\operatorname*{x}\left(  P'\right)
\geq\operatorname*{x}\left(  A\right)  +1$, so that $\operatorname*{x}\left(
A'\right)  \geq\operatorname*{x}\left(  A\right)  $. Combining this
with $\operatorname*{x}\left(  A'\right)  \leq\operatorname*{x}\left(
A\right)  $, we obtain $\operatorname*{x}\left(  A'\right)
...
A'\right)  =\operatorname*{y}\left(  A\right)  $). Hence, these two
...
Hence, the existence of a path $p$ from $A$ to $B'$ shows that $\operatorname*{x}(A) \leq \operatorname*{x}(B')$ and $\operatorname*{y}(A) \leq \operatorname*{y}(B')$.
Similarly, the existence of $p'$ yields $\operatorname*{x}(A') \leq \operatorname*{x}(B')$ and $\operatorname*{y}(A') \leq\operatorname*{y}(B)$.

Thus,
\begin{align}
\operatorname*{x}\left(  A'\right)   &  \leq\operatorname*{x}\left(
A\right)  \leq\operatorname*{x}\left(  B'\right)  \leq
...
\[
x=\operatorname*{x}\left(  A'\right)  ,\qquad y=\operatorname*{y}%
\left(  A\right)  ,\qquad x=\operatorname*{x}\left(  B\right)  ,\qquad
y=\operatorname*{y}\left(  B'\right)  ,
\]
...
h_n\left(  \widetilde{p'}\right)  \right)  <\operatorname*{x}\left(
h_n\left(  \widetilde{p}\right)  \right)  $, whereas all sufficiently high
$N\in\mathbb{Z}$ satisfy $\operatorname*{x}\left(  h_n\left(
\widetilde{p'}\right)  \right)  >\operatorname*{x}\left(  h_n\left(
\widetilde{p}\right)  \right)  $. Hence, the set of all $N\in\mathbb{Z}$ that
satisfy $\operatorname*{x}\left(  h_n\left(  \widetilde{p'}\right)
\right)  <\operatorname*{x}\left(  h_n\left(  \widetilde{p}\right)  \right)
...
\widetilde{p'}\right)  \right)  }_{<\operatorname*{x}\left(
...
\]
Therefore, $\operatorname*{x}\left(  h_{M+1}\left(  \widetilde{p'%
...
\url{https://math.stackexchange.com/questions/2870640} (applied to $\omega_{a} = \wt a$) for a detailed derivation of Proposition~\ref{prop.LGV.concrete}.}
...
\begin{align*}
\operatorname*{x}(A_1) & \geq \operatorname*{x}(A_2) \geq \cdots \geq \operatorname*{x}(A_k); \\
\operatorname*{y}(A_1) & \leq \operatorname*{y}(A_2) \leq \cdots \leq \operatorname*{y}(A_k); \\
\operatorname*{x}(B_1) & \geq \operatorname*{x}(B_2) \geq \cdots \geq \operatorname*{x}(B_k); \\
\operatorname*{y}(B_1) & \leq \operatorname*{y}(B_2) \leq \cdots \leq \operatorname*{y}(B_k).
\end{align*}
Then,
\[
\sum_{\pp \in N(\uu,\vv)} \wt \pp = \det\left(  \sum_{p\in N(A_i,B_j) } \wt p\right)_{1\leq i\leq k,\ 1\leq j\leq k} .
\]
...
Let $\sigma\neq\operatorname*{id}$ be a permutation in $\mathfrak{S}_k$. We
...
$\operatorname*{x}(A_j) \leq \operatorname*{x}(A_i)$, $\operatorname*{y}(A_j) \geq \operatorname*{y}(A_i)$, $\operatorname*{x}(B_{\sigma(i)}) \leq \operatorname*{x}(B_{\sigma(j)})$, and $\operatorname*{y}(B_{\sigma(i)}) \geq \operatorname*{y}(B_{\sigma(j)})$.
...
The \defn{diagram $[\lambda]$} of a $k$-pseudo-partition $\lambda = \tup{\lambda_1, \lambda_2, \dotsc,\lambda_k}$ is defined as the set
$\left\{  \left(  i,j\right)  \in \ive{k} \times \left\{1,2,3,\ldots\right\}  \mid  j \leq \lambda_i \right\}$.
...
If $\lambda = \tup{\lambda_1, \lambda_2, \dotsc, \lambda_k}$ is a $k$-pseudo-partition, then a \defn{tableau of shape $\lambda$} is a map $T \colon [\lambda] \to \set{1,2,3,\ldots}$.
For each such tableau $T$ and each $(i,j) \in [\lambda]$, we refer to the value $T(i,j)$ as the \defn{entry of $T$ in cell $\left(i,j\right)$}.
As usual, we represent a tableau $T$ of shape $\lambda$ by placing the entry $T(i,j)$ into the box corresponding to $(i,j) \in [\lambda]$.
...
\begin{itemize}
\item the entries of $T$ are weakly increasing along each row (\textit{i.e.}, we have $T(i,j_1) \leq T(  i,j_2)$ whenever $(i,j_1)  \in [\lambda]$ and $(i,j_2) \in [\lambda]$ satisfy $j_1 < j_2$);

\item the entries of $T$ are strictly increasing down each column (\textit{i.e.}, we have $T(i_1,j) < T(i_2,j)$ whenever $(i_1,j) \in [\lambda]$ and $(i_2,j) \in [\lambda]$ satisfy $i_1 < i_2$).
\end{itemize}

For example, the second tableaux in~\eqref{eq.tableau.exas} is semistandard, while the first is not.
...
\[
\wt(T) := \prod_{(i,j) \in [\lambda]} x_{T(i,j)}
\]
...
Then,
\begin{align*}
&  \sum_{\substack{T \text{ is a semistandard tableau}\\\text{of shape } \lambda \text{ and surface }s }} \wt T \\
&  = \left(  \prod_{i=1}^{k} x_{s_i} \right)  \det\left(  h_{\lambda_j-j+i-1}(  x_1,x_2,\ldots,x_{s_j})  \right)_{1\leq i\leq k,\ 1\leq j\leq k} .
\end{align*}
\end{thm}
...
\begin{proof}
[Proof of Theorem~\ref{thm.tableau.jt}.]Define two $k$-vertices $\uu%
=\left(  A_1,A_2,\ldots,A_k\right)  $ and $\vv=\left(
B_1,B_2,\ldots,B_k\right)  $ by%
\[
A_i=\left(  k-i,1\right)  \qquad\text{and}\qquad B_i=\left(  \lambda
_i+k-i-1,s_i\right)  .
\]
...
\begin{align*}
\sum_{\pp \in N(\uu,\vv)} \wt \pp & = \det\left(  \sum_{p \in  N(A_i,B_j)}\wt p \right)_{1\leq i\leq k,\ 1\leq j\leq k}
\\ & = \det\left( h_{\lambda_j-j+i-1}(x_1, x_2, \dotsc, x_{s_j}) \right)_{1\leq i\leq k,\ 1\leq j\leq k},
\end{align*}
...
\[
\Phi \colon N(\uu, \vv) \to \set{ \text{semistandard tableaux of shape } \lambda \text{ and surface } s}.
\]
...
Now, let $T$ be the tableau of shape $\lambda$ that sends each $(i,j) \in [\lambda]$  to $p_{i,j}$.
...
If the column $j$ of $[\lambda]$ has no gaps between row $i_1$ and row $i_2$ (that is, if we have $(i,j) \in [\lambda]$ for each $i \in [i_1, i_1+1, \dotsc, i_2]$), then this follows from~\eqref{pf.thm.tableau.jt.col-strict-no-gap}.
Hence, without loss of generality, we assume that column $j$ of $[\lambda]$ has at least one gap between row $i_1$ and row $i_2$.
In other words, there exists some $q \in [i_1, i_1+1, \dotsc, i_2]$ such that $(q,j) \notin [\lambda]$.
...
From~\eqref{pf.thm.tableau.jt.row-weak}, we obtain $p_{i_1,j}\leq s_{i_1%
}\leq s_{q}$ (since $i_1<q$, but the sequence $s$ is weakly increasing).
...
and surface $s$ (since $T\left(  i,\lambda_i\right)  =p_{i,\lambda_i%
}=s_i$ for all $i\in\ive{k}$). So let us set%
\[
...
\lambda \text{ and surface } s}} \wt(T) \\
&  = \sum_{\pp\in N(\uu,\vv)  }\left(
\prod_{i=1}^{k}x_{s_i}\right)  \wt \pp = \left( \prod_{i=1}^{k}x_{s_i}\right)  \sum_{\pp \in N(\uu,\vv)  } \wt \pp\\
&  = \left(  \prod_{i=1}^k x_{s_i} \right)  \det\left(
h_{\lambda_j-j+i-1}\left(  x_1,x_2,\ldots,x_{s_j}\right)  \right)_{1\leq i\leq k,\ 1\leq j\leq k} .
\end{align*}
\end{proof}
...
If $A$ is a finite set of integers, and if $k \in \ive{\abs{A}}$, then \defn{$\max_k A$} denotes the $k$-th largest element of $A$.
For example, $\max\nolimits_2 \set{4,5,6,8} = 6$.

Next, we define two binary relations $\succeq$ and $\gg$ on the powerset of $\ive{n}$:

\begin{itemize}
\item Given two subsets $A$ and $B$ of $\ive{n}$, we say that \defn{$A\succeq B$} if and only if $\abs{ I } = \abs{J}$ and every $k\in \ive{ \abs{I} }$ satisfies $\max_k A \geq \max_k B$.
...
Let $A$ and $B$ be two subsets of $\ive{n}$, and let $\phi \colon B \to A$ be a bijection.
Assume that $\phi(b) \geq b$ for each $b \in B$.
Then, $A \succeq B$.
\end{lemma}
...
We say that the MLQ $\mathbf{q}=\left(  q_1,\ldots,q_{\ell-1}\right)  $ is
\defn{interlacing} if each $i\in\left\{  2,3,\ldots,\ell-1\right\}  $
satisfies%
\begin{equation}
q_i^{\left(  1\right)  }\succeq q_{i-1}^{\left(  1\right)  }\gg
q_i^{\left(  2\right)  }\succeq q_{i-1}^{\left(  2\right)  }\gg
q_i^{\left(  3\right)  }\succeq q_{i-1}^{\left(  3\right)  }\gg\cdots\gg
q_i^{\left(  i\right)  } \label{eq.determinant_form.interlacing.def}%
\end{equation}
(that is, $q_i^{\left(  j\right)  }\succeq q_{i-1}^{\left(  j\right)  }\gg
q_i^{\left(  j+1\right)  }$ for all $j\in\left[  i-1\right]  $).
...
\end{equation}
Thus, the columns of the diagram $[\lambda]$ are aligned to the bottom, and have lengths $p_{\ell-1}(\mm), p_{\ell-2}(\mm), \dotsc, p_1(\mm)$ (from left to right).
...
\[
P \colon \left\{  \text{MLQs of type } \mm \right\} \to \left\{ \text{tableaux of shape } \lambda \right\}
\]
...
Now, we define the map $P$ as follows: If $\mathbf{q}=\left(  q_1%
,q_2,\ldots,q_{\ell-1}\right)  $ is any MLQ of type $\mathbf{m}$, then we
let $P(\qq)$ be the tableau of shape $\lambda$ that
looks as follows:%
\begin{equation}
...
\begin{itemize}
\item The entries of $P \tup{\mathbf{q}}$ are weakly increasing along each row
if and only if all $i \in \ive{\ell-1}$ and $j \in \ive{i-1}$ satisfy
$q_i^{\left(j\right)} \succeq q_{i-1}^{\left(j\right)}$.
\item The entries of $P \tup{\mathbf{q}}$ are strictly increasing down each
...
(Namely, we can construct this $\qq$ by recovering the sets $q_i^{(j)}$ from the appropriate cells of $T$ in~\eqref{pf.lem:determinant_form.bij1.visual}, and combine them to obtain queues $q_i$ and an MLQ $\qq$.
...
Hence, we have proven all four properties (a), (b), (c) and~(d).
\end{proof}
...
Consider the injection $P$ from Lemma~\ref{lem:determinant_form.bij1}.
Now recall the four properties (a)--(d) in Lemma~\ref{lem:determinant_form.bij1}.
...
\begin{align*}
\sum_{\substack{T\text{ is a semistandard tableau}\\\text{of shape }%
\lambda\text{ and surface }s}}\wt T &  =\underbrace{\sum
_{\substack{\mathbf{q}=\left(  q_1,q_2,\ldots,q_{\ell-1}\right)  \text{ is
an interlacing}\\\text{MLQ of type }\mathbf{m}\text{ such that}\\\text{the
surface of }P(\qq)\text{ is }s}}}_{\substack{=\sum
_{\substack{\mathbf{q}=\left(  q_1,q_2,\ldots,q_{\ell-1}\right)  \text{ is
an interlacing}\\\text{MLQ of type }\mathbf{m}\text{ such that}\\\text{the
list of all elements of }q_{\ell-1}\text{ in}\\\text{increasing order is }%
s}}\\\text{(by property (b))}}}\underbrace{\wt \left(  P\left(
\mathbf{q}\right)  \right)  }_{\substack{=\wt \mathbf{q}%
\\\text{(by property (a))}}}\\
&  =\sum_{\substack{\mathbf{q}=\left(  q_1,q_2,\ldots,q_{\ell-1}\right)
\text{ is an interlacing}\\\text{MLQ of type }\mathbf{m}\text{ such
that}\\\text{the list of all elements of }q_{\ell-1}\text{ in}%
\\\text{increasing order is }s}}\wt \mathbf{q}\\
&  =\sum_{\substack{\mathbf{q}=\left(  q_1,q_2,\ldots,q_{\ell-1}\right)
\text{ is an interlacing}\\\text{MLQ of type }\mathbf{m}\text{ with }%
q_{\ell-1}=S}}\wt \mathbf{q}%
\end{align*}
(since the list of all elements of $q_{\ell-1}$ in increasing order is $s$ if and only if $q_{\ell-1}=S$).
Therefore, the claim of Corollary~\ref{cor:determinant_form.bij1c} follows from Theorem~\ref{thm.tableau.jt}.
\end{proof}

We need another notation: If $u\in\mathcal{W}_n$ and $t\in\mathbb{N}$, then
we say that $u$ is \defn{weakly decreasing up to level $t$} if and only if
every two sites $i<j$ in $\left[  n\right]  $ satisfying $u_j\leq t$ must
satisfy $u_i\geq u_j$. Equivalently, $u$ is weakly decreasing up to level
$t$ if and only if $u$ becomes weakly decreasing when all letters larger than
$t$ are removed. For example, the word ${\color{gray}5}4{\color{gray}55}4332{\color{gray}5}221{\color{gray}5}$ is weakly decreasing up
to level $4$ (and up to any level $\leq4$).

\begin{lemma}
\label{lem:determinant_form.interl-A}Let $u\in\mathcal{W}_n$ and $t>0$.
...
\[
\set{ p \in \ive{n} \mid \bigl( q(u) \bigr)_p = h} \succeq \set{  p \in \ive{n} \mid u_p = h} \gg \set{ p \in \ive{n} \mid \bigl( q(u) \bigr)_p = h+1} .
\]
\end{enumerate}
\end{lemma}
...
Fix a permutation $\left(  i_1,i_2,\ldots,i_n\right)  $ of $\left(  1,2,\ldots,n\right)  $
such that $u_{i_1}\leq u_{i_2}\leq\cdots\leq u_{i_n}$. Recall the
...
which $i=i_k$; we let $j_k$ denote the site $j$ that is found during this
...
u\right)  \right)  _{j_k}=u_{i_k}$ if $k\leq\left\vert q\right\vert $, and
otherwise $\left(  q\left(  u\right)  \right)  _{j_k}=u_{i_k}+1$. This
...
\begin{align}
\left(  q\left(  u\right)  \right)  _j  & \leq t\qquad\text{for each }j\in
q; \label{pf.lem:determinant_form.interl-A.1} \\
\left(  q\left(  u\right)  \right)  _j  & =t+1\qquad\text{for each }j\notin
q \label{pf.lem:determinant_form.interl-A.2} .
\end{align}
...
Applying this to $j_{?} = j_{!}$, we conclude that the letter $\bigl( q(u) \bigr)_{j_{!}}$ must already be set.
Moreover, this letter must have been set in Phase~II (because the site $j_{!}$ belongs to $q$, whereas Phase~I only sets letters in sites outside of $q$).
Hence, this letter must be $\leq u_{i_k}$ (because all letters that have been set in Phase~II before the current step are $\leq u_{i_k}$).
Thus, $\bigl( q(u) \bigr)_{j_{!}} \leq u_{i_k}$.
Also, $\bigl( q(u) \bigr)_{j_k}=u_{i_k}$ (due to how Phase~II works).
Combining these, we obtain $\bigl( q(u) \bigr)_{j_{!}} \leq \bigl( q(u) \bigr)_{j_k}=u_{i_k}$.
...
This contradicts $j_k\notin q$, and so Claim~\ref{claim:fixed_k_decreasing_i_from_j}
\end{subproof}

\begin{claim}
\label{claim:smaller_site_jbang}
Let $k\in \ive{n} $ be such that $k \leq \abs{q}$.
Let $j_{!}$ be a site in $q$ such that $j_{!}\geq i_k$.
...
Define
\begin{align*}
A & := \left\{ p\in \ive{n} \mid \bigl( q(u) \bigr)_p = h \right\},
\\ B & := \left\{ p\in \ive{n} \mid u_p = h \right\},
\\ C & := \left\{ p\in \ive{n} \mid \bigl( q(u) \bigr)_p = h+1 \right\}.
\end{align*}
...
Also, if we had $c\notin q$, then~\eqref{pf.lem:determinant_form.interl-A.2} would yield $\bigl( q(u) \bigr)_{c} = t+1$, which would contradict $\bigl( q(u) \bigr)_c = \underbrace{h}_{<t} + 1 < t + 1$.
Thus, we have $c\in q$.
...
Next, we shall connect the interlacingness of an MLQ with its action on the word $1\cdots1$:

\begin{lemma}
\label{lem:determinant_form.interl-act}
Let $\ell$ be a positive integer. Let
$B = \left\{  b_1 < b_2 < \cdots < b_r \right\}  \subseteq \ive{n}$.
Let $\left( v_1 \geq v_2 \geq \dotsm \geq v_r \right)$ be a weakly decreasing $r$-tuple of integers such that $\left\{ v_1, v_2, \dotsc, v_r \right\} = \ive{\ell-1}$.
...
Then, $u = \qq(1\cdots1)$ if and only if the MLQ $\qq$ is interlacing and satisfies $q_{\ell-1} = B$.
\end{lemma}
...
For each $i\in\left\{  0,1,\ldots,\ell-1\right\}  $, let $u^{\left[  i\right]} \in \mcW_n$ be the word $\left(  q_1,q_2,\ldots,q_i\right) \left(  1\cdots1\right)  = q_i\left(  q_{i-1}\left(  \cdots\left( q_1\left(  1\cdots1\right)  \right)  \right)  \right)$.
This word $u^{\left[  i\right]  }$ has type $\left(  m_1, m_2, \dotsc, m_i, n-p_i(\mm), 0, 0, \ldots \right)$.
(This is proven similarly to Lemma~\ref{rmk:mlq-type}, using $\abs{q_i} = m_i$.)
...
\end{equation}
The definition of $u^{\left[  \ell-1\right]  }$ yields $u^{\left[\ell-1\right]  } = \qq(1\cdots1)$.
...
We have $u = \qq(1\cdots1) = u^{\left[  \ell-1\right]  }$.
...
\Darij{For the induction step, TODO using Lemma~\ref{lem:determinant_form.interl-A}(a).}
\end{subproof}

\begin{claim}
\label{claim:description_q_i^(j)}
Let $i\in \ive{\ell-1}$ and $j \in \ive{i}$.
Then, $q_i^{(j)}$ is the set of all $p \in \ive{n}$ satisfying $( u^{\left[  i\right] } )_p = j$.
\end{claim}

\begin{subproof}
\Darij{Explain how this follows from Claim~\ref{claim:weakly_decreasing_level_i} and the type of $u^{\left[  i\right]  }$. TODO}
\end{subproof}
...
\begin{subproof}
Descending induction on $j$.
Use Claim~\ref{claim:weakly_decreasing_level_i} and Claim~\ref{claim:description_q_i^(j)} too.
\Darij{TODO Use Lemma~\ref{lem:determinant_form.interl-A}(b).}
\end{subproof}
...
Then
\[
\swt{u} = \left(  \prod_{i \in B} x_i \right)  \det \left(  h_{\gamma_j+i-j-1}(x_1,\dotsc,x_{b_j}) \right)_{1 \leq i \leq r,\ 1 \leq j \leq r}.
\]
\end{thm}
...
Moreover, the definition of $\mathbf{m}$ shows that $r=m_1+m_2%
...
type }\mathbf{m};\\u=\mathbf{q}\left(  1\cdots1\right)  }}\wt %
\mathbf{q}\\
&  = \sum_{\substack{\mathbf{q}=\left(  q_1,q_2,\ldots,q_{\ell-1}\right) \text{ is an MLQ of type }\mathbf{m};\\\mathbf{q}\text{ is interlacing and satisfies } q_{\ell-1}=B}}\wt \mathbf{q}\qquad\left(  \text{by Lemma~\ref{lem:determinant_form.interl-act}}\right) \\
&  = \sum_{\substack{\mathbf{q}=\left(  q_1,q_2,\ldots,q_{\ell-1}\right) \text{ is an interlacing} \\\text{MLQ of type }\mathbf{m} \text{ with } q_{\ell-1}=B}}\wt \mathbf{q}\\
&  = \underbrace{\left(  \prod_{i=1}^{k}x_{b_i}\right)  }_{=\prod_{i \in B} x_i} \det\Biggl(  \underbrace{h_{\lambda_j-j+i-1}\left(x_1,x_2,\ldots,x_{b_j}\right)  }_{\substack{=h_{\gamma_j+i-j-1}(x_1,\dotsc,x_{b_j})\\\text{(since $\lambda_j=\gamma_j$)}}} \Biggr)_{1\leq i\leq k,\ 1\leq j\leq k} \\
&  \qquad\left(
  \begin{array}[c]{c}
  \text{by Corollary~\ref{cor:determinant_form.bij1c}, applied to } B \text{ and } b_i\\
  \text{instead of } S \text{ and } s_i
  \end{array}
  \right) \\
&  = \left( \prod_{i\in B} x_i \right)  \det\left(  h_{\gamma_j+i-j-1}(x_1,\dotsc,x_{b_j}) \right)_{1\leq i\leq r,\ 1\leq j\leq r} .
\end{align*}
\end{proof}
...
% We say $\mcI \subseteq \ZZ / n \ZZ$ is a \defn{balanced set} if each $i \in \mcI$ is balanced.
% If $\mcI$ is connected (as a subset of the cycle graph), we call $\mcI$ a \defn{balanced interval}.
...
% \begin{enumerate}
 % \item For any inclusion-maximal balanced interval $\mcI$, we have $\lvert q_1 \cap \mcI \rvert = \lvert q_2 \cap \mcI \rvert$.
 % \item The empty set is balanced.
