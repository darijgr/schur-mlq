...
Assume also that $q(u)$ has exactly $\abs{q}$ letters that are not $t$\travis{This was ``at most $t$'', but that is all $n$ letters because $q(u)$ is up to level $t$. So I am assuming you meant strictly less than $t$ (which is equivalent to those not being $t$).}.
Then:
