...
For example, the two tableaux in~\eqref{eq.tableau.exas} have surfaces $(3,1,1,7)$ and $(3,3,5,5)$, respectively.
...
\begin{vershort}
Indeed, let $\tup{p_1, p_2, \dotsc, p_k} \in N(\uu,\vv)$ be an arbitrary NILP; thus, $p_i$ is a path from $A_i$ to $B_i$.
\end{vershort}
\begin{verlong}
...
Define the $r$-pseudo-partition $\lambda^{\mm}$ by~\eqref{eq.determinant_form.interlacing.lam} (using $r$ instead of $k$, since $r = p_{\ell-1}(\mm)$).
