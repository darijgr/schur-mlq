...
\begin{lemma}[Duality]
  ...
  Define a new word $u^*$ by letting $u^*_i = \ell + 1 - u_{n+1-i}$ and a new queue $q^*$ by letting $i \in q^*$ if and only if $n+1-i \notin q$.
 Then $q(u)_i = \ell + 2 - q^*(u^*)_{n+1-i}$ for all $i$.
\end{lemma}
...
Thus, $u^*$ is obtained by reversing $u$ and taking the dual letters.

Phase~I (resp.~II) in the construction of $q(u)$ corresponds to Phase~II (resp.~I) in the construction of $q^*(u^*)$ when the word is reversed and the letters are replaced by their duals.
...
Namely, for $k \in \set{p_i(\mm) \mid i \geq 0}$, then
\begin{itemize}
\item let $S_k(w)$ denote the set of the indices $i \in \ive{n}$
	  corresponding to the $k$ smallest letters $w_i$ of $w$;
\item let $L_k(w)$ denote the set of the indices $i \in \ive{n}$
	  corresponding to the $n-k$ largest letters $w_i$ of $w$.\travis{This set does not seem to be used currently.}
\end{itemize}
Observe that $S_k(w)$ and $L_k(w)$ are two complementary subsets of $\ive{n}$.
...
Let $w, w' \in \mcW_n$.
Then $\bigl( S_k(w) \bigr)_{k=0}^n = \bigl( S_k(w') \bigr)_{k=0}^n$ if and only if $w = w'$.
\end{lemma}

\begin{proof}
Clearly $w = w'$ implies $\bigl( S_k(w) \bigr)_{k=0}^n = \bigl( S_k(w') \bigr)_{k=0}^n$.
Next, we assume $\bigl( S_k(w) \bigr)_{k=0}^n = \bigl( S_k(w') \bigr)_{k=0}^n$.
Note that $w$ and $w'$ have the same type $\mm$.
Fix some $k$, and let $j$ be such that $k = p_j(\mm)$.
Thus, we have $S_k(w) \setminus S_{k-p_j(\mm)}(w) = S_k(w') \setminus S_{k-p_j(\mm)}(w')$ defined, which we denote by $P_k$.
\travis{This version of $k$ smallest letters is actively fighting us here.}
By construction, we have $w_i = j = w'_i$ for all $i \in P_k$.
...
Let $q$ be a queue.
The dual configuration of $\tup{ S_k(w) , q }$ has the form $\tup{q^{\dagger} , S_k\bigl(q(w)\bigr) }$, where $q^{\dagger}$ is some queue.
\end{prop}

\begin{proof}
Let $C' = (q'_1, q'_2)$ denote the dual configuration of $C = \tup{ S_k(w) , q }$.
The notations of $c^{\uparrow}$ and $c^{\downarrow}$ refer to $C$.

Suppose $k \leq \abs{q}$.
Therefore, for all $j \in S_k(w)$, there exists a maximal balanced interval $\mcI_j$ such that $j \in \mcI_j$.
...
Hence, $q'_2 \subseteq S_k\bigl( q(w) \bigr)$.
However, we clearly have $\abs{q'_2} = k = \abs{S_k\bigl( q(w) \bigr)}$, and so $q'_2 = S_k\bigl( q(w) \bigr)$.

For $k > \abs{q}$, by a similar argument as above, we have that the dual configuration of $\tup{ \ive{n} \setminus S_k(w) , \ive{n} \setminus q }$ has the form $\tup{\ive{n} \setminus S_k\bigl(q(w)\bigr), q^{\dagger} }$.\travis{This isn't quite right, but I will figure it out.}

Note that every balanced block for $(q_1, q_2)$ corresponds to a balanced block of $(q_2^*, q_1^*)$, where $q_i^*$ is the queue from Lemma~\ref{le:dual}. Also from Lemma~\ref{le:dual}, we have $q(u) = (q^*(u^*))^*$ (recall that $q^*(u^*)$ has $\ell+1$ classes).
\end{proof}
...
$\qq = \tup{S_k(u), q_1, q_2}$ transforms under the action of
$\fraks_1 \fraks_2 \fraks_1$: Namely, we have
\begin{align*}
\tup{S_k(u), q_1, q_2} & \overset{\fraks_1}{\longmapsto} \tup{\ast, S_k\bigl(q_1(u)\bigr), q_2}
\\ & \overset{\fraks_2}{\longmapsto} \tup{\ast, \ast, S_k\bigl(q_2(q_1(u)) \bigr) }
\\ & \overset{\fraks_1}{\longmapsto} \tup{\ast, \ast, S_k\bigl(q_2(q_1(u)) \bigr)},
\end{align*}
\Travis{Do you like it better in picture form?}
\[
...
\node (i3) at (0,-2) {$q_2$};
\node (s11) at (1,0) {$\ast$};
\node (s12) at (1,-1) {$S_k\bigl(q_1(u) \bigr)$};
\node (s13) at (1,-2) {$q_2$};
\node (s21) at (2,0) {$\ast$};
\node (s22) at (2,-1) {$\ast$};
\node (s23) at (2,-2) {$S_k\bigl(q_2(q_1(u)) \bigr)$};
\node (t1) at (3,0) {$\ast$};
\node (t2) at (3,-1) {$\ast$};
\node (t3) at (3,-2) {$S_k\bigl(q_2(q_1(u)) \bigr)$};
...
\begin{align*}
\tup{S_k(u), q_1, q_2} & \overset{\fraks_2}{\longmapsto} \tup{S_k(u), q'_1, q'_2}
\\ & \overset{\fraks_1}{\longmapsto} \tup{\ast, S_k\bigl(q'_1(u) \bigr) , q'_2}
\\ & \overset{\fraks_2}{\longmapsto} \tup{\ast, \ast, S_k\bigl(q'_2( q'_1(u)) \bigr)} .
\end{align*}
\[
\begin{tikzpicture}[xscale=3]
\node (i1) at (0,0) {$S_k(u)$};
\node (i2) at (0,-1) {$q_1$};
\node (i3) at (0,-2) {$q_2$};
\node (s11) at (1,0) {$S_k(u)$};
\node (s12) at (1,-1) {$q'_1$};
\node (s13) at (1,-2) {$q'_2$};
\node (s21) at (2,0) {$\ast$};
\node (s22) at (2,-1) {$S_k\bigl(q'_1(u) \bigr)$};
\node (s23) at (2,-2) {$q'_2$};
\node (t1) at (3,0) {$\ast$};
\node (t2) at (3,-1) {$\ast$};
\node (t3) at (3,-2) {$S_k\bigl(q'_1(q'_2(u)) \bigr)$};
...
MLQs must be identical:
\[
\tup{\ast, \ast, S_k\Bigl( q'_2\bigl(q'_1(u) \bigr) \Bigr)}
=
\tup{\ast, \ast, S_k\Bigl( q_2\bigl(q_1(u) \bigr) \Bigr)}.
\]
Hence, we have
\[
S_k\bigl( q'_2(q'_1(u) ) \bigr) = S_k\bigl( q_2(q_1(u)) \bigr).
\]
In other words, $S_k \bigl( C'(u) \bigr) = S_k \bigl( C(u) \bigr)$.
\end{proof}
