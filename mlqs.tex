...
\begin{enumerate}
 ...
 \Travis{It should be stated that every $i \in \ive{n}$ is balanced.}
...
Moreover, we have $\sigma_1\bigl( \word(\qq) \bigr) = \word(\qq)$ since the word $\word(\qq)$ has as many letters $1$ as it has letters $2$, but the map $\sigma_1$ leaves such words unchanged.
Hence, the claim is holds when $\abs{q_1} = \abs{q_2}$.
Thus, we assume that $\abs{q_1} \neq \abs{q_2}$.
Therefore, there exists at least one unbalanced site in $\qq = (q_1, q_2)$.
...
Hence, along with~\eqref{pf.prop:braid.word-zeta}, we can apply $\zeta$ to $\qq$ any number of times without changing our goal\travis{This diction needs to be tweaked.}.
Thus, without loss of generality, we assume that the site $1$ is unbalanced in the construction of the dual configuration of $\qq = (q_1, q_2)$ since at least one unbalanced site $j$ exists and we can cyclic shift until it is $1$.
Therefore, no minimal balanced interval has the form $\inter[i,j]$ with $i > j$).
...
In order to prove $\swt{u}_{\sigma} = \swt{u}_{\sigma s_i}$, we need to show for a $\sigma$-twisted MLQs $\qq$ of type $\mm$ satisfying $u = \qq (1 \dotsm 1)$ and $\fraks_i \qq$ is a $\sigma s_i$-twisted MLQs of type $\mm$ satisfying $u = (\fraks_i \qq) (1 \dotsm 1)$.
The only nontrivial part is showing $u = (\fraks_i\qq) (1 \dotsm 1)$.
The proof of this claim is reduced to showing that for any configuration $C = \tup{q_1, q_2}$ and any word $w \in \mcW_n$ the dual configuration $\fraks_1 C = C' = (q'_1, q'_2)$ of $C$ satisfies $C'(w) = C(w)$.
