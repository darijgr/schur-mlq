...
\item Given two subsets $A$ and $B$ of $\left[  n \right]  $, we say that
\defn{$A \gg B$} if and only if every $a \in A$ and $b \in B$ satisfy
$a > b$.
\end{itemize}
...
$.
Note that $A \gg \varnothing$ and $\varnothing \gg A$ for any subset
$A$ of $\ive{n}$ (for vacuous reasons), so that $\gg$ is not a partial
order (but it becomes a partial order if we forbid $\varnothing$).

The following criterion (proof left to the reader) will be useful:

\begin{lemma} \label{lem:determinant_form.gale1}
Let $A$ and $B$ be two subsets of $\ive{n}$, and let $\phi : B \to A$
be a bijection.
Assume that $\phi\tup{b} \geq b$ for each $b \in B$.
Then, $A \succeq B$.
\end{lemma}
...
increasing\footnote{In more detail:
\begin{itemize}
...
$q_{i}^{\left(  j\right)  }$ are nonempty. This ensures that the southern
neighbor of a cell in $\widetilde{q_{i}^{\left(  j\right)  }}$ belongs either
to $\widetilde{q_{i}^{\left(  j\right)  }}$ again or to
$\widetilde{q_{i-1}^{\left(  j-1\right)  }}$, rather than (say) to
$\widetilde{q_{i-2}^{\left(  j-2\right)  }}$.)
\end{itemize}
});
in other words, the tableau $P\left(  \mathbf{q}\right)  $ is
...
\begin{statement}
\textit{Claim 1:} Let $k\in \left[ n\right] $ be such that $k\leq \left\vert
q\right\vert $. Let $j_{!}$ be a site in $q$ that lies between $i_{k}$
(inclusive) and $j_{k}$ (inclusive) on the circle $\mathbb{Z}/n\mathbb{Z}$.
(If $i_{k}>j_{k}$, then this means that $j_{!}\geq i_{k}$ or $j_{!}\leq j_{k}
$; otherwise, it means that $i_{k}\leq j_{!}\leq j_{k}$.) Then, $\left(
q\left( u\right) \right) _{j_{!}}\leq \left( q\left( u\right) \right)
_{j_{k}}=u_{i_{k}}$.
\end{statement}

[\textit{Proof of Claim 1:} Consider the unique step of the construction of $%
q\left( u\right) $ at which $i=i_{k}$. This step is part of Phase~II, since $%
k\leq \left\vert q\right\vert $. Therefore, this step sees $j_{k}$ being
defined as the first $j$ weakly to the right (cyclically) of $i=i_{k}$ such
that $j\in q$ and $\left( q\left( u\right) \right) _{j}$ is not set. Also, $%
\left( q\left( u\right) \right) _{j_{k}}=u_{i_{k}}$ (due to how Phase~II
works). If $j_{!}=j_{k}$, then Claim 1 follows immediately from this. Thus,
let us WLOG assume that $j_{!}\neq j_{k}$. Hence, the site $j_{!}$ lies
between $i_{k}$ (inclusive) and $j_{k}$ (\textbf{exclusive}) on the circle $%
\mathbb{Z}/n\mathbb{Z}$.

Recall again that $j_{k}$ is the first $j$ weakly to the right (cyclically)
of $i=i_{k}$ such that $j\in q$ and $\left( q\left( u\right) \right) _{j}$
is not set. Hence, for every site $j_{?}$ in $q$ that lies between $i_{k}$
(inclusive) and $j_{k}$ (exclusive), the letter $\left( q\left( u\right)
\right) _{j_{?}}$ must already be set. Applying this to $j_{?}=j_{!}$, we
conclude that the letter $\left( q\left( u\right) \right) _{j_{!}}$ must
already be set. Moreover, this letter must have been set in Phase~II
(because the site $j_{!}$ belongs to $q$, whereas Phase~I only sets letters
in sites outside of $q$). Hence, this letter must be $\leq u_{i_{k}}$
(because all letters that have been set in Phase~II before the current step
are $\leq u_{i_{k}}$). Thus, $\left( q\left( u\right) \right) _{j_{!}}\leq
u_{i_{k}}$. Also, $\left( q\left( u\right) \right) _{j_{k}}=u_{i_{k}}$ (due
to how Phase~II works). Combining these, we obtain $\left( q\left( u\right)
\right) _{j_{!}}\leq \left( q\left( u\right) \right) _{j_{k}}=u_{i_{k}}$.
This proves Claim 1.]

\begin{statement}
\textit{Claim 2:} Let $k\in \left[ n\right] $ be such that $k\leq \left\vert
q\right\vert $ and $u_{i_{k}}<t$. Then, $i_{k}\leq j_{k}$.
\end{statement}

[\textit{Proof of Claim 2:} Assume the contrary. Then, $i_{k}>j_{k}$. Hence,
the interval between $i_{k}$ and $j_{k}$ \textquotedblleft wraps
around\textquotedblright\ the circle (\textit{i.e.}, it is an interval only
in $\mathbb{Z}/n\mathbb{Z}$, not in $\left[ n\right] $).

Let $j_{\min }$ be the smallest element of $q$. Then, %
\eqref{pf.lem:determinant_form.interl-A.1} and %
\eqref{pf.lem:determinant_form.interl-A.2} reveal that $\left( q\left(
u\right) \right) _{j_{\min }}$ is the first among the letters of $q\left(
u\right) $ that are $\leq t$, and therefore also the largest of these
letters (since $q\left( u\right) $ is weakly decreasing up to level $t$).
But this largest letter must be $t$ (since the word $q\left( u\right) $ has
at least one letter equal to $t$). Hence, $\left( q\left( u\right) \right)
_{j_{\min }}=t$.

If the site $j_{\min }$ would lie between $i_{k}$ (inclusive) and $j_{k}$
(inclusive) on the circle $\mathbb{Z}/n\mathbb{Z}$, then Claim 1 (applied to 
$j_{!}=j_{\min }$) would yield $\left( q\left( u\right) \right) _{j_{\min
}}\leq \left( q\left( u\right) \right) _{j_{k}}=u_{i_{k}}$; this would
contradict $\left( q\left( u\right) \right) _{j_{\min }}=t>u_{i_{k}}$.
Hence, the site $j_{\min }$ cannot lie between $i_{k}$ (inclusive) and $j_{k}
$ (inclusive). Since the interval between $i_{k}$ and $j_{k}$
\textquotedblleft wraps around\textquotedblright\ the circle, this shows
that $j_{\min }>j_{k}$. Thus, $j_{k}\notin q$ (since $j_{\min }$ is the
smallest element of $q$). But $k\leq \left\vert q\right\vert $, and thus $%
\left( q\left( u\right) \right) _{j_{k}}$ is set in Phase~II; therefore, $%
j_{k}\in q$. This contradicts $j_{k}\notin q$. This contradiction shows that
our assumption was false, and so Claim 2 is proven.]

\begin{statement}
\textit{Claim 3:} Let $k\in \left[ n\right] $ be such that $k\leq \left\vert
q\right\vert $. Let $j_{!}$ be a site in $q$ such that $j_{!}\geq i_{k}$.
Then, $\left( q\left( u\right) \right) _{j_{!}}\leq \left( q\left( u\right)
\right) _{j_{k}}=u_{i_{k}}$.
\end{statement}

[\textit{Proof of Claim 3:} If $j_{!}\leq j_{k}$, then Claim 3 follows from
Claim 1. Hence, WLOG assume that we don't have $j_{!}\leq j_{k}$. Thus, $%
j_{!}>j_{k}$.

Consider the unique step of the construction of $q\left( u\right) $ at which 
$i=i_{k}$. This step is part of Phase~II, since $k\leq \left\vert
q\right\vert $. Recall that $j_{k}$ is the value of $j$ found during this
step; thus, $j_{k}\in q$ (since this step is part of Phase~II). Also, $%
\left( q\left( u\right) \right) _{j_{k}}=u_{i_{k}}$ (due to how Phase~II
works). Both $j_{k}$ and $j_{!}$ belong to $q$; hence, both letters $\left(
q\left( u\right) \right) _{j_{!}}$ and $\left( q\left( u\right) \right)
_{j_{k}}$ are $\leq t$ (by \eqref{pf.lem:determinant_form.interl-A.1}).
Hence, from $j_{k}<j_{!}$, we obtain $\left( q\left( u\right) \right)
_{j_{k}}\geq \left( q\left( u\right) \right) _{j_{!}}$ (since $q\left(
u\right) $ is weakly decreasing up to level $t$). Thus, $\left( q\left(
u\right) \right) _{j_{!}}\leq \left( q\left( u\right) \right)
_{j_{k}}=u_{i_{k}}$. This proves Claim 3.]

We are now ready to prove the two parts of the lemma:

(a) Assume the contrary. Thus, there exist sites $x$ and $y$ in $\left[ n%
\right] $ such that $x<y$ and $u_{x}<u_{y}<t$. Let $k$ and $\ell $ be such
that $x=i_{k}$ and $y=i_{\ell }$; then, $u_{i_{k}}=u_{x}<u_{y}=u_{i_{\ell }}$
and thus $u_{i_{k}}<u_{i_{\ell }}=u_{y}<t$. Hence, $k<\ell $ (since $%
u_{i_{k}}<u_{i_{\ell }}$ and $u_{i_{1}}\leq u_{i_{2}}\leq \cdots \leq
u_{i_{n}}$). Due to $u_{i_{k}}<t$, we know that the letter $\left( q\left(
u\right) \right) _{j_{k}}$ must be set in Phase~II of the algorithm (since
otherwise, it would be set in Phase~I, which would yield $\left( q\left(
u\right) \right) _{j_{k}}=u_{i_{k}}+1<t+1$, contradicting the fact that all
letters set during Phase~I are $t+1$). Hence, $k\leq \left\vert q\right\vert 
$ and $\left( q\left( u\right) \right) _{j_{k}}=u_{i_{k}}$. Similarly, $\ell
\leq \left\vert q\right\vert $ and $\left( q\left( u\right) \right)
_{j_{\ell }}=u_{i_{\ell }}$. Hence, $\left( q\left( u\right) \right)
_{j_{k}}=u_{i_{k}}<u_{i_{\ell }}=\left( q\left( u\right) \right) _{j_{\ell }}
$. Since the letters $\left( q\left( u\right) \right) _{j_{k}}$ and $\left(
q\left( u\right) \right) _{j_{\ell }}$ of $q\left( u\right) $ are also $\leq
t$ (because $\left( q\left( u\right) \right) _{j_{k}}=u_{i_{k}}=u_{x}<t$ and 
$\left( q\left( u\right) \right) _{j_{\ell }}=u_{i_{\ell }}=u_{y}<t$), this
entails $j_{k}>j_{\ell }$ (since $q\left( u\right) $ is weakly decreasing up
to level $t$). From Claim 2, we have $i_{k}\leq j_{k}$ and similarly $%
i_{\ell }\leq j_{\ell }$. Thus, $i_{k}=x<y=i_{\ell }\leq j_{\ell }<j_{k}$.
Thus, the site $j_{\ell }$ lies between $i_{k}$ (inclusive) and $j_{k}$
(inclusive). Therefore, Claim 1 (applied to $j_{!}=j_{\ell }$) yields $%
\left( q\left( u\right) \right) _{j_{\ell }}\leq \left( q\left( u\right)
\right) _{j_{k}}=u_{i_{k}}$. This contradicts $\left( q\left( u\right)
\right) _{j_{\ell }}=u_{i_{\ell }}>u_{i_{k}}=\left( q\left( u\right) \right)
_{j_{k}}$. This contradiction shows that our assumption was false, and so
Lemma~\ref{lem:determinant_form.interl-A}(a) is proven.

(b) Let $h\in \left[ t-1\right] $. Thus, $h<t$.

Note that $\left( j_{1},j_{2},\ldots ,j_{n}\right) $ is a permutation of $%
\left( 1,2,\ldots ,n\right) $ (since each letter of $q\left( u\right) $ is
set exactly once during the construction of $q\left( u\right) $). Thus, the $%
j_{1},j_{2},\ldots ,j_{n}$ are distinct.

If $k\in \left[ n\right] $ satisfies $i_{k}\in B$, then%
\begin{equation}
k\leq \left\vert q\right\vert   \label{pf.lem:determinant_form.interl-A.b.1}
\end{equation}%
\footnote{\textit{Proof.} Let $k\in \left[ n\right] $ be such that $i_{k}\in
B$. We must prove that $k\leq \left\vert q\right\vert $.
\par
Assume the contrary. Thus, $k>\left\vert q\right\vert $.
\par
From $i_{k}\in B$, we obtain $u_{i_{k}}=h$ (by the definition of $B$).
\par
Consider the unique step of the construction of $q\left( u\right) $ at which 
$i=i_{k}$. This step is part of Phase~I, since $k>\left\vert q\right\vert $.
Hence, $j_{k}\notin q$ and $\left( q\left( u\right) \right)
_{j_{k}}=u_{i_{k}}+1$ (by the way Phase I operates). Thus, $%
u_{i_{k}}+1=\left( q\left( u\right) \right) _{j_{k}}=t+1$ (by %
\eqref{pf.lem:determinant_form.interl-A.2}, since $j_{k}\notin q$), so that $%
u_{i_{k}}=t$. This contradicts $u_{i_{k}}=h<t$. This contradiction shows
that our assumption was wrong, qed.}

Let $A=\left\{ p\in \left[ n\right] \ \mid \ \left( q\left( u\right) \right)
_{p}=h\right\} $, $B=\left\{ p\in \left[ n\right] \ \mid \ u_{p}=h\right\} $
and $C=\left\{ p\in \left[ n\right] \ \mid \ \left( q\left( u\right) \right)
_{p}=h+1\right\} $. Then, we must prove that $A\succeq B\gg C$.

Let us first prove $A\succeq B$. Indeed, let $i_{k}\in B$ for some $k\in %
\left[ n\right] $. From $i_{k}\in B$, we obtain $u_{i_{k}}=h$ (by the
definition of $B$), so that $u_{i_{k}}=h<t$. Also, %
\eqref{pf.lem:determinant_form.interl-A.b.1} yields $k\leq \left\vert
q\right\vert $. Thus, the letter $\left( q\left( u\right) \right) _{j_{k}}$
is set during Phase II of the construction. Hence, $\left( q\left( u\right)
\right) _{j_{k}}=u_{i_{k}}=h$. Therefore, $j_{k}\in A$ (by the definition of 
$A$). Also, Claim 2 yields $i_{k}\leq j_{k}$. Hence, $j_{k}\geq i_{k}$.

Forget that we fixed $k$. We thus have shown that if $i_{k}\in B$ for some $%
k\in \left[ n\right] $, then $j_{k}\in A$ and $j_{k}\geq i_{k}$. Hence, we
can define a map%
\begin{equation*}
\phi :B\rightarrow A,\ i_{k}\mapsto j_{k}
\end{equation*}%
(since each element of $B$ can be written as $i_{k}$ for a unique $k\in %
\left[ n\right] $), and it satisfies $\phi \left( b\right) \geq b$ for each $%
b\in B$ (since $j_{k}\geq i_{k}$). This map $\phi $ is furthermore injective
(since the algorithm shows that the $j_{1},j_{2},\ldots ,j_{n}$ are
distinct) and surjective\footnote{\textit{Proof.} Let $a\in A$. We must
prove that there exists some $k\in \left[ n\right] $ satisfying $i_{k}\in B$
and $j_{k}=a$.
\par
Indeed, recall that $\left( j_{1},j_{2},\ldots ,j_{n}\right) $ is a
permutation of $\left( 1,2,\ldots ,n\right) $. Thus, there exists some $k\in %
\left[ n\right] $ satisfying $j_{k}=a$. Consider this $k$. All we need to
prove now is that $i_{k}\in B$.
\par
We have $j_{k}=a$ and thus $\left( q\left( u\right) \right) _{j_{k}}=\left(
q\left( u\right) \right) _{a}=h$ (since $a\in A$).
\par
If we had $k>\left\vert q\right\vert $, then the letter $\left( q\left(
u\right) \right) _{j_{k}}$ would be set in Phase I of the construction;
thus, we would have $j_{k}\notin q$, so that $\left( q\left( u\right)
\right) _{j_{k}}=t+1$ (by \eqref{pf.lem:determinant_form.interl-A.2}). But
this would contradict $\left( q\left( u\right) \right) _{j_{k}}=h<t<t+1$.
Thus, we cannot have $k>\left\vert q\right\vert $. Hence, $k\leq \left\vert
q\right\vert $. Hence, the letter $\left( q\left( u\right) \right) _{j_{k}}$
is set in Phase II of the construction. Therefore, $\left( q\left( u\right)
\right) _{j_{k}}=u_{i_{k}}$, so that $u_{i_{k}}=\left( q\left( u\right)
\right) _{j_{k}}=h$. In other words, $i_{k}\in B$. This is precisely what we
wanted to show.}. Thus, it is bijective. Hence, we have found a bijective
map $\phi :B\rightarrow A$ which furthermore satisfies $\phi \left( b\right)
\geq b$ for each $b\in B$. Therefore, Lemma \ref{lem:determinant_form.gale1}
shows that $A\succeq B$.

It remains to show that $B\gg C$. To do so, we need to check that $b>c$ for
every $b\in B$ and $c\in C$. So let us fix $b\in B$ and $c\in C$. We must
prove that $b>c$. Assume the contrary; thus, $c\geq b$.

From $b\in B$, we obtain $b\in \left[ n\right] $ and $u_{b}=h$. From $c\in C$%
, we obtain $c\in \left[ n\right] $ and $\left( q\left( u\right) \right)
_{c}=h+1$. Write $b$ in the form $b=i_{k}$ for some $k\in \left[ n\right] $.
Then, $i_{k}=b\in B$, so that $k\leq \left\vert q\right\vert $ (by %
\eqref{pf.lem:determinant_form.interl-A.b.1}). Also, if we had $c\notin q$,
then \eqref{pf.lem:determinant_form.interl-A.2} would yield $\left( q\left(
u\right) \right) _{c}=t+1$, which would contradict $\left( q\left( u\right)
\right) _{c}=\underbrace{h}_{<t}+1<t+1$. Thus, we have $c\in q$. Hence,
Claim 3 (applied to $j_{!}=c$) shows that $\left( q\left( u\right) \right)
_{c}\leq \left( q\left( u\right) \right) _{j_{k}}=u_{i_{k}}$ (since $c\geq
b=i_{k}$). In view of $i_{k}=b$, this rewrites as $\left( q\left( u\right)
\right) _{c}\leq u_{b}$. This contradicts $\left( q\left( u\right) \right)
_{c}=h+1>h=u_{b}$. This contradiction proves that our assumption was false.
Hence, $B\gg C$ is proven. Combining this with $A\succeq B$, we obtain $%
A\succeq B\gg C$. This completes the proof of Lemma~\ref%
{lem:determinant_form.interl-A}(b).
\end{proof}
...
TODO: Should we prove this? Not quite trivial.
 ...
 and then we can recursively label them from right to left by starting with the trivial labeling of the empty queue and applying each column-queue to find a labeling for the next column to its left.
