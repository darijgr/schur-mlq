...
% I still think this is useless. We never fixed $\qq$. -- Travis
...
\Darij{Don't like the previous sentence too much: it would better fit a lemma that assumes weak decreasingness of $u$ and claims weak decreasingness of $q(u)$, but our lemma does the exact opposite! I think "reflects" is a better word (as in category theory's "reflects limits"), but not many would understand it.}
...
% that $\qq(1^n)$ is weakly decreasing if and only if the MLQ $\qq$ is interlacing.
% [Darij:] This needs a lot more relativization to be correct.
...
\Darij{The following example needs to be revamped since the lemma was changed.}
\Travis{I tweaked the two sentences, but it really does not need to be altered. I guess we could remove the second bullet point if you feel it is a little out of place, but the first is fine and I am inclined to leave it there.}
\begin{example}
\mbox{}
\begin{itemize}
...
Let $\qq = \tup{q_1, q_2, \dotsc, q_{\ell}}$.
Note that $u := \qq(1^n)$ has type $\mm$ if and only if $\qq$ has type $\mm$ by Lemma~\ref{lemma:mlq-type}.
...
Lemma~\ref{lem:determinant_form.interl-A}(a) (applied to $u'$, $q_\ell$ and $\ell$ instead of $u$, $q$ and $t$) shows that $u'$ is weakly decreasing up to level $\ell-1$.
...
By our induction assumption, the word $u' = u'_1 u'_2 \dotsm u'_n  := \qq'(1^n)$ is weakly decreasing up to level $\ell - 1$ of type
\[
\mm' := (m_1, m_2, \dotsc, m_{\ell-1}, n - p_{\ell-1}(\mm), 0, \ldots)
\]
since $\abs{q_{\ell}} = p_{\ell}(\mm)$.
\Darij{I don't understand the argument starting from the next sentence.}
\Travis{The bijection given by Lemma~\ref{lem:determinant_form.gale1} exactly corresponds to Phase~II under a particular choice of permutation (i.e., doing letters from right to left) using $q_{\ell-1}^{(h)} = \{p \mid u'_p = h\}$. I got tired of stating obvious things once you spend a quick moment unraveling all the definitions and restating what was in the other direction. However, I have added this last bit to emphasize it more.

The $q_{\ell}^{(h)} \gg q_{\ell}^{(h+1)}$ is equivalent to the weakly decreasing condition once you have $q_{\ell}^{(h)} = \{p \mid u_p = h\}$. I removed the second reinvocation of Lemma~\ref{lem:determinant_form.gale1} as the definition of $\succeq$ is sufficient to see $q_{\ell}^{(h)} \gg q_{\ell}^{(h+1)}$. }
Furthermore, note that $q_{\ell-1}^{(h)} = \{p \mid u'_p = h\}$.
Hence, we have $q_{\ell}^{(h)} = \{p \mid u_p = h\}$ for all $h \in \ive{\ell}$ from the definition of Phase~II, that $q_{\ell}^{(h)} \succeq q_{\ell-1}^{(h)}$, and Lemma~\ref{lem:determinant_form.gale1}.
Furthermore, for all $h \in \ive{\ell-1}$, we have $q_{\ell}^{(h)} \gg q_{\ell}^{(h+1)}$ since $q_{\ell}^{(h)} \succeq q_{\ell-1}^{(h)} \gg q_{\ell}^{(h+1)}$.
...
\Travis{I added some examples, which should make it clear what is going on, even if it is not entirely rigorous.}
