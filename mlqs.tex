...
\item spontaneous symmetry breaking~\cite{AHR98,CEM01,EFGM95,GLEMSS95},
\item describing the formations of shocks~\cite{DJLS93,Ferrari92,FF94,Liggett76}, and
...
We also introduce the notation of a $\sigma$-twisted MLQ, where $\sigma$ is a permutation, although this is implicitly used in~\cite{AAMP}.
...
Furthermore, we note that our weighting scheme can be extended to multiline process used to determine the steady state distribution of the totally asymmetric zero range process (TARZP)~\cite{KMO16TARZP,KMO16TARZPII}, where multiple particles can occupy the same site.
...
Similarly, this extension of our results could potentially be used to describe the steady state distribution for the inhomogeneous TARZP defined in~\cite{KMO16II}.
...
\label{lemma:queue_merge}
  ...
  Let $\qq$ be an MLQ \travis{I don't think I used the fact it is an ordinary MLQ in the proof.} of type $\merge{i}\mm$.
  For the word
  $
...
\begin{proof}
  ...
  Let $t$ (resp.~$t^{\vee}$) be the split class of $q^*(u)$ (resp.~$q^*(v)$), and so the type of $(q^*, q_1, \dotsc, q_k, X)$ is
  \[
...
Does our proof have any implications for the work of Danilov and Koshevoy or van Leeuwen?
...
\begin{prop}
  ...
  We have $\swt{u}_{\tau}\sigma = \swt{u \sigma}_{\tau}$ for any $\sigma \in C_n$ and $\tau \in \SymGp{\ell-1}$, where $\SymGp{n}$ acts on $\mcW_n$ from the right by $(u_1 \dotsm u_n) \sigma = u_{\sigma(1)} \dotsm u_{\sigma(n)}$, that is to say permutations act on positions.
\end{prop}
...
Given the description of the combinatorial $R$-matrix using the Robinson--Schensted--Knuth (RSK) bijection~\cite{Shimozono02}, this $\SymGp{\ell}$ action can be considered as corresponding to the one given by van Leeuwen~\cite[Lemma~2.3]{vanLeeuwen-dc}.\footnote{This could also be considered as an interpretation of the Littlewood--Richardson rule, that $s_{c_1^{r_1}} s_{c_2^{r_2}}$, where $s_{\lambda}$ is a Schur function, is multiplicity free, and the Howe duality~\cite[Ch.~9,App.~B]{BS17}.}
