...
We describe how our definition of queues relates to the constructions in~\cite{AasLin17,AssSea18} in Appendix~\ref{app:queue-relations}.
...
\begin{remark}
Our notion of an (ordinary) MLQ is equivalent to what is called a ``discrete MLQ'' in~\cite[\S 2.2]{AasLin17}, where we can recover the labeling of level $k$ from the word $q_k( \cdots q_1(1 \dotsm 1) \cdots )$ (see item~(a) in Appendix~\ref{app:queue-relations}).
...
%=====================================================================
\section{Connections between queue constructions}
\label{app:queue-relations}

Let $u$ be a word and $q$ be a queue.
Our definition of $q(u)$ is connected to the constructions in~\cite{AasLin17,AssSea18}.

To clarify the connection, we introduce a mild variation of our visualization of queue by a row of circles with each $i \in q$ being a circle in position $i$ as in Example~\ref{ex:first_queue}.
(This is similar to~\eqref{eq:boxes-and-balls-1} except we don't represent the remaining elements $i \notin q$ by boxes.)
If $r$ is another queue and $I$ is a set, then a \defn{labeling} of $r$ by $I$ means a map $f \colon r \to I$.
To represent such a labeling $f$, we label each of the circles with its corresponding value (\textit{i.e.}, we label the circle in position $i$ by $f(i)$).
We refer to a value $f(i)$ as the \defn{label} of $i$ in $f$.
...
\item For $i = i_1, i_2, \ldots, i_{\abs{r}}$, do the following.
    Find the first site $j$ weakly to the right (cyclically) of $i$ such that $j \in q$ and $g(j)$ is not set.
    Then set $g(j) = f(i)$.

\item For all sites $j \in q$ for which $g(j)$ is not set yet, set $g(j) = k+1$.
...
Thus, our action of queues on words, with its two phases, is more general than their action on labelings.)

The action of queues on labelings is connected with constructions in~\cite{AasLin17,AssSea18} as follows:
...
an array with $\ell$ rows (corresponding to the queues) and $n$ columns (corresponding to the sites $1, 2, \dotsc, n$), where the $i$-th row has a circle in each site $j \in q_i$ (and in no other sites).
(Note that the roles of $N$ and $n$ in~\cite{AasLin17} correspond to the roles of $n$ and $\ell$ in our notations.)
The labeling procedure in~\cite{AasLin17} then labels the circles in each row, assigning to each of the $\ell$ queues $q_1, q_2, \ldots, q_{\ell}$ a corresponding labeling $f_i \colon q_i \to \ive{i}$.
In our language, this procedure is given recursively by
\[
f_i = q_i(f_{i-1}) \qquad \text{for all } i \in \ive{\ell} ,
\]
where $f_0$ is understood to be the (trivial) labeling of the empty queue $\emptyset$.
% In our language, such a labeling can be viewed as a sequence of $\ell$ words $w_1, w_2, \ldots, w_\ell$:
% Every time the $i$-th row has a circle labelled $j$, we put the letter $j$ in the corresponding position of $w_i$;
...
\item[(b)] The construction of the thread decomposition in~\cite[Def.~3.5]{AssSea18} is similar, except that the sites are now assumed to belong to $\NN$ rather than $\ZZ / n \ZZ$ (so there is no ``wrapping around'' any more), that the roles of rows and columns are interchanged, and that instead of setting $g(j) = k+1$ at the end of the construction, we fill the remaining circles with distinct new labels.
