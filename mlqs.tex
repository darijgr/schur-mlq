...
In the beginning, no letter of $v \in \mcW_n$ is set, and no site $i$ is frozen.
Pick a permutation $\tup{i_1, i_2, \ldots, i_n}$ of $\tup{1, 2, \ldots, n}$
such that $u_{i_1} \leq u_{i_2} \leq \cdots \leq u_{i_n}$.
(The order between sites $i$ with equal $u_i$ does not matter, as we will see in Remark~\ref{rmk:order-agnostic}.)
...
(i.e., we can perform the steps of the algorithm in any order as long as
the steps of Phase I are processed in the order of decreasing $i$ and
the steps of Phase II are processed in the order of increasing $i$).

Furthermore, if we switch two adjacent entries $i_k$ and $i_{k+1}$ of the
...
\begin{itemize}
 ...
 \item If $k = \abs{q}$, then we recall that the two phases can be
...
\end{itemize}
Thus, the word $v$ does not depend on the choice of the
permutation $\tup{i_1, i_2, \ldots, i_n}$, as long as it satisfies
$u_{i_1} \leq u_{i_2} \leq \cdots \leq u_{i_n}$.
This shows that $q(u)$ is well-defined.
\end{remark}
...
Consider the permutation $\tup{i_1, i_2, \ldots, i_n}$ used in the construction of $q(u)$.
...
Indeed, $\vee^{(k)} w$ can be obtained from $w$ by decrementing (by $1$) all but the $k$ smallest letters of $w$.

If $u \in \mcW_n$ is a word of type $\mm$,
if $k \in \set{ p_j(\mm) \mid j \geq 1 }$,
and if $q$ is a queue such that $\lvert q \rvert \neq k$, then
\begin{equation}
\vee^{(k)} q(u) = q(\vee^{(k)} u)
\label{pf.lemma:queue_merge.2}
\end{equation}
(and in particular, both $\vee^{(k)} q(u)$ and $\vee^{(k)} u$
are well-defined).
(Indeed, $\vee^{(k)} u$ is defined since $k \in \set{ p_j(\mm) \mid j \geq 1 }$;
furthermore, $\vee^{(k)} q(u)$ is defined since the type $\nn$ of $q(u)$
satisfies
$k \in \set{ p_j(\mm) \mid j \geq 1 } \subseteq \set{ p_j(\mm) \mid j \geq 1 } \cup \set{\abs{q}} = \set{ p_j(\nn) \mid j \geq 1 }$.
It thus remains to prove \eqref{pf.lemma:queue_merge.2}.
The permutation $\tup{i_1, i_2, \ldots, i_n}$ in the construction of $q(u)$
...
But $\vee^{(k)} q(u)$ is obtained from $q(u)$ in exactly the same way.
Hence, $\vee^{(k)} q(u) = q(\vee^{(k)} u)$, so that \eqref{pf.lemma:queue_merge.2} is proven.)

Now, set $k = p_i(\mm)$. Then,
\begin{equation}
...
\end{equation}
by repeated use of \eqref{pf.lemma:queue_merge.2}, since none of the queues in $\qq$ have size $k$.
...
We shall now prove a determinantal formula for $\swt{u}$ when $u$ is a special kind of word: a word whose letters range from $1$ to $\ell$, which has each letter from $1$ to $\ell-1$ appear at least one, and which is weakly decreasing if we disregard the letters $\ell$.
...
Let $u \in \mcW_n$ be a word whose letters are $< k$.
