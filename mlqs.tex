...
Moreover, the terminology does not feel like it both naturally arose nor can be used in a natural way.}
...
The standardization of a $k$-frow $u$ is the word $\widetilde{u} := \widetilde{u}_1 \cdots \widetilde{u}_{n}$ obtained as follows. Let $i_1 > i_2 > \cdots > i_r$, where $r = \lvert \{ i \in \ive{n} \mid u_i < k \} \rvert$, such that $u_{i_j} < k$ for all $1 \leq j \leq r$, and set
\[
\widetilde{u}_i = \begin{cases}
j & \text{if } u_i < k \text{ and } i = i_j \\
r + 1 & \text{otherwise,}
\end{cases}
\]
Thus, we have
...
\end{align*}
Note that the standardization is order preserving in the sense of $u_i \geq u_{i'}$ if and only if $\widetilde{u}_i > \widetilde{u}_{i'}$ for all $u_i, u_{i'} \neq k$.

\Travis{Standardize and apply the proof of~\cite[Prop.~3.7]{AasLin17} to give the bijection with the lattice paths via bully paths up to a final set of verticals.}
