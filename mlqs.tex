...
In Section~\ref{sec:JT_formula}, we show a Jacobi-Trudy like formula for special words $u$, which we use to prove some of the conjectures in~\cite{AasLin17}.
In Section~\ref{sec:tasep}, we describe the connection between MLQs and the TASEP.
In Sections~\ref{sec:thm_proof}, we give a proof of our main theorem (Theorem~\ref{thm:permutation}).
...
Note that if $q$ is an $r$-queue, then $q^*$ is the $(n-r)$-queue obtained by reflecting $[n] \setminus q$ across the middle of $[n]$.
...
(since any word obtained by actions of queues on $q'(1 \dots 1)$
will have its type $\nn$ satisfy
$k \in \set{ p_j(\nn) \mid j \geq 0 } $).
...
For $k \in \set{p_i(\mm) \mid i \geq 0}$, we set $[w]_k = 0$.
