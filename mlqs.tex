...
Combining this with $\abs{q'_2} = \abs{[u]_k} = k = \abs{[q(u)]_k}$, we obtain $q'_2 = [q(u)]_k$. Thus, Proposition~\ref{prop:SL.dual} is proven in the case when $k < \abs{q}$.
...
\begin{equation}
\label{eq:dual_claim}
\tup{ \ive{n} \setminus q, \ive{n} \setminus [u]_k }' = \tup{ \ive{n} \setminus [q(u)]_k, \ive{n} \setminus q^{\dagger} }.
\end{equation}
\Darij{This is perhaps a bit too much to ask of the reader. I suggest we instead apply the $k < \abs{q}$ case to the contragredient duals $u^*$, $q^*$ and $n-k$ instead of $u$, $q$ and $k$, and then use Lemma~\ref{le:dual} to transform this, after realizing that $(q_1^*, q_2^*)' = (q_1'*, q_2'*)$ for any configuration $(q_1, q_2)$ (this is worth pointing out beforehand). This also avoids the paragraph below.}
...
Applying this to~\eqref{eq:dual_claim}, we obtain $\tup{ [u]_k , q }' = \tup{q^{\dagger} , [q(u)]_k }$.
\end{proof}
...
In order to prove $\swt{u}_{\sigma} = \swt{u}_{\sigma s_i}$, we need to find a weight preserving bijection between the $\sigma$-twisted MLQs $\qq$ of type $\mm$ satisfying $u = \qq (1 \dotsm 1)$ and the $\sigma s_i$-twisted MLQs $\qq$ of type $\mm$ satisfying $u = \qq (1 \dotsm 1)$.
The bijection we will use is a restriction of $\fraks_i$. To show that it is well-defined, we need to prove that $\fraks_i$ sends the former MLQs to the latter; the only nontrivial part of this is that the image $\fraks_i\qq$ of an MLQ $\qq$ satisfying $u = \qq (1 \dotsm 1)$ will still satisfy $u = (\fraks_i\qq) (1 \dotsm 1$).
Thus, the proof is reduced to showing that for any configuration $C = \tup{q_1, q_2}$
and any word $w \in \mcW_n$ the dual configuration
$C' = (q'_1, q'_2)$ of $C$ satisfies $C'(w) = C(w)$.
