...
Next, let $\inter[i,j]$ denote a closed (cyclic) interval from $i$ to $j$.
...
Note for $r_1 < r_2$ and $j$ unbalanced, we have $j \notin q_1$ and $j \in q_2$.
...
We then construct $\word(\qq)$ by reading $M_{\qq}$ from top-to-bottom, left-to-right.
...
Thus, we write a sequence of parentheses for $\qq$ by for each $j = 1, 2, \dotsc, n$ (in that order), we write
\begin{itemize}
\item an opening parenthesis `(' if $j \in q_1$ and $j \notin q_2$,
\item an opening parenthesis `)' if $j \notin q_1$ and $j \in q_2$,
\item an canceling pair of parenthesis `()' if $j \in q_1$ and $j \in q_2$,
...
By~\cite[Prop.~5.6.1]{Loth}, we can cyclically shift $w$ such that either there  are only opening parentheses `(' or closing parentheses `)`, but not both and this does not change the result.
In other words, we have $s_1 \zeta^a w = \zeta^a s_1 w$, where $\zeta$ is the cyclic shift $w_j \mapsto w_{j+1}$, where indices are taken modulo $n$.
We slightly abuse of notation and denote the cyclic shift of an MLQ by $\zeta^a \qq = (\zeta^a q_1, \zeta^a q_2)$, where $\zeta^a q_k = \set{ j + a \mod{n} \mid j \in q_k}$ for $k = 1,2$.
However, we are justified in doing this since $\zeta^k \qq = \zeta^k w$ if and only if $\qq = w$ for any $k \in \ZZ$.
By the definition of balanced intervals being cyclic, we have $\zeta^a \fraks_1 \qq = \fraks_1 \zeta^a \qq$.
However, it is clear that $\zeta^a \qq = \zeta^a w$ if and only if $\fraks_1 \zeta^a \qq = \sigma_1 \zeta^a w$.
...
$\fraks_i \circ \fraks_{i+1} \circ \fraks_i = \fraks_{i+1} \circ \fraks_i \circ \fraks_{i+1}$.
\end{proof}
...
The proof thus reduces down to showing for a configuration $C$ and word $u$, we have $C(u) = C'(u)$.
