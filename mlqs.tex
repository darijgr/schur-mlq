...
\Darij{Travis doesn't like the bracketty subproof formatting; I want it to have some "enclosing" element. Travis, is this better? I'd prefer elephant bookends, but I don't see them in LaTeX.}
...
Thus, from Lemma~\ref{lemma:queue_merge}, there exists some $\sigma$-twisted MLQ $\widetilde{\qq}'$ such that $\widetilde{\qq}'(1 \dotsm 1) = \std{u}$, and from Theorem~\ref{thm:permutation}, there exists a corresponding MLQ $\widetilde{\qq}$ such that $\widetilde{\qq}(1 \dotsm 1) = \std{u}$.\footnote{This is essentially the same as the proof of Theorem~\ref{thm:merge}. Moreover, this is done bijectively; see also Remark~\ref{rmk:bijective_proof}.}
...
\begin{comment}
We begin with a proof of~\eqref{eq.LGV.single-paths}, which is a simple and classical fact that appears implicitly in various texts on enumerative combinatorics (e.g., \cite[(2.36)]{Stanley-EC1}).
...
For example, both $\tup{5,3,4,2,2}$ and $\tup{6,2,3,4,1}$ are pseudo-partitions of length $5$.
...
These tableau have surfaces $(5,3,5,7)$ and $(4,3,5,5)$, respectively, and weights $x_1 x_2^2 x_3^2 x_4 x_5^2 x_7$ and $x_1^2 x_2 x_3 x_4^2 x_5^3$, respectively.
...
Then, for each $i \in \ive{k}$, the path $p_i$ is a path from $A_i = (k-i, 1)$ to $B_i = (\lambda_i+k-i-1,s_i)$, and therefore contains exactly $(\lambda_i+k-i-1) - (k-i) = \lambda_i - 1$ east-steps.
\end{vershort}
\begin{verlong}
...
\Darij{This is classical and should be replaced in vershort by something like "Follow the classical JT proof."}

[\textit{Proof of~\eqref{pf.thm.tableau.jt.col-strict-no-gap}:}
\begin{vershort}
We assume the contrary and pick the $i, j$ with the \emph{smallest} $j$ for which $p_{i,j} \geq p_{i+1,j}$.
...
The proof of Lemma~\ref{lem:determinant_form.interl-act} is tedious and technical; we thus have banished it into the Appendix (Section~\ref{app:JT-proofs}).
...
This pseudo-partition $\lambda^{\mm}$ has length $r$, since $r = p_{\ell-1}(\mm)$.
...
\begin{proof}
  ...
  $\qq (1 \dotsm 1) = \bigl( (\sigma \qq)(1 \dotsm 1) \bigr) \sigma$.
...
\begin{equation}
f_k(j) = \bigl( q_k( \cdots q_1(1 \dotsm 1) \cdots ) \bigr)_j.
...
Thus, by induction,
\[
\omega_i \tup{ f_i } = q_i( \cdots q_1(1 \dotsm 1) \cdots )
 \qquad \text{for all } i \in \set{0, 1, \dotsc, \ell}
\]
(since $\omega_0 \tup{ f_0 } = 1 \dotsm 1$).
Hence,
\[
f_i(j) = \bigl( q_i( \cdots q_1(1 \dotsm 1) \cdots ) \bigr)_j
 \qquad \text{for all } i \in \set{0, 1, \dotsc, \ell}
        \text{ and } j \in q_i .
\]
...
%=====================================================================
%\section{Technical proofs for Section~\ref{sec:JT_formula}}
\section{Proof of Lemma~\ref{lem:determinant_form.interl-act}}
\label{app:JT-proofs}
...
The goal of this section is to prove Lemma~\ref{lem:determinant_form.interl-act}.
...
Let $j_{!} \in q$ be a site that lies between $i_k$ (inclusive) and $j_k$ (inclusive) on the circle $\ZZ/n\ZZ$.
...
\Darij{Which intervals? The ``lie between'' parts? OK, I can do that.}
...
\Darij{Where exactly is this not PC? And should we care? My suggestion is to move this proof into verlong anyway, while in vershort it gets replaced by some good pictures showing the ``balls'' ßin the $r^{(j)}$ and $q^{(j)}$ and how everything moves to the right place. But I can't draw that picture, at least not in LaTeX.}
...
\begin{vershort}
(since $\abs{q_i} = m_1 + m_2 + \cdots + m_i$).
\end{vershort}
\begin{verlong}
(since its type is $\tup{m_1, m_2, \dotsc, m_i, n-p_i(\mm), 0, 0, \ldots}$, and since $\abs{q_i} = m_1 + m_2 + \cdots + m_i$).
\end{verlong}
...
We must prove that $u = \qq(1 \dotsm 1)$.
...
Hence, $u = u^{\left[\ell-1\right]} = \qq(1 \dotsm 1)$.
