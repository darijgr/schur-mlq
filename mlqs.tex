...
We define \defn{$\SP\tup{q_1, q_2}$} to be the following data: the sequence of parentheses obtained from $q_1$ and $q_2$; the matching obtained by the algorithm; and the correspondence between sites and \textbf{closing} parenthesis (i.e., which site contributes which closing parentheses).
...
Note that $C$ and $C'$ have the same balanced intervals, since the parentheses that were matched for $C$ remain matched in $C'$ and the remaining parentheses are all of one kind.
...
Then, $\SP\tup{q_1 \setminus \set{i}, q_2 \setminus \set{j}}$ is obtained from $\SP\tup{q_1, q_2}$ by removing a pair of matched parentheses.
...
The parenthesis sequence $\SP\tup{q_1 \setminus \set{i}, q_2 \setminus \set{j}}$ looks exactly the same, except that $\alpha$ and $\gamma$ are missing.
...
\Darij{I don't understand the above.}
