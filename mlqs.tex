...
\begin{lemma}
\label{lem.LGV.hex}
Let $A$, $B$, $A'$ and $B'$ be four vertices such that
\[
\operatorname*{x}(A') \leq\operatorname*{x}(A), \qquad \operatorname*{y}(A') \geq \operatorname*{y}(A),\qquad \operatorname*{x}(B') \leq \operatorname*{x}(B), \qquad \operatorname*{y}(B') \geq \operatorname*{y}(  B).
\]
...
Let $A, B \subseteq \ive{n}$ such that there exists $\phi \colon B \to A$ a bijection satisfying $\phi(b) \geq b$ for each $b \in B$.
Then, $A \succeq B$.
\end{lemma}
...
\begin{align*}
\sum_{\substack{T\text{ is a semistandard tableau}\\\text{of shape } \lambda \text{ and surface } s}}\wt(T)
  & = \underbrace{\sum_{\substack{\qq=\tup{q_1, q_2, \dotsc, q_{\ell-1}} \text{ is an interlacing}\\\text{MLQ of type } \mm \text{ such that} \\\text{the surface of }P(\qq) \text{ is } s}}}_{\substack{=\sum_{\substack{\qq=\tup{q_1,q_2,\ldots,q_{\ell-1}} \text{ is an interlacing}\\\text{MLQ of type }\mm \text{ such that} \\\text{the list of all elements of } q_{\ell-1} \text{ in} \\\text{increasing order is } s}}\\\text{(by property (b))}}}\underbrace{\wt\bigl(  P(\qq) \bigr)  }_{\substack{= \wt(\qq) \\\text{(by property (a))}}}\\
...
&  = \sum_{\substack{\qq = \tup{q_1, q_2, \dotsc, q_{\ell-1}} \text{ is an interlacing} \\\text{MLQ of type } \mm \text{ with } q_{\ell-1}=B}} \wt(\qq)\\
