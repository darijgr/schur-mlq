...
This can be easily seen by considering the parentheses as an infinite shifted-$n$-periodic Motzkin path, where a ``nothing'' corresponds to a flat step.
...
\Darij{The Motzkin stuff isn't really a justification to me, much that I like the picture. I prefer to use Newman's diamond lemma to see that the final result does not depend on the order of matching. Also, the Motzkin path is not $n$-periodic in general, because a site $j \in q_1 \cap q_2$ contributes two letters.}
...
\Darij{We aren't studying balanced intervals anymore.
Instead, argue symmetry for the parenthesis-matching algorithm on the reduced parenthesis sequence.
Note that we need ``reduced''.}
