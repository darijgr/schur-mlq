...
\end{itemize}
Let $k = q(u)_j$ if $q(u)_j$ is set during Phase~I and $k = q(u)_j - 1$ otherwise.
Therefore, if $q(u)_j$ is set during Phase~I, we have $q\bigl( f_k(u) \bigr)_i \geq 2$ and $q\bigl( f_k(u) \bigr)_j = 1$.
...
Let $u \in \mcW_n$ be a word of type $\mm$, and fix some $k = p_j(\mm)$ for some $j \geq 1$.
...
\begin{itemize}
 \item all letters of $w$ are $\leq k$;
...
If $w$ is a word and $k$ is a positive integer, then \defn{$w^{-1}(k)$} is defined to be the set $\set{i \in \ive{n} \mid w_i = k}$.

If $q$ is a queue and $p \in \ive{\abs{q}}$, then \defn{$\max_p q$} means the $p$-th largest element of $q$.
