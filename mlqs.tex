...
For each arc $a$ of the lattice, we define the \defn{weight} of $a$ as
\[
\wt(a) = \begin{cases}
...
We do this to give a tableaux interpretation of our LGV Lemma determinant.
A psuedo-partition is similar to the concept of partitions except it allows entries to increase by $1$.
The corresponding semistandard tableaux is just a calque of the usual notion of semistandard tableaux extended to pseudo-partitions.
We will then prove a Jacobi--Trudi-like formula for a generating function of these semistandard tableaux of a fixed pseudo-partition and rightmost entries of each row.
Later we will translate these tableaux into MLQs that yield a specific word when applied to $1\dotsm 1$.

Let us formalize these definitions.
Fix some $k \in \NN$.
A \defn{pseudo-partition} shall mean a $k$-tuple $\lambda = \tup{\lambda_1, \lambda_2, \dotsc, \lambda_k}$ of positive integers such that
\[
\text{each } i \in \ive{k-1} \text{ satisfies } \lambda_i + 1 \geq \lambda_{i+1}.
\]
We say $\lambda$ has \defn{length} $k$.
For example, $\tup{5,3,4,2,2}$ and $\tup{6,2,3,4,1}$ are both pseudo-partitions of length $5$.
...
If $T$ is a tableau of shape $\lambda$, then the \defn{surface} of $T$ is defined to be the $k$-tuple $(s_1, s_2, \dotsc, s_k)$, where $s_i$ is the rightmost entry of the $i$-th row of $T$ (that is, $s_i = T(i,\lambda_i)$).
For example, the two tableaux in~\eqref{eq.tableau.exas}
...
\ydiagram{3,2,3,1}\,.
\]
Assuming $n \geq 7$, the following are two tableaux of shape $\tup{3,2,3,1}$:
\[
...
\]
\travis{I changed the first example to something that is more subtly not semistandard.}
The right tableau is semistandard, while the left one is not (the two $5$'s in the rightmost column).
These tableau have surfaces $(5,3,4,7)$ and $(4,3,5,5)$, respectively, and weights $x_1 x_2^2 x_3^2 x_4 x_5^2 x_7$ and $x_1^2 x_2 x_3 x_4^2 x_5^3$, respectively.
\travis{I consolidated these examples because someone used to tableaux only needs to see this example to understand (or extrapolate what should be) the definitions.}
\end{example}
...
\]
Since $\lambda$ is a pseudo-partition and $s$ is weakly increasing, we can apply Proposition~\ref{prop.LGV.concrete} to obtain
\begin{equation}
\label{pf.thm.tableau.jt.1}
\begin{split}
\sum_{\pp \in N(\uu,\vv)} \wt(\pp) & = \det\left(  \sum_{p \in  N(A_i,B_j)}\wt(p) \right)_{i, j \in \ive{k}} \\
& \overset{\eqref{eq.LGV.single-paths}}{=} \det\left( h_{\lambda_j-j+i-1}(x_1, x_2, \dotsc, x_{s_j}) \right)_{i, j \in \ive{k}}.
\end{split}
\end{equation}
...
Indeed, let $\tup{p_1, p_2, \dotsc, p_k} \in N(\uu,\vv)$ be an arbitrary NILP.
Since $p_i$ is a path from $A_i = (k-i, 1)$ to $B_i = (\lambda_i+k-i-1,s_i)$, it must contain exactly $(\lambda_i+k-i-1) - (k-i) = \lambda_i - 1$ east-steps.
...
\travis{This is the standard JT proof, right? Can we do it all at once for $p_{i,j} < p_{i',j}$ for any $i < i'$? Basically, we can restrict to two rows and get semistandardness (with no gaps). I really feel like we should be able to use the standard JT proof and basically remove the majority of this proof. At the very least, I believe this is ``routine'' and could be left to the reader with a hint along the lines of, ``Follow the usual JT proof.''}
%Let $i \in \ive{k-1}$.
...
% You remind the reader of these things below when you state what the contradiction is  -- Travis
...
We note that during the construction of the bijection $\Phi$, the fixed surface $s$ condition on the semistandard tableaux translated into requiring each path $p_i$ to having a final arc $B_i \to (\lambda_i + k - i, s_i)$.
Otherwise the rest of the proof is similar to the usual Jacobi-Turdi bijection relating semistandard tableaux and NILPs.
...
\end{itemize}
}\travis{This is a really long footnote for something inside of a parenthetical. We should try to either shorten it or move it into the main body.});
...
Let $s = \tup{s_1, s_2, \dotsc, s_k}$. \travis{These are the same $s_i$ as in the statement, correct?}
...
\end{align}
\end{subequations}
\travis{This follows trivially from the definition of Phase~II: the $\abs{q}$ smallest elements of $q(u)$ are at $j \in q$, which by assumption are at most $t$. Everything else is strictly larger than $t$ (which is used during Phase~I). It does not need a whole paragraph to explain!}
...
Let $j_{!} \in q$ that lies between $i_k$ (inclusive) and $j_k$ (inclusive) on the circle $\ZZ/n\ZZ$.
(If $i_k>j_k$, then this means that $j_{!} \geq i_k$ or $j_{!}\leq j_k$; otherwise, it means that $i_k\leq j_{!}\leq j_k$.) Then, $\bigl( q(u) \bigr)_{j_{!}} \leq \bigl( q(u) \bigr)_{j_k} = u_{i_k}$.
\end{claim}
...
\]
since $q(u)$ is weakly decreasing up to level $t$, the dichotomy~\eqref{pf.lem:determinant_form.interl}, and that $q(u)$ has at least one letter equal to $t$.

\Travis{You need notation for these intervals. It is too cumbersome explaining it in words.}
...
This map $\phi$ is injective since the $j_1, j_2, \dotsc, j_n$ are distinct (recall that $\tup{ j_1, j_2, \dotsc, j_n}$ is a permutation of $\tup{1, 2, \dotsc, n}$ because each letter of $q(u) $ is set exactly once during the construction of $q(u)$).
Furthermore, the map $\phi$ is surjective.\footnote{\textit{Proof.}
  Let $a\in A$.
...
\travis{No paragraph long footnotes. Either this is obvious, detail left for the reader, or it needs to actually be part of the proof.}
...
Moreover, we know how often each letter appears: namely, each $k \in \ive{i}$ appears exactly $m_k$ times (since the word $u^{\left[  i\right]  }$ has type $\tup{m_1, m_2, \dotsc, m_i, n-p_i(\mm), 0, 0, \ldots}$).

Recall that the word $u^{\left[  i\right]  }$ has type $\tup{m_1, m_2, \dotsc, m_i, n-p_i(\mm), 0, 0, \ldots}$.
Hence, the letters $1,2,\dotsc,i$ that appear in this word are exactly the $m_1 + m_2 + \cdots + m_i$ \travis{Why not use $p_i(\mm)$?} smallest letters of this word.
In other words, they are the $\abs{q_i}$ smallest letters of this word (since $m_1 + m_2 + \cdots + m_i = \abs{q_i}$\travis{Same here. I think it gives a better association.}).
...
This word $q_i( u^{\left[  i-1\right]} ) = u^{\left[  i\right]}$ has type $\tup{m_1, m_2, \dotsc, m_i, n-p_i(\mm), 0, 0, \ldots}$.
Thus, $u^{\left[  i\right]}$ has at least one letter equal to $i$ since $m_i > 0$ and is weakly decreasing up to level $i$ by Claim~\ref{claim:weakly_decreasing_level_i}.
Also, $u^{\left[  i\right]}$ has exactly $\abs{q_i}$ letters that are at most $i$ since $\abs{q_i} = m_1 + m_2 + \cdots + m_i$.
...
For the induction step, we fix $h \in \ive{\ell-1}$, and we assume
(as induction hypothesis)
that Claim~\ref{claim:describing_u^[i]} holds for $i = h - 1$.
...
\end{cases}
\]
In view of $\set{b_1, b_2, \ldots, b_r} = B = q_{\ell-1}$,
this shows that
the letters $u_t$ for $t \notin q_{\ell-1}$ are all $\ell$,
whereas
the letters $u_t$ for $t \in q_{\ell-1}$ are in $\ive{\ell-1}$.
Hence, the letters $u_t$ for $t \in q_{\ell-1}$ are precisely
those letters of $u$ that are at most $\ell-1$.
Thus, these letters appear in weakly decreasing order
(since $u$ is weakly decreasing up to level $\ell-1$), and we
also know how often each of the letters $1, 2, \ldots, \ell-1$
appears among them (namely, each $k \in \ive{\ell-1}$ appears
$m_k$ times, since $u$ has type $\mm$).
Hence,
\begin{itemize}
...
\end{cases}
\]
Comparing this with~\eqref{pf.lem:determinant_form.interl-act.fwd.5}, we conclude that
$\left( u^{\left[\ell-1\right]} \right)_t = u_t$ for all $t \in \ive{n}$.
...
Let $\ell$ be a positive integer.
Let $B = \set{ b_1 < b_2 < \cdots < b_r} \subseteq \ive{n}$.
Let $\tup{v_1 \geq v_2 \geq \cdots \geq v_r}$ be a weakly decreasing $r$-tuple of integers such that $\set{v_1, v_2, \ldots, v_r} = \ive{\ell-1}$.\travis{We probably want to introduce the notation of a weakly decreasing word, then we can just say ``Let $v = v_1 v_2 \dotsm v_r$ be a weakly decreasing packed word.''}
Define a word $u \in \mcW_n$ by $u_i=v_j$ if $i = b_j$ for some $j$, otherwise $u_i = \ell$.
For each $j \in \ive{r}$, set $\gamma_j = \ell - v_j$.\footnote{The value $\gamma_j$ can also be described as the number of distinct letters in the (non-cyclic) word $v_1 \dotsm v_j$.}
Then
\[
...
\end{equation}
and so if we set $m_{\ell} = n - r$, we define a type $\mm = \tup{m_1, m_2, \dotsc, m_{\ell}, 0, 0, \ldots}$.
Moreover, the word $u$ has type $\mm$ by the construction of $u$.
...
Consider the pseudo-partition $\lambda^{\mm} = \tup{\lambda_1,\lambda_2, \dotsc, \lambda_r}$.
...
%& \\ & = \left( \prod_{b \in B} x_b \right)  \det\left(  h_{\gamma_j+i-j-1}(x_1,\dotsc,x_{b_j}) \right)_{i, j \in \ive{r}} ,
\end{align*}
where the last equality is from Corollary~\ref{cor:determinant_form.bij1c} (applied to $B$ and $b_i$ instead of $S$ and $s_i$).
% This is pointless. The reader is certainly smart enough to do the second (trivial) equality and at the end of the previous paragraph we state the first equality. They will still have to look up the first equality anyways.
