...
% We have not defined words with $r$ letters.
% And our words are cyclic, while $v$ is not.
% One fewer line isn't worth the extra confusion.
Define a word $u(v) \in \mcW_n$
with $\ell$ classes
by
\begin{equation}
...
$\sigma_S$ (recall that this stands for $u(v_{\sigma_S})$)
for $S \subseteq T$.
Hence, the $\sum \swt{u'}$ in the above equality rewrites as $\psi(T)$.
This proves the first equality of the corollary.
To prove the second equality, we set
$\ell = r - \abs{T} + 1$, and we let
$v = \tup{v_1 \geq v_2 \geq \cdots \geq v_r}$ be the
$r$-tuple of integers given by $v_j = \ell - j + \abs{\set{t \in T \mid t > r-j}}$.
Note that this $r$-tuple $v$ can be uniquely characterized by the properties
that $v_1 = r - \abs{T} = \ell-1$ and $v_r = 1$ and
$v_{i+1} = v_i - \begin{cases} 0, & \text{if } r-i \in T; \\ 1, & \text{if } r-i \notin T
                 \end{cases}$ for each $i \in \ive{r-1}$;
thus, it is weakly decreasing and satisfies
$\set{v_1, v_2, \ldots, v_r} = \ive{\ell-1}$.
This characterization yields $u(v) = \bigvee_T w_0$.
...
\\ & \hspace{20pt} \cdot \det \begin{pmatrix}
%gamma_j = 1,1,2,3,3
1 & 0 & 0 & 0 & 0 \\
h_1[1] & 1 & 1 & 1 & 0 \\
...
\begin{subproof}
Suppose there exists a $\kappa$ such that $j_{\kappa} < i_{\kappa}$ (\textit{i.e.}, the path of $u_{i_{\kappa}}$ ``wraps around the cycle'').
Without loss of generality, assume $\kappa$ is minimal.
Note that $u_{i_{\kappa}} < t$ since $\kappa \leq \abs{q} - m_t$.
Moreover, there does not exist an $h < \kappa$ such that $j_h = \min q$ as otherwise this would violate the weakly decreasing up to level $t$ condition as $u_{i_h} < t$ as well.
...
%Since $u$ is weakly decreasing, by Lemma~\ref{lemma:order_indep} we can assume that $i_{\abs{q}-m_t} < \cdots < i_2 < i_1$.
...
\item We require that every letter in $\ive{\ell-1}$ appears in $u$.
  To see why, consider $u = 2{\color{gray}4}1{\color{gray}4}$, which is of type $\mm = \tup{1,1,0,2,0,\ldots}$, and note $i$ is weakly decreasing up to level $3$.
...
(since $v_1 \geq v_2 \geq \cdots \geq v_r$ and
$\set{v_1, v_2, \ldots, v_r} = \ive{\ell-1}$).
Therefore, we can apply Lemma~\ref{lem:determinant_form.interl-act} to $u = u(v)$.
...
&  = \sum_{\substack{\qq = \tup{q_1, q_2, \dotsc, q_{\ell-1}} \text{ is an interlacing} \\\text{MLQ of type } \mm \text{ with } q_{\ell-1}=B}} \wt(\qq) \qquad \text{(by Lemma~\ref{lem:determinant_form.interl-act} for $u = u(v)$)} \\
...
\]
from the MLQ $\qq_D = (q_1, q_2, \dotsc, q_{\ell})$ to obtain the correct labelings, where $\ell + \ell'$ is the largest label appearing in the Kohnert filling.
...
\begin{example}
We consider the Kohnert filling from Example~\ref{ex:thread_decomp}.
The following MLQ, given as a graveyard diagram, is an MLQ that gives the corresponding Kohnert filling:
\[
...
Note that all circles that appear to the left of the dashed line correspond to the Kohnert filling and that the labels match.
\end{example}
