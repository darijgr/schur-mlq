...
\end{remark}
\Darij{Why? I know why, but it takes me another definition -- the ``p-sequence'' of a type $\mm$, which is defined to be $\tup{p_0(\mm), p_1(\mm), p_2(\mm), \ldots}$ (and which uniquely determines $\mm$). Using this p-sequence, we observe that the action of a queue on a word merely inserts the size of the queue into the p-sequence at the appropriate position (``insort''), so by applying $\qq$ to $11\cdots 1$ we will end up with the p-sequence of $\mm$ and therefore with the same type.}
...
\label{lemma:queue_merge}
  ...
  Let $\qq$ be a $\sigma$-twisted MLQ of type $\merge{i}\mm$ for some $\sigma$.
  For the word
  $
...
\end{equation}
\end{comment}
The type of $u$ is $\mm$ (by Remark~\ref{rmk:mlq-type}, applied to the twisted MLQ formed by both $q'$ and $\qq$).
\Darij{Needs explanatory work: What is this twisted MLQ? Why is its type $\mm$ ?}
...
\begin{proof}
  ...
  Let $\mm$ have $\ell$ classes; then, $\merge{i}\mm$ has $\ell-1$
  classes.
  First note that
  \[
...
\Darij{Does $\bigl( [w]_k \bigr)_{k=0}^n$ even make sense
if we don't assume $w$ to be near standard?}
