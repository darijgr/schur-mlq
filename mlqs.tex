...
\end{equation}
We call this the \defn{graveyard diagram} of $q$.
...
Fix a set $B = \set{b_1 < b_2 < \cdots < b_r}$ of sites.
For a permutation $v \in \SymGp{r}$, let $u(v)$ be the corresponding word as defined in Theorem~\ref{thm:determinant_form} from $v$ and $B$.
\darij{What does this mean?}
\travis{The word $u(v)$ is the word $u$ given in the statement of the theorem. I have tried to explain this more. Does this answer your question?}
...
\darij{Why exactly?}
\travis{Yes, it should be Theorem~\ref{thm:merge}, and you need to apply it for each $i \in T$.}
\begin{align*}
  \psi(T) & = \left(\prod_{i\in T} e_{p_i(\mm)}(x_1, \dotsc, x_n) \right) \swt{ \bigvee_T w_0}
  \\ & = \left( \prod_{i\in B} x_i \right) \left(\prod_{i\in T} e_{p_i(\mm)}(x_1, \dotsc, x_n) \right) \det\bigl(h_{i-j+1+\gamma_j}(x_1, \dotsc, x_{b_j})\bigr)_{i, j \in \ive{r}},
\end{align*}
...
q_3^{(1)} = \set{8,9,13,14}.
\end{gather*}
Similar to the graveyard diagram of~\eqref{eq:boxes-and-balls-1}, we depict the graveyard diagram of $\qq$ with row $i$ having the element $p \in q_i^{(j)}$ represented a circle at position $p$ labeled by $\bigl( q_i( \cdots q_1(1^n) \cdots ) \bigr)_p$ for all $j \in \ive{i-1}$ and elements of $\ive{15} \setminus q_i$ represented by squares (\textit{i.e.}, we suppress filling the squares with an $i+1$):
\[
...
From the definition of $q(u)$ and there are precisely $\abs{q}$, all letters set during Phase~II of the algorithm are at most $t$, while all of letters set during Phase~I are strictly greater than $t$ (see also Remark~\ref{rmk:t-splitting}).
