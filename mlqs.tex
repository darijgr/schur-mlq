...
\travis{Reference~\cite{Angel06}. who was the first to do 2-line queues.}
...
\label{lemma:queue_merge}
  Let $\mm$ be a packed type, and let $X$ be a $p_i(\mm)$-queue.
  Let $\qq$ be a MLQ of packed type $\mm' = \merge{i}\mm$.
  For the word
  $
...
\begin{proof}
  ...
  Next, assume that for $\qq = (q_1, \dotsc, q_k)$, with $q_k$ being an $r$-queue, of type $\mm$ that $v = \qq(1 \dotsm 1) = \merge{i} u$.
  Let $q^*$ be an $r^*$-queue with $r^* > r$, since we only consider packed words, and the type of $(q^*, q_1, \dotsc, q_k, X)$ is
  \[
  \mm^* := (m_1, \dots, m_k, r^*-p_k, n - r^* - p_k, 0, 0, \ldots).
  \]
  \travis{We need to incorporate the $\sigma$ because this is a $\sigma$-twisted MLQ to obtain the type.}
  Let $t$ (resp.~$t^{\vee}$) be the split class of $q^*(u)$ (resp.~$q^*(v)$).
  Note that $t - 1 \leq t^{\vee} \leq t$.
  If $i \geq t$, then $t^{\vee} = t$ and $q^*\bigl(v) = \merge{i+1} q^*(u)$ from Lemma~\ref{le:mono}, and the claim follows since $X$ is a $p_{i+1}(\mm^*)$-queue.
  If $i < t - 1$, then $t^{\vee} = t - 1$ and $q^*\bigl(v) = \merge{i+1} q^*(u)$ from Lemma~\ref{le:mono}, and the claim follows since $p_i(\mm) = p_i(\mm^*)$.
  Finally, assume $i = t - 1$, in which case the $i$'s are added during Phase~I or Phase~II of $q^*(u)$ but are instead handled by Phase~III of $q^*(v)$.
  However, depending on high or low, they give the same answer in Phase~III because they are placed last and agree with that once things shift propertly.
  \travis{To be finished. The $i = t$ case might need to be handled in the same way as the $i = t-1$.}
\end{proof}
...
\label{thm:merge}
  ...
  Let
  \[
  \mm' = (m_1, \dotsc, m_{i-1}, m_i + m_{i+1}, m_{i+2}, \ldots).
  \]
  For any packed \travis{This is a misuse of ``packed'' as we should have already said $\mm'$ was packed previously, which comes from $\mm$ being packed.} word $v$ of type $\mm'$, we have
\[
...
\begin{example}
We give an example to illustrate the proof of Theorem~\ref{thm:merge}.
To compute $\swt{135452}$, we need to examine MLQs of type $(1,1,1,1, 2, 0, 0, \ldots)$.
We take a particular MLQ $\qq$ and add the $5$-queue $\set{1,2,3,5,6}$ as follows:
\[
\qq =
\begin{tikzpicture}[baseline=66,scale=0.8]
...
\travis{Here we give the action on positions as a right action, whereas below it is a left. We should at least be internally consistent.}
...
\begin{proof}
  ...
  \[
  C(u) = q_2(q_1(u)) = q_2(q_1(u_{i\leftrightarrow j})) = C(u_{i\leftrightarrow j}).
  \]
\end{proof}
...
\begin{prop}
  ...
  We have $\swt{u}_{\tau} = \swt{\sigma u}_{\tau}$ for any $\sigma \in C_n$ and $\tau \in \SymGp{\ell-1}$, where $\SymGp{n}$ acts by $\sigma u_1 \dotsm u_n = u_{\sigma(1)} \dotsm u_{\sigma(n)}$, that is to say permutations act on positions.
  \travis{The side of this action is inconsistent with the above.}
\end{prop}

\begin{proof}
  For a queue $q$, define $\sigma q := \{ \sigma(i) \mid i \in q\}$.
  It is clear from the definition of a queue that $(\sigma q)(u) = q(u\sigma)\sigma$, and the claim follows.
  \travis{Check left vs right action on position.}
\end{proof}
...
We also note that this $\SymGp{\ell}$ action on MLQs has already been considered by Danilov and Koshevoy~\cite{DanilovKoshevoy} in a different context (see also~\cite[Ch.~4]{Gorodentsev2}).
...
\end{tikzpicture}
\]
where the equality comes from applying the Yang--Baxter equation and that $R^2 = \id$.
