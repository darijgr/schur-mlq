...
Thus we can assume that both of these statement hold,
...
Furthermore, for all $h \in \ive{\ell-1}$, we have $q_{\ell}^{(h)} \gg q_{\ell}^{(h+1)}$ since $q_{\ell}^{(h)} \succeq q_{\ell-1}^{(h)} \gg q_{\ell}^{(h+1)}$ and from Lemma~\ref{lem:determinant_form.gale1}.
...
Therefore, for an MLQ $\qq = \tup{q_1, \ldots, q_{\ell-1}}$ of type $\mm$, we have the following chain of equivalences:
\begin{align*}
     & \tup{u(v) = \qq(1^n)} \\
\iff & \left( \text{the word $\qq(1^n)$ has type $\mm$ and is weakly decreasing up to level $\ell-1$} \right. \\
     & \qquad \left. \text{ and its letters $\leq \ell-1$ lie in the positions $j \in B$} \right) \\
\iff & \left( \text{the word $\qq(1^n)$ has type $\mm$ and is weakly decreasing up to level $\ell-1$,} \right. \\
     & \qquad \left. \text{ and we have $q_{\ell-1} = B$} \right) \\
\iff & \left( \text{the MLQ $\qq$ has type $\mm$ and is interlacing, and we have $q_{\ell-1} = B$} \right)
\end{align*}
(by Lemma~\ref{lem:determinant_form.interl-act}).
...
&  = \sum_{\substack{\qq = \tup{q_1, q_2, \dotsc, q_{\ell-1}} \text{ is an interlacing} \\\text{MLQ of type } \mm \text{ with } q_{\ell-1}=B}} \wt(\qq) \qquad \text{(by the above chain of equivalences)} \\
