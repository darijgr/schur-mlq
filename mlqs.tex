...
Let $q$ be a queue.
Assume that $\abs{q}$ is $\geq$ to the number of letters of $u$ that are $< \ell-1$.
...
(Is this true in this form? XXX)
\end{lemma}

\begin{proof}
Consider the $t$ in Remark~\ref{rmk:t-splitting}.
Then, $t = \ell-1$ (or, more precisely, we can pick $t = \ell-1$).
Hence, all letters of $q(u)$ apart from $\ell$ are set in Phase II of the algorithm.
...
is not set,
we find such a $j$ before having to wrap from $n$ back to $1$).
