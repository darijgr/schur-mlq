...
For each $p \in \ive{n}$, let $j_p$ denote the site that is set by $i_p$.
Note that we can always rematch
\[
(_1\ (_2\ )_2\ )_1 \longrightarrow (_1\ (_2\ )_1\ )_2,
\]
where there can be any number of frozen letters in between.
Thus, we start with the natural bracketing.
By a straightforward induction argument, we can apply the rematching moves above so that matching agrees with the construction of $q(u)$ with the permutation $(i_1, i_2, \dotsc, i_n)$.
\Travis{This needs more work, but it is the general idea.}
