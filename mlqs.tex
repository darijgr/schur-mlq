...
In Sections~\ref{sec:thm_proof} and~\ref{sec:altproof}, we give two proofs of our main theorem.
...
permutation $\tup{i_1, i_2, \ldots, i_n}$ that satisfy $u_{i_k} = u_{i_{k+1}}$,
...
\begin{proof}
  ...
  \]
  this is a $(s_{i-1} \dotsm s_1)$-twisted MLQ of type $\mm$ and weight
...
Next, define the standardization of $u$ to be the word $\widetilde{u} := \widetilde{u}_1 \cdots \widetilde{u}_{n}$ obtained as follows. Let $k$ be minimal such that $\lvert \{ i \in \ive{n} \mid u_i < k \} \rvert = r$. Note that $k$ may not appear in $u$. Denote
\[
W := \{ w_1 > w_2 > \cdots > w_r \mid u_{w_i} < k \},
\]
and set
\[
\widetilde{u}_i = \begin{cases}
j & \text{if } u_i < k \text{ and } i = w_j \\
r + 1 & \text{otherwise.}
\end{cases}
\]
That is to say, the standardization is replacing the $i$-th letter that is not $k$ read right-to-left with $i$ and all letters $k$ with $r + 1$.
For example, we have
\begin{align*}
u & = {\color{gray} 4}3{\color{gray} 44}33{\color{gray} 4}22{\color{gray} 4}211{\color{gray} 4},
\\ \widetilde{u} & = {\color{gray} 9}8{\color{gray} 99}76{\color{gray} 9}54{\color{gray} 9}321{\color{gray} 9}.
\end{align*}
Note that the type $\widetilde{\mm}$ of $\widetilde{u}$ is $\widetilde{m}_i = 1$ for $i \leq r$ and $\widetilde{m}_r = m_k$.
%Note that we can reconstruct $u$ given some $\widetilde{u}$ using only $v$.
Additionally, note that the standardization is order preserving in the sense of $u_i \geq u_j$ if and only if $\widetilde{u}_i > \widetilde{u}_j$.

There exists $T$ such that $\bigvee_T \widetilde{u} = u$.
Thus, from Lemma~\ref{lemma:queue_merge}, there exists some $\sigma$-twisted MLQ $\widetilde{\qq}'$ such that $\widetilde{\qq}'(1 \cdots 1) = \widetilde{u}$, and from Theorem~\ref{thm:permutation}, there exists a corresponding MLQ $\widetilde{\qq}$ such that $\widetilde{\qq}(1 \cdots 1) = \widetilde{u}$.\footnote{This is essentially the same as the proof of Theorem~\ref{thm:merge}. Moreover, this is done bijectively; see also Remark~\ref{rmk:bijective_proof}.}
...
Thus, given disjoint $r$-paths $\rho = \{ \rho_i \}_{i=1}^r$ with corresponding MLQ $\widetilde{\qq} = (\widetilde{q}_1, \dotsc, \widetilde{q}_r)$, we construct $\qq_{\rho} = (q_1, \dotsc, q_{\ell-1})$ by
\[
...
Therefore, we have the word $u$ in Theorem~\ref{thm:determinant_form} and its standardization
\begin{align*}
u & = 33{\color{gray} 4}2{\color{gray} 4}22{\color{gray} 4}1,
\\ \widetilde{u} & = 65{\color{gray} 7}4{\color{gray} 7}32{\color{gray} 7}1.
\end{align*}
...
For each queue $q$, let $q^*$ be the \defn{contragredient dual queue} of $q$; this is the queue defined by $i \in q^* \Longleftrightarrow n+1-i \notin q$.
Let $C = (q_1, q_2)$ be an $(r_1, r_2)$-configuration.
Then, the balanced and the unbalanced intervals of $C$ are exactly the balanced and unbalanced intervals of the $(n-r_1, n-r_2)$-configuration $C^* = (q_1^*, q_2^*)$, reflected around the middle of $\ive{n}$.
Furthermore, for a word $u \in \mcW_n$ with $\ell-1$ classes, let $u^*$ be obtained by replacing $k \leftrightarrow \ell - k$.
Then for $u \in \set{1,2}^n$, we have $C(u)_i = C^*(u^*)^*_{n+1-i}$, where $u^*$ is the word obtained from $u$ by switching $1$'s and $2$'s, where we treat $C^*(u^*)$ as a word with $4$ classes.
(This follows from applying Lemma~\ref{le:dual} twice.)
\end{remark}
...
\begin{proof}
  As in the proof of Lemma~\ref{le:BU}, we assume that $r_1 > r_2$, noting that the case $r_1 < r_2$ follows from Remark~\ref{rmk:balanced-dual-let} and Lemma~\ref{le:BU}.
  
  Let $B$ be a balanced interval, and let $B^c := [\ell] \setminus B$ denote the complement of $B$.
  Note that for $B \subseteq \inter(i,j)$, the value of $C(u)_k$ for $k \in B^c$ is determined entirely by $q_1\cap B^c$, $q_2\cap B^c$, and $u\rvert_{B^c}$.
  Thus we can reduce to the case when there is no such balanced interval $B$ contained in $\inter(i,j)$.  
  ...
  Suppose $k = 0$ (\textit{i.e.}, $j = i + 1$).
  If $q_1(u)_j = 1$, then $q_1(u) = q_1(u_{i \leftrightarrow j})$ since the $u_j = 2$ does not set either $q_1(u)_i$ or $q_1(u)_j$.
  ...
  Since the squares in the lower row of the balanced interval of $j$ will all be labeled $4$, we see that in finding the first site to the left of $j$ when considering $q_1(u)_j = 2$ in the computation of $q_2\bigl( q_1(u) \bigr)$ from $q_1(u)$, the search goes beyond $i+k+1$.
...
\end{enumerate}
In each of these cases, it is easy to see that all particles in $u$ starting in unbalanced positions in $C$ or $C'$ simply are assigned to the same column two rows down, and that the labelling procedure inside each balanced interval is the same in $C$ and $C'$.
...
\Darij{The following definitions are clumsy, but they're the best
I can do.}
...
Furthermore, a \defn{trifiguration} shall mean a triple of queues.

If $Q = \tup{q_1, q_2, q_3}$ is a trifiguration, then we define two
trifigurations $\fraks_1 Q$ and $\fraks_2 Q$ as follows:
\begin{itemize}
\item We let $\fraks_1 Q = \tup{q'_1, q'_2, q_3}$, where
	  $\tup{q'_1, q'_2}$ is the dual configuration of $\tup{q_1, q_2}$.
\item We let $\fraks_2 Q = \tup{q_1, q'_2, q'_3}$, where
	  $\tup{q'_2, q'_3}$ is the dual configuration of $\tup{q_2, q_3}$.
\end{itemize}
Thus, $\fraks_1$ and $\fraks_2$ are two endomorphisms of the set of
all trifigurations.
It turns out that these endomorphisms satisfy the
\defn{braid relation}:

\begin{prop} \label{prop:braid}
We have $\fraks_1 \circ \fraks_2 \circ \fraks_1
	   = \fraks_2 \circ \fraks_1 \circ \fraks_2$.
\end{prop}

\begin{proof}
\darij{TODO!}
This follows either from \cite[Ch.~5, (5.6.3)]{Loth}, or
from crystal theory~\cite[Rule 3.10]{NY97}\travis{I'm not sure if this is the correct reference, but it does describe the $R$-matrix action}.

(Here is the connection to Lothaire:
A configuration $\tup{q_1, q_2}$ can be encoded as a $2n$-letter word
$w$ consisting of three types of letter: $-$, $($ and $)$.
Namely, the $\tup{2i-1}$-th letter of $w$ shall be $-$ if $i \notin q_1$,
and otherwise $($.
Meanwhile, the $2i$-th letter of $w$ shall be $-$ if $i \notin q_2$,
and otherwise $)$.
The letter $-$ is neutral; it won't hinder the matching of the parentheses.
Now, do cyclic parenthesis matching of the word $w$
(``cyclic'' means that a $($ at the end can be matched with a $)$ at
the beginning).
The operation $\sigma$ from Lothaire (specifically, his $\sigma_i$,
but we are in the 2-letter case) is just flipping all the unmatched
parentheses (although he doesn't define it this way).
But this corresponds to taking the dual configuration.)
\end{proof}

Next, we define two queues corresponding to a word $u \in \mcW_n$.
Namely, if $w \in \mcW_n$ and if $k \in \set{0, 1, \ldots, n}$, then
\begin{itemize}
\item we let $S_k(w)$ denote the set of the indices $i \in \ive{n}$
	  corresponding to the $k$ smallest letters $w_i$ of $w$,
	  if this set is uniquely determined;
	  otherwise, we leave $S_k(w)$ undefined.
\item we let $L_k(w)$ denote the set of the indices $i \in \ive{n}$
	  corresponding to the $n-k$ largest letters $w_i$ of $w$,
	  if this set is uniquely determined;
	  otherwise, we leave $L_k \tup{w}$ undefined.
\end{itemize}
Thus, the queues $S_k(w)$ and $L_k(w)$ are defined exactly
when $k \in \set{ p_i(\mm) \mid i \geq 0 }$ (with $\mm$
being the type of $w$), and otherwise are undefined.
Also, observe that $S_k(w)$ and $L_k(w)$ are two
complementary subsets of $\ive{n}$.
\Travis{This is bad notation as it looks like $S_k$ and $L_k$ are queues acting on the word $w$ and so $S_k(w)$ and $L_k(w)$ look like are words.}

The crucial tools in our second proof of Theorem~\ref{thm:permutation}
will be the following two facts:

\begin{lemma} \label{lem:SL.reconstruct}
A word $w \in \mcW_n$ can be uniquely reconstructed if its type and
the family of all queues $S_k(w)$ (for the $k$ for which they
are well-defined) are known.
\end{lemma}

\Travis{This statement is circular. I think you want to say given some family of queues satisfying some properties, then we can uniquely reconstruct $w$. Another way to say it is the map $w \mapsto (\mm, (S_k(w))_{k=0}^n)$ is injective.}
...
Let $w \in \mcW_n$ be a word, and let
$k \in \set{0, 1, \ldots, n}$ be such that
$S_k \tup{w}$ is well-defined.
Let $q$ be any queue.
Then, the dual configuration of $\tup{ S_k(w) , q }$
has the form $\tup{ ? , S_k\bigl(q(w)\bigr) }$
(where the question mark means a queue we aren't interested in).
\end{prop}

\Darij{FUCK: If $k = \abs{q}$, then $S_k \tup{w}$ is not
defined, but $S_k \tup{q\tup{w}}$ is (and equals $q$).
This looks like a manageable but annoying problem.

Good news: If $w$ has distinct letters, then
$S_k \tup{w}$ is ALWAYS defined. So we need to WLOG assume
that the letters of $w$ are distinct.
Lemma~\ref{lemma:queue_merge_commute} helps make this
assumption.}

\begin{proof}
\Darij{TODO. (This is where the serious work is.)}

MAIN IDEA:

First consider the case that $k$ is small; this means
that $k < \abs{q}$. Hence, the smallest $k$ letters of $w$
all get digested in Phase II. Then, the argument below
is basically about re-interpreting the Phase II movements as
a way to construct dual configurations.
Essentially: A letter falls down and scans to the right for
the smallest available spot in $q$; this creates balanced
intervals.
Here is, I think, how this proof can be formalized:
First, define the \defn{queueing interval} of a site $i$
as the cyclic interval starting at this site $i$ and ending at
the box $j$ where the ``ball from $i$ settles'' after
falling down to the bottom (what is our language for this,
again?).
In the following, notations like $c^\uparrow$ and
$c^\downarrow$ will refer to the configuration
$\tup{ S_k\tup{w} , q }$.
...
A set of sites will be called \defn{top-heavy} if each of its connected components (regarded as a subgraph of the cycle) is top-heavy.
Now, we claim that the union of the queueing intervals of all
the sites $i \in S_k \tup{w}$ is top-heavy.
Indeed, this is just saying that at each point in Phase~II
of the algorithm, the union of the queueing intervals used
so far is top-heavy.
To prove this, just observe that each element of a queue
that falls into this union must have already been set
(in our language: each box on the bottom row that falls
into this union has been filled).
Therefore, this union is top-heavy.
But this means that this union is balanced, because the
total number of balls at the top equals the total number
of boxes at the bottom (namely, $k$, because as we said all
the boxes have been filled), and therefore the
same holds for each connected component (otherwise, there
would be some connected component with fewer balls on top
than boxes on the bottom).
So all the balls in $S_k \tup{w}$ and all the boxes in
$S_k \tup{q\tup{w}}$ are balanced.
The unbalanced boxes are the ones where no balls have
settled.
The rest is clear from here.

When $k$ is large (that is, $k > \abs{q}$),
maybe something like this still works,
but what I did was showing instead that the dual configuration
of $\tup{ \ive{n} \setminus q, L_k \tup{w} }$ has the form
$\tup{ L_k \tup{q\tup{w}} , ? }$ (this is similar to the case
of $k$ small, because the largest $n-k$ letters of $w$ all
get digested in Phase I), and then apply one of the symmetries
of dual configurations.

When $k$ is middle-sized (that is, $k = \abs{q}$), the argument
is very simple.
\end{proof}
...
$C'$ of $C$ satisfies $C' \tup{u} = C \tup{u}$.

So let us consider $C$, $u$ and $C'$.
Write $C'$ in the form $C' = \tup{q'_1, q'_2}$.

It is easy to see that the words
$C(u) = q_2\bigl( q_1(u) \bigr)$
and
$C' \tup{u} = q'_2\bigl( q_1'(u) \bigr)$
have the same type;
...
$S_k\bigl( C'(u) \bigr) = S_k\bigl( C(u) \bigr)$
for all $k \in \set{0, 1, \ldots, n}$ for which these are
well-defined.
According to Lemma~\ref{lem:SL.reconstruct}, this will yield
$C'(u) = C(u)$, and thus our proof will be complete.

So let $k \geq 0$.

\Darij{see the FUCK above -- the argument below only works
if $S_k \tup{u}$ is well-defined, so it doesn't work for
$k = \abs{q_1}$ and for $k = \abs{q_2}$.
As I said, standardization would help.}

Using Proposition~\ref{prop:SL.dual} to compute dual
configurations, we can see how the triconfiguration
$\tup{S_k(u), q_1, q_2}$ transforms under the map
$\fraks_1 \circ \fraks_2 \circ \fraks_1$: Namely,
\[
\tup{S_k \tup{u}, q_1, q_2}
\overset{\fraks_1}{\mapsto}
\tup{?, S_k \tup{q_1 \tup{u}}, q_2}
\overset{\fraks_2}{\mapsto}
\tup{?, ?, S_k \tup{ q_2 \tup{ q_1 \tup{u} } }}
\overset{\fraks_1}{\mapsto}
\tup{?, ?, S_k \tup{ q_2 \tup{ q_1 \tup{u} } }} .
\]
Likewise, the same triconfiguration
$\tup{S_k(u), q_1, q_2}$ transforms under the map
$\fraks_2 \circ \fraks_1 \circ \fraks_2$ as follows:
\[
\tup{S_k \tup{u}, q_1, q_2}
\overset{\fraks_2}{\mapsto}
\tup{S_k \tup{u}, q'_1, q'_2}
\overset{\fraks_1}{\mapsto}
\tup{?, S_k \tup{ q'_1 \tup{u} } , q'_2}
\overset{\fraks_2}{\mapsto}
\tup{?, ?, S_k \tup{ q'_2 \tup{ q'_1 \tup{u} } }} .
\]
But the two maps are equal (by
Proposition~\ref{prop:braid}); thus, the resulting
triconfigurations must be identical:
\[
\tup{?, ?, S_k\Bigl( q'_2\bigl(q'_1(u) \bigr) \Bigr)}
=
\tup{?, ?, S_k\Bigl( q_2\bigl(q_1(u) \bigr) \Bigr)}
\]
Hence, $S_k\Bigl( q'_2\bigl(q'_1(u) \bigr) \Bigr)
= S_k\Bigl( q_2\bigl(q_1(u) \bigr) \Bigr)$.
In other words,
$S_k \bigl( C'(u) \bigr) = S_k \bigl( C(u) \bigr).$
This completes our proof, as explained above.
\end{proof}
