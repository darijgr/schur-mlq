...
\begin{example}
\begin{itemize}
\item Let $n = 9$, $u = 322131133$, $t = 3$ and $q = \set{1, 2, 3, 5, 6, 7}$.
...
\[
q_i^{(j)} = \set{ p \in \ive{n} \mid ( u^{\left[  i\right] } )_p = j }
...
Hence, the letters $1,2,\dotsc,i$ that appear in this word are exactly the $m_1 + m_2 + \cdots + m_i$ smallest letters of this word.
In other words, they are the $\abs{q_i}$ smallest letters of this word (since $m_1 + m_2 + \cdots + m_i = \abs{q_i}$).
...
\item And so on, until we get to the $j \in \ive{n}$ satisfying $(u^{\left[  i\right]})_j = i$.
\end{itemize}
In other words:
\begin{itemize}
...
This word $q_i\left(  u^{\left[  i-1\right]  }\right) = u^{\left[  i\right]}$ has type $\tup{m_1, m_2, \dotsc, m_i, n-p_i(\mm), 0, 0, \ldots}$; thus, it has at least one letter equal to $i$ (since $m_i > 0$), and is weakly decreasing up to level $i$ (by Claim~\ref{claim:weakly_decreasing_level_i}).
Also, this word has exactly $\abs{q_i}$ letters that are at most $i$ (since its type is $\tup{m_1, m_2, \dotsc, m_i, n-p_i(\mm), 0, 0, \ldots}$, and since $\abs{q_i} = m_1 + m_2 + \cdots + m_i$).
...
\]
(since $q_i\left(  u^{\left[  i-1\right]  }\right) = u^{\left[  i\right]}$).
