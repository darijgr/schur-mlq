...
However, it seems unlikely that our weighting is related to the steady state distribution for the inhomogeneous TASEP~\cite{AM13,AL14} or TARZP~\cite{KMO16II}.
...
In Sections~\ref{sec:thm_proof} and~\ref{sec:altproof}, we give two different proofs of our main theorem.
In Section~\ref{sec:remarks}, we give some additional remarks about our results.
\travis{Update once the sections have been decided.}
...
\item[Phase I]
  For $i = i_n, i_{n-1}, \ldots, i_{\abs{q}+1}$, do the following.
    Find the first $j$ weakly to the left (cyclically) of $i$ such that $j \notin q$ and $v_j$ is not set.
    Then set $v_j = u_i + 1$.

\item[Phase II]
  For $i = i_1, i_2, \ldots, i_{\abs{q}}$, do the following.
    Find the first $j$ weakly to the right (cyclically) of $i$ such that $j \in q$ and $v_j$ is not set.
    Then set $v_j = u_i$.
\end{description}
...
the steps of Phase~I (resp.\ Phase~II) are processed in the order of decreasing (resp.\ increasing) $i$).
\end{remark}
...
If we switch two adjacent entries $i_k$ and $i_{k+1}$ of the
...
In particular, Lemma~\ref{lemma:order_indep} states that the order between sites $i$ with equal $u_i$ does not matter.

\begin{remark}
\label{rmk:t-splitting}
Let $u$, $r$ and $q$ be as before.
...
For each queue $q$, let $q^*$ be the \defn{contragredient dual queue} defined by $i \in q^*$ if and only if $n+1-i \notin q$.
Similarly, for each word $u$ with $\ell$ classes, let $u^*$ be the \defn{contragredient dual word} defined by $u^*_i = \ell + 1 - u_{n+1-i}$.
For a fixed $k$, we call $\ell + 1 - k$ the \defn{contragredient dual letter} of $k$.
Note that for an $r$-queue $q$, and $q^*$ is the $(n-r)$-queue obtained by reflecting $[n] \setminus q$ across the middle of $[n]$.
Similarly, $u^*$ is obtained by reversing $u$ and taking the contragredient dual letters.

\begin{lemma}[Contragredient duality]
  \label{le:dual}
  Let $q$ be a queue and $u$ be a word with $\ell$ classes.
  Then we have $q(u) = q^*(q^*)^*$, \textit{i.e.}, we have $q(u)_i = \ell + 2 - q^*(u^*)_{n+1-i}$ for all $i$.
\end{lemma}
...
For the next lemma, we need further notation:
For any map $f \colon \set{1,2,\ldots} \to \set{1,2,\ldots}$ and word $u$,
we abuse notation to let $f(u)$ be the word obtained by applying $f$ to each letter of $u$.
...
\begin{proof}
Let $k \in \ZZ_{\geq 1}$ be arbitrary.
Consider some fixed permutation $\tup{i_1, i_2, \ldots, i_n}$ used in the construction of $q(u)$.
This same permutation works for the construction of $q\bigl(f_k(u) \bigr)$
since $\bigl( f_k(u) \bigr)_a \leq \bigl( f_k(u) \bigr)_b$ whenever $u_a \leq u_b$.
Thus, the construction of $q\bigl( f_k(u) \bigr)$ proceeds exactly as the
construction of $q(u)$, except that the letters used to build
the word $q\bigl( f_k(u) \bigr)$ are those of $f_k(u)$ instead of those of $u$.
%In particular, for a given site $k$, the letter $v_k$ is set at
%the same moment as the letter $v'_k$ is set; we just say that
%``site $k$ is set''.
% commented stuff below went here.
Suppose $q(u)_i \leq q(u)_j$.
So either
\begin{itemize}
\item $q(u)_i$ and $q(u)_j$ are set in the same phase or
\item $q(u)_i$ is set in Phase~II and $q(u)_j$ in Phase~I.
\end{itemize}
Therefore, we have $q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j$.

For the converse direction, we shall prove the contrapositive.
So we assume that $q(u)_i > q(u)_j$.
Thus, either
\begin{itemize}
\item $q(u)_i$ and $q(u)_j$ are set in the same phase or
\item $q(u)_i$ is set in Phase~I and $q(u)_j$ in Phase~II.
\end{itemize}
Let
\[
k = \begin{cases}
q(u)_j & \text{if $q(u)_j$ is set during Phase~II,} \\
q(u)_j - 1 & \text{otherwise.}
\end{cases}
\]
Therefore, if $q(u)_j$ is set during Phase~II, we have $q\bigl( f_k(u) \bigr)_i \geq 2$ and $q\bigl( f_k(u) \bigr)_j = 1$.
Otherwise, we have $q\bigl( f_k(u) \bigr)_i = 3$ and $q\bigl( f_k(u) \bigr)_j \leq 2$.
In either case, $q\bigl( f_k(u) \bigr)_i > q\bigl( f_k(u) \bigr)_j$.

\begin{comment}
Hence, for each $m \in \ive{n}$, we have
\begin{equation}
 q(f_k(u))_m
 = \begin{cases}
    f_k(q(u)_m), & \text{ if } m \in q ; \\
    f_k(q(u)_m - 1) + 1, & \text{ if } m \notin q
   \end{cases}
 .
 \label{pf.le:mono.correspondence}
\end{equation}

Now, assume that $q(u)_i \leq q(u)_j$.
We want to prove that
$q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j$.
If $i \in q$ and $j \notin q$, then this is obvious.
If $i \notin q$ and $j \in q$, then we obtain a contradiction to
$q(u)_i \leq q(u)_j$.
Hence, only two cases remain: the case when both $i$ and $j$ belong to $q$,
and the case when neither $i$ nor $j$ belongs to $q$.
Let us only consider the second case (as the first is analogous).
So $i \notin q$, and thus~\eqref{pf.le:mono.correspondence} yields
$q(f_k(u))_i = f_k(q(u)_i - 1) + 1$.
Similarly,
$q(f_k(u))_j = f_k(q(u)_j - 1) + 1$.
But $q(u)_i \leq q(u)_j$, so that
$f_k(q(u)_i - 1) \leq f_k(q(u)_j - 1)$
and thus
$q(f_k(u))_i = f_k(q(u)_i - 1) + 1 \leq f_k(q(u)_j - 1) + 1 = q(f_k(u))_j$,
as we desired to prove.

We thus have shown that if $q(u)_i \leq q(u)_j$, then
$q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j$ for all $k \in \ZZ_{\geq 1}$.

Conversely, assume that
$q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j$ for all $k \in \ZZ_{\geq 1}$.
We must show that $q(u)_i \leq q(u)_j$.
%Assume the contrary.
%Hence, $q(u)_i > q(u)_j$.
If $i \in q$ and $j \notin q$, then this is obvious.
If $i \notin q$ and $j \in q$, then we get a contradiction (since
$q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j$ will fail for each $k$).
Thus, again, only two cases remain: the case when both $i$ and $j$ belong to $q$,
and the case when neither $i$ nor $j$ belongs to $q$.
Let us only consider the second case (as the first is analogous).
Thus, both $q(u)_i$ and $q(u)_j$ are $\geq 2$.
As before, we obtain
$q(f_k(u))_i = f_k(q(u)_i - 1) + 1$ and
$q(f_k(u))_j = f_k(q(u)_j - 1) + 1$ for each $k$.
Thus, for each $k$, we have
$f_k(q(u)_i - 1) + 1 = q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j = f_k(q(u)_j - 1) + 1$
and therefore $f_k(q(u)_i - 1) \leq f_k(q(u)_j - 1)$.
But this implies that $q(u)_i - 1 \leq q(u)_j - 1$
(because if two positive integers $a$ and $b$ satisfy $f_k(a) \leq f_k(b)$ for each $k \in \ZZ_{\geq 1}$, then $a \leq b$).
In other words, $q(u)_i \leq q(u)_j$, and the proof is complete.
\end{comment}
\end{proof}
...
Let $t$ be as in Remark~\ref{rmk:t-splitting}.
Note that the permutation $\tup{i_1, i_2, \ldots, i_n}$ in the construction of $q(u)$
also works for the construction of $q(\merge{h} u)$;
thus,
$q(\merge{h} u) = \merge{h} q(u)$ for all $h < t$ and $q(\merge{h} u) = \merge{h+1} q(u)$ for all $h \geq t$. 
%since equal entries can be processed in any order in the definition of the action of $q$.

Assume $q(u)_i \leq q(u)_j$.
Then, $f_k(u) = \merge{1} \merge{2} \cdots \merge{k-1} \merge{k+1} \cdots \merge{\ell-1} u$
(where $\ell$ is the number of classes in $u$),
and so (by the preceding observation) $q(f_k(u)) = \merge{r_1} \merge{r_2} \cdots \merge{r_{\ell-2}} q(u)$
for some $r_1, \ldots, r_{\ell-2}$.
Hence, $q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j$ for all $k \in \ZZ_{\geq 1}$.

Conversely, assume $q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j$ for all $k \in \ZZ_{\geq 1}$.
Suppose that $q(u)_i > q(u)_j$, then we can apply merges to $u$ to obtain $f_{k'}$ with $k' = q(u)_j$.
...
Indeed, the word $\vee^{(k)} w$ can be obtained from $w$ by decrementing (by $1$) all but the $k$ smallest letters of $w$.
...
Fix some $j \geq 1$, and let $k = p_j(\mm)$.
If $q$ is a queue such that $\lvert q \rvert \neq k$, then
\[
\vee^{(k)} q(u) = q(\vee^{(k)} u).
\]
...
\begin{proof}
The word $\vee^{(k)} u$ is well-defined since $k = p_j(\mm)$ for some $j$;
...
Now, set $k = p_i(\mm)$. Then, we have
\begin{equation}
\label{pf.lemma:queue_merge.4}
 \merge{i} u = \merge{i} \qq\bigl( q' (1 \dotsm 1) \bigr) = \vee^{(k)} \qq\bigl( q' (1 \dotsm 1) \bigr) = \qq\bigl( \vee^{(k)} q' (1 \dotsm 1) \bigr)
\end{equation}
by repeated use of Lemma~\ref{lemma:queue_merge_commute} since none of the queues in $\qq$ have size $k$.
...
\begin{proof}
  ...
  \end{itemize}
  Given any such pair $\tup{\qq, X}$, we set
  \[
...
Thus, $\qq (1 \dotsm 1) = u$, and the type of $\qq$ is the type $\mm$ of $u$.
In particular, $q_{\ell-1} = B$.
...
Next, define the \defn{near standardization} of $u$ to be the word $\std{u} := \std{u}_1 \cdots \std{u}_{n}$ obtained as follows. Let $k$ be minimal such that $\lvert \{ i \in \ive{n} \mid u_i < k \} \rvert = r$. Note that $k$ may not appear in $u$. Denote
\[
...
\end{cases}
\]
That is to say, the near standardization is replacing the $i$-th letter that is not $k$ read right-to-left with $i$ and all letters $k$ with $r + 1$.
For example, we have
\begin{align*}
...
\end{align*}
Note that the type $\widetilde{\mm}$ of $\std{u}$ is $\widetilde{m}_i = 1$ for $i \leq r$ and $\widetilde{m}_r = m_k$.
%Note that we can reconstruct $u$ given some $\std{u}$ using only $v$.
Additionally, note that the near standardization is order preserving in the sense of $u_i \geq u_j$ if and only if $\std{u}_i > \std{u}_j$.
...
\end{align*}
The fact that $\qq = \qq_{\rho'}$ follows from (a mild generalization of) the monotonicity lemma (Lemma~\ref{le:mono}).
\end{proof}
...
Hence, our proof is reduced to constructing the \defn{dual}\footnote{This is a different duality than the contragredient duality of a queue given by Lemma~\ref{le:dual}.} $(r_2,r_1)$-configuration to $C$, which we denote by $C'$, such that $C'$ satisfies $C(u) = C'(u)$, $\wt(C) = \wt(C')$, and $C'' = C$ (\textit{i.e.} the dual of $C'$ equals $C$).
...
In Part~A we describe how to split any configuration into \defn{balanced} and \defn{unbalanced} intervals.
...
For the remainder of this section, we fix an $(r_1,r_2)$-configuration $C = (q_1, q_2)$ and binary word $u \in \set{1,2}^n$.
...
Then, the interval $\mcI$ of $C$ is balanced if and only if $\mcI^* = \set{ n + 1 - i \mid i \in \mcI }$ of the $(n-r_1, n-r_2)$-configuration $C^* = (q_1^*, q_2^*)$ is balanced.
In other words, the balanced intervals of $C$ are exactly the balanced intervals of $C^*$ but have been reflected around the middle of $\ive{n}$.
In addition, we have
\[
C(u) = q_2\bigl( q_1(u) \bigr) = q_2\bigl( q^*_1(u^*)^* \bigr) = q_2^*\bigl( q_1^*(u^*) \bigr)^* = C^*(u^*)^*
\]
from applying Lemma~\ref{le:dual} twice.
In particular, for $u \in \set{1,2}^n$, we have $C(u)_i = 6 - C^*(u^*)_{n+1-i}$ where we treat $u$ and $C^*(u^*)$ as a word with $2$ and $4$ classes, respectively.
\end{remark}
...
%balanced intervals in C and C' are the same (because #unbalanced columns is same in both)
%%%%%%%%%%
...
\begin{proof}
Let $\delta_{ab}$ denote the Kronecker delta.
Note that $q(u)_k = u_k + \delta_{2u_k}$ for all $k \in \inter(i,j)$ by our assumptions
(since we can start each phase by going through the $k \in \inter(i, j)$),
and similarly $q(u_{i \leftrightarrow j})_k = u_k + \delta_{2u_k}$ for all $k \in \inter(i,j)$.
Therefore, if $u_i \neq u_j$, then we have $q(u)_i = 3$ and $q(u)_j = 1$
as $q(u)_i$ (resp.~$q(u)_j$) is the first unset letter encountered by
the $2$ (resp.~$1$) in Phase~I (resp.\ Phase~II).
Similarly for $q(u_{i \leftrightarrow j})$, and hence the claim follows.
If $u_i = u_j$, then the claim is trivial (as $u = u_{i \leftrightarrow j}$).
\end{proof}
...
\label{le:BB}
  ...
  \begin{itemize}
    ...
    \item $T(k) \in \{(1,\bigcirc),(2,\square)\}$ for all balanced $k\in\inter(i,j)$ \travis{Do you really want balanced here? If so, then the proof does not work as there are no conditions on the unbalanced letters.}.
  \end{itemize}
...
\label{le:BU}
  ...
  \begin{itemize}
    ...
    \item $u_k = 1$ for all unbalanced $k \in \inter (i,j)$, and
...
\begin{proof}
  We assume here that $r_1 > r_2$, noting that the dual case $r_1 < r_2$ follows from Remark~\ref{rmk:balanced-dual-let} and Lemma~\ref{le:UB}.
  Thus, we have $k \in q_1$ for all unbalanced $k$, and so $q_1(u) = q_1(u_{i\leftrightarrow j})$ by Lemma~\ref{le:orig}.
\end{proof}
...
\label{le:UB}
  ...
  \begin{itemize}
    ...
    \item $u_k = 2$ for all unbalanced $k \in \inter(i,j)$, and
    \item $T(k) \in \{(1,\bigcirc),(2,\square)\}$ for all balanced $k \in \inter(i,j)$.
  \end{itemize}
  Then we have $C(u) = C(u_{i \leftrightarrow j})$.
\end{lemma}
...
\begin{proof}
  We assume that $r_1 > r_2$, noting that the case $r_1 < r_2$ follows from Remark~\ref{rmk:balanced-dual-let} and Lemma~\ref{le:BU}.
  Hence, we have $T(i) = (1, \bigcirc)$, which implies $q_1(u)_i = 1$. Moreover,  for all unbalanced $k \in \inter(i,j)$, we have $T(k) = (2, \bigcirc)$ and $k \notin q_2$.
  
  Recall from the proof of Lemma~\ref{le:orig} that we have $q_1(u)_k = u_k + \delta_{2u_k}$ for all balanced $k \in \inter(i,j)$.
  Hence, we have $q_2\bigl( q_1(u) \bigr)_k = 4$ for all balanced $k \in \inter(i,j) \setminus q_2$.
  Moreover, for any balanced interval $\mcI \subseteq \inter(i,j)$, we must have $q_2\bigl( q_1(u) \bigr)_k = 1$ for all $k \in \mcI \cap q_2$.
  In particular, we can set every $q_1(u)_k$ and $q_2\bigl( q_1(u) \bigr)_k$ for $k \in \mcI$ by using $u_{k'}$ and $q_1(u)_{k'}$, respectively, with $k' \in \mcI$.
  Therefore, we can assume there does not exist a balanced interval $\mcI \subseteq \inter(i,j)$.
  
  Suppose $j = i + 1$.
  If $q(u)_j = 1$, then the $2$ from $u_j$ passes over both sites $i$ and $j$.
  Hence, we have $q_1(u_{i \leftrightarrow j}) = q_1(u)$, which implies $C(u) = C(u_{i \leftrightarrow j})$.
  Next, if $q(u)_j = 2$, then we can set $q(u)_j$ by using $u_j$.
  Thus, we have $q_1(u_{i \leftrightarrow j}) = q_1(u)_{i \leftrightarrow j}$.
  If $j \in q_2$, then $q_2\bigl( q_1(u) \bigr)_j = 1$ and can be set by $q_1(u)_i$.
  This implies that $q_2\bigl( q_1(u_{i \leftrightarrow j}) \bigr)_j = 1$ and $C(u) = C(u_{i \leftrightarrow j})$.
  Instead, assume $j \notin q_2$, and so $q_2\bigl( q_1(u) \bigr)_j = 4$ since $j$ is balanced.
  Hence, $q_2\bigl( q_1(u) \bigr)_j = 4$ cannot be set by either $q_1(u)_i$ or $q_1(u)_j$, which implies $C(u) = C(u_{i \leftrightarrow j})$.
  
  Next, suppose $i+1$ is balanced, and so we have $T(k) = (1, \bigcirc)$ and $k \notin q_2$ as otherwise there would be a balanced interval contained in $\inter(i,j)$.
  However, the proof that $C(u) = C(u_{i \leftrightarrow j})$ is similar to the case when $j = i+1$ as $q_1(u)_i$ and $q_1(u)_j$ cannot set $q_2\bigl( q_1(u) \bigr)_k = 4$ for all $k \in \inter(i,j)$.
  
  Finally, suppose that $i+1$ is unbalanced.
  We proceed by induction on $\abs{\inter[i,j]}$.
  Assume the claim holds for all intervals $\mcI$ such that $\abs{\mcI} < \abs{\inter[i,j]}$.
  We have $q_1(u_{i \leftrightarrow (i+1)}) = q_1(u)_{i \leftrightarrow (i+1)}$ as above.
  Therefore, we have
  \[
    C(u_{i \leftrightarrow j}) = C\biggl( \left(u_{i \leftrightarrow (i+1)}\right)_{(i+1) \leftrightarrow j} \biggr) = C(u_{i \leftrightarrow (i+1)}) = C(u)
  \]
...
Recall that $m_1(u)$ denotes the number of occurrences of $1$ in $u$ (and $u_0$).\travis{I believe you assuming $r_1 > r_2$ here. What about when $r_1 \leq r_2$?}
\begin{enumerate}
\item There is some balanced $i$ such that $T(i) = (1, \square)$, but there is no balanced $j$ such that $T(j) = (2,\bigcirc)$.
  This occurs when $m_1(u) > r_1$. \travis{This is only one direction of implication outright. Consider $C = \set{ \{1,3\},\{1\} }$ with $u = 112$. Granted, with Lemma~\ref{le:BU}, this would get a reduction, but this indicates more details are needed to say ``equivalently.''}
\item There is some balanced $i$ such that $T(i) = (2, \bigcirc)$, but there is no balanced $j$ such that $T(j) = (1,\square)$.
  This occurs when $r_2 \geq m_1(u)$.\travis{Actually, this implication is not obvious. I had to apply~\ref{le:UB} and do a little induction.}
\item For all balanced $i$, we have $T(i) \in \{(1,\bigcirc),(2,\square)\}$.
  This occurs when $r_1 \geq m_1(u) > r_2$.
\end{enumerate}
In each of these cases, it is easy to see that the labeling procedure inside each balanced interval is the same in $C$ and $C'$.
Moreover, all letters in $u$ starting in unbalanced positions in $C$ or $C'$ simply are assigned to the same column two rows down.
...
%=====================================================================
\section{Alternative proof of Theorem~\ref{thm:permutation}}
\label{sec:altproof}

In this section, we shall give a different proof of
Theorem~\ref{thm:permutation}, which relies on the braid relation for dual
configurations. We need some definitions first.

As in Section \ref{sec:thm_proof}, we define the concept of an
$\tup{r_1, r_2}$-configuration $C$, and that of the
corresponding dual $\tup{r_2, r_1}$-configuration $C'$.
A \defn{configuration} shall simply mean a pair of queues (\textit{i.e.},
an $\tup{r_1, r_2}$-configuration for some $r_1$ and $r_2$).
Thus, any configuration $C$ has a \defn{dual configuration} $C'$.

Fix $k \geq 1$.
In the following, an \defn{MLQ} will just mean a $k$-tuple of queues
(without any restriction on their sizes).
We want to define an action of $\SymGp{k}$ on MLQs
with simple transpositions $\fraks_i$ acting by
\[
\fraks_i \tup{q_1, q_2, \dotsc, q_k} = \tup{q_1, \dotsc, q_{i-1}, q'_i, q'_{i+1}, q_{i+2}, \dotsc, q_k},
\]
...
Thus, to show this is indeed an action, it remains to show the following.
...
\]
for any MLQ $\qq$ and any $i \in \set{1, 2, \ldots k-1}$.
\end{prop}
...
To any MLQ $\qq = \tup{q_1, q_2, \ldots, q_k}$, we assign the
$k \times n$-matrix $M_{\qq} \in A^{k \times n}$, whose $\tup{i, j}$-th
entry is $i$ if $j \in q_i$ and $\circ$ otherwise.

If $M \in A^{k \times n}$ is any $k \times n$-matrix, and if
$j \in \set{1, 2, \ldots, n}$, then $\col_j M$ will denote
the $j$-th column of $M$ read from top to bottom (and
regarded as a $k$-letter word in $A^*$).
Furthermore, $\colw M \in A^*$ will denote the concatenation
$\tup{\col_1 M} \tup{\col_2 M} \cdots \tup{\col_n M}$.
For example, if $n = 5$, $k = 3$ and $\qq = \tup{\set{1, 3}, \set{2},
\set{2, 5}}$, then
\[
 M_{\qq}
 =
 ...
 \end{array}
 \qquad \text{ and } \qquad
 \colw \tup{M_{\qq}} = 1 \circ \circ \circ 2 3 1 \circ \circ \circ \circ \circ \circ \circ 3 .
\]

Given an MLQ $\qq$, we denote the word
$\colw \tup{M_{\qq}}$ by $\word \qq$.
Clearly, an MLQ $\qq$ is uniquely determined by
$\word \qq$.
In other words, the map $\word : \set{\text{MLQs}} \to A^*$
is injective.

Now, for each $i \in \set{1, 2, \ldots, k-1}$, we define an
operator $\sigma_i : A^* \to A^*$.
This operator $\sigma_i$ acts on a word
$p \in A^*$ % = \tup{p_1, p_2, \ldots, p_\ell}$
as follows:
\begin{itemize}
 \item Treat all letters $i$ in $p$ as opening parentheses,
       and all letters $i+1$ as closing parentheses,
       while all other letters are considered frozen.
       Now, match as many parentheses as possible
       according to the standard parenthesis-matching
       algorithm (i.e.: every time you find an opening
       ...
       part (i.e., the word obtained by removing all frozen
       letters) is
...
\end{itemize}
According to~\cite[Ch.~5, (5.6.3)]{Loth}, these operators
$\sigma_i$ satisfy
\begin{equation}
 \sigma_i \circ \sigma_{i+1} \circ \sigma_i
 = \sigma_{i+1} \circ \sigma_i \circ \sigma_{i+1}
 \label{pf.prop:braid.loth-eq}
\end{equation}
for all $i \in \set{1, 2, \ldots, k-2}$.
(Note that our letter $i$ corresponds to the $a_{k+1-i}$
in \cite{Loth}, since \cite{Loth}'s definition of $\sigma_i$
involves $a_i$ rather than $i+1$ as closing parenthesis
and $a_{i+1}$ rather than $i$ as opening one. Also,
\cite{Loth} does not include the letter $\circ$
in the alphabet, but this makes no difference to the proof,
since all letters $\circ$ are always frozen.)

Now, we claim that
\begin{equation}
 \word \tup{\fraks_i \qq} = \sigma_i \tup{\word \qq}
 \qquad \text{ for each MLQ } \qq \text{ and each } i .
 \label{pf.prop:braid.inter}
\end{equation}

[\textit{Proof of \eqref{pf.prop:braid.inter}:}
Idea: Fix an MLQ $\qq = \tup{q_1, q_2, \ldots, q_k}$
and an $i \in \set{1, 2, \ldots, k-1}$.
The word $\word \qq$ consists of $n$ blocks, each corresponding
to some column of the matrix $M_{\qq}$; by abuse of notation,
we shall refer to these blocks as the \textit{columns}
of $\word \qq$.
Consider the parenthesis-matching algorithm applied
to the word $\word \qq$;
simultaneously, consider the construction of the dual
configuration $\tup{q'_i, q'_{i+1}}$ of the configuration
$\tup{q_i, q_{i+1}}$.
We shall relate the frozen and unmatched letters in the
former algorithm to the balanced and unbalanced column in
the latter construction.
Note that when we say ``column $i$ is balanced'' (resp.
``unbalanced''), what we mean is that $i \in \ive{n}$ is
balanced (resp. unbalanced).

Each time the former algorithm matches an opening parenthesis
in column $u$ with a closing parenthesis in column $v$
(notice that $u \leq v$, because the algorithm does not
``wrap around'' the circle), the interval $\int[u, v]$
is is revealed to be a balanced interval for the latter
construction
(because all parentheses inbetween them are already
matched, and clearly are matched to each other because
otherwise the two parentheses in columns $u$ and $v$
would have prevented them from matching).
The same, of course, holds for the intervals $\int[u, v]$
for which there is no parenthesis-matching activity at all
between columns $u$ and $v$ of $\word q$.
Thus, if $u \in \ive{n}$ is such that the $u$-th column
of $\word \qq$ is frozen (i.e., all letters in this
column are frozen), then $u$ is a balanced letter.
As for the letters (necessarily $i$'s and $i+1$'s) that
do not get frozen at the end of the parentheses-matching
algorithm, we know that they form the word
$\underbrace{))\cdots)}_{a\text{ parentheses}}
\underbrace{((\cdots(}_{b\text{ parentheses}}$
for some $a$ and $b$.
They correspond to $a$ elements of $q_{i+1}$ and to
$b$ elements of $q_i$, with all the former lying to
the left of all the letter.
Let us assume $a \geq b$ (leaving the opposite case
to the reader).
Then, all the $b$ non-frozen opening parentheses,
as well as the first $b$ non-frozen closing parentheses,
will together form a balanced interval that ``wraps
around the circle'' (or, more precisely: the columns in
which they lie, as well as all the columns inbetween,
will do so).
The remaining $a-b$ non-frozen closing parentheses (or, more
precisely, the columns in which they lie) are unbalanced
(indeed, there must be at least $\abs{q_i} - \abs{q_{i+1}} = a-b$
unbalanced columns, and since we have ruled out all the other
columns, it is clear that the $a-b$ columns containing
non-frozen closing parentheses must be the ones).
Now, the construction of the dual configuration $\tup{q'_i, q'_{i+1}}$
from $\tup{q_i, q_{i+1}}$ involves moving these
$a-b$ unbalanced columns from $q_i$ to $q_{i+1}$.
Meanwhile, the definition of $\sigma_i \tup{\word q}$ from
$\word q$ involves replacing the corresponding $a-b$
closing parentheses by opening parentheses.
This is clearly the same thing.
Thus, \eqref{pf.prop:braid.inter} follows.]

\darij{TODO! Flesh this out.}

The equality \eqref{pf.prop:braid.inter} can be rewritten as a
commutative diagram:
\[
\xymatrix{
...
\qquad \text{ for all } i .
\]
In view of the injectivity of the map $\word : \set{\text{MLQs}} \to A^*$,
this diagram allows us to translate the equality
\eqref{pf.prop:braid.loth-eq} into
$\fraks_i \circ \fraks_{i+1} \circ \fraks_i
= \fraks_{i+1} \circ \fraks_i \circ \fraks_{i+1}$.
But this proves Proposition~\ref{prop:braid}.
\end{proof}
...
The crucial tools in our second proof of Theorem~\ref{thm:permutation} will be the following two facts:

\begin{lemma} \label{lem:SL.reconstruct}
Let $w, w' \in \mcW_n$.
Then $\bigl( [w]_k \bigr)_{k=0}^n = \bigl( [w']_k \bigr)_{k=0}^n$ if and only if $w = w'$.
\end{lemma}

\begin{proof}
Clearly $w = w'$ implies $\bigl( [w]_k \bigr)_{k=0}^n = \bigl( [w']_k \bigr)_{k=0}^n$.
Next, we assume that $\bigl( [w]_k \bigr)_{k=0}^n = \bigl( [w']_k \bigr)_{k=0}^n$.
Note that $w$ and $w'$ have the same type $\mm$.
Fix some $k$, and let $j$ be such that $k = p_j(\mm)$.
Thus, we have $[w]_k \setminus [w]_{k-p_j(\mm)} = [w']_k \setminus [w']_{k-p_j(\mm)}$ defined, which we denote by $P_j$.
\travis{This version of $k$ smallest letters is actively fighting us here.}
By construction, we have $w_i = j = w'_i$ for all $i \in P_j$.
Since this holds for all letters $k$, we have $w = w'$.
\end{proof}

\begin{prop} \label{prop:SL.dual}
Let $w \in \mcW_n$ be a word of type $\mm$ such that $k = p_{\alpha}(\mm)$ for some $\alpha$.
Let $q$ be a queue.
The dual configuration of $\tup{ [w]_k , q }$ has the form $\tup{q^{\dagger} , [q(w)]_k }$, where $q^{\dagger}$ is some queue.
\end{prop}

\begin{proof}
Let $C' = (q'_1, q'_2)$ denote the dual configuration of $C = \tup{ [w]_k , q }$.
The notations of $c^{\uparrow}$ and $c^{\downarrow}$ refer to $C$.

Suppose $k \leq \abs{q}$.
Therefore, for all $j \in [w]_k$, there exists a maximal balanced interval $\mcI_j$ such that $j \in \mcI_j$.
Recall that in Phase~II of computing $q(w)$, a letter $w_j$ moves weakly to the right until it finds the first unset $q(w)_{i_j}$ with $i_j \in q$.
We show $i_j \in q'_2 \cap \mcI_j$ by induction on $j$.
Next, suppose $i_a \in q'_2 \cap \mcI_a$ for all $a < j$ and we have set all $q(w)_{i_a}$ during Phase~II in computing $q(w)$.
Let $\mcI_j = \inter[e,f]$.
Since $\mcI_j$ is balanced and by our induction assumption, we have that
\[
c^{\uparrow}[i,j] \geq \abs{\set{ h \in \inter[j,f] \mid q(w)_h \text{ is currently set} }} + 1.
\]
Since we have $c^{\uparrow}\mcI_j = c^{\downarrow}\mcI_j$, there must exist a $i_j \in q'_2 \cap \mcI_j$ such that $q(w)_{i_j}$ is unset.
Thus, we set $q(w)_{i_j} = w_j$.
Note that a similar argument holds for $j = 1$.
Hence, $q'_2 \subseteq [q(w)]_k$.
However, we clearly have $\abs{q'_2} = k = \abs{[q(w)]_k}$, and so $q'_2 = [q(w)]_k$.

Now suppose $k > \abs{q}$.
Note that $\ive{n} \setminus [w]_k$ are the indices of the largest $n-k$ letters of $w$ and $n - \abs{q} < n-k$.
So from a similar argument as above but instead using Phase~I, we have
\begin{equation}
\label{eq:dual_claim}
\tup{ \ive{n} \setminus q, \ive{n} \setminus [w]_k }' = \tup{ \ive{n} \setminus [q(w)]_k, \ive{n} \setminus q^{\dagger} }.
\end{equation}
Next, it is straightforward to see that an interval $\mcI$ is balanced for $(q_1, q_2)$ if and only if $\mcI$ is balanced for $(\ive{n} \setminus q_2, \ive{n} \setminus q_1)$.
Hence, we have $(q_1,q_2)' = (q'_1, q'_2)$ if and only if $(\ive{n} \setminus q_2, \ive{n} \setminus q_1)' = ( \ive{n} \setminus  q'_2, \ive{n} \setminus q'_1)$.
Applying this to~\eqref{eq:dual_claim}, we obtain $\tup{ [w]_k , q }' = \tup{q^{\dagger} , [q(w)]_k }$.
\end{proof}

We can now give our second proof of Theorem~\ref{thm:permutation}:

\begin{proof}[Second proof of Theorem~\ref{thm:permutation}.]
We follow the same approach as in the first proof:
We must show that if $C = \tup{q_1, q_2}$ is any configuration
and if $u \in \mcW_n$ is any word, then the dual configuration
$C' = (q'_1, q'_2)$ of $C$ satisfies $C'(u) = C(u)$.

From Lemma~\ref{le:mono}, it is sufficient to consider standard words $u$.
It is straightforward to see that the words
$C(u) = q_2\bigl( q_1(u) \bigr)$
and
$C'(u) = q'_2\bigl( q_1'(u) \bigr)$
have the same type from Equation~\eqref{eq:queue_type_change}.
...
$[C'(u)]_k = [C(u)]_k$
for all $k \in \set{0, 1, \ldots, n}$ for which these are
well-defined.
According to Lemma~\ref{lem:SL.reconstruct}, this will yield $C'(u) = C(u)$, and thus our proof will be complete.

Note that $q_i(u)$ and $q'_i(u)$ are also standard words.
Using Proposition~\ref{prop:SL.dual} to compute dual
configurations, we can see how the MLQ
$\qq = \tup{[u]_k, q_1, q_2}$ transforms under the action of
$\fraks_1 \fraks_2 \fraks_1$: Namely, we have
\begin{align*}
\tup{[u]_k, q_1, q_2} & \overset{\fraks_1}{\longmapsto} \tup{\ast, [q_1(u)]_k, q_2}
\\ & \overset{\fraks_2}{\longmapsto} \tup{\ast, \ast, \bigl[ q_2\bigl( q_1(u) \bigr) \bigr]_k}
\\ & \overset{\fraks_1}{\longmapsto} \tup{\ast, \ast, \bigl[ q_2\bigl( q_1(u) \bigr) \bigr]_k},
\end{align*}
where $\ast$ denotes some queue.
Likewise, the action of $\fraks_2 \fraks_1 \fraks_2$ is given by
\begin{align*}
\tup{[u]_k, q_1, q_2} & \overset{\fraks_2}{\longmapsto} \tup{[u]_k, q'_1, q'_2}
\\ & \overset{\fraks_1}{\longmapsto} \tup{\ast, [q'_1(u)]_k , q'_2}
\\ & \overset{\fraks_2}{\longmapsto} \tup{\ast, \ast, \bigl[ q'_2\bigl( q'_1(u) \bigr) \bigr]_k}.
\end{align*}
...
\[
\tup{\ast, \ast, \bigl[ q'_2\bigl(q'_1(u) \bigr) \bigr]_k}
=
\tup{\ast, \ast, \bigl[ q_2\bigl(q_1(u) \bigr) \bigr]_k}.
\]
Hence, we have
\[
\bigl[ q'_2\bigl( q'_1(u) \bigr) \bigr]_k = \bigl[ q_2\bigl( q_1(u) \bigr) \bigr]_k.
\]
In other words, $[C'(u)]_k = [C(u)]_k$.
\end{proof}
...
\node (i3) at (0,-2) {$q_2$};
\node (s11) at (1,0) {$\ast$};
\node (s12) at (1,-1) {$[q_1(u)]_k$};
\node (s13) at (1,-2) {$q_2$};
\node (s21) at (2,0) {$\ast$};
\node (s22) at (2,-1) {$\ast$};
\node (s23) at (2,-2) {$\bigl[ q_2\bigl( q_1(u) \bigr) \bigr]_k$};
\node (t1) at (3,0) {$\ast$};
\node (t2) at (3,-1) {$\ast$};
\node (t3) at (3,-2) {$\bigl[ q_2\bigl( q_1(u) \bigr) \bigr]_k$};
...
\node (s13) at (1,-2) {$q'_2$};
\node (s21) at (2,0) {$\ast$};
\node (s22) at (2,-1) {$[q'_1(u)]_k$};
\node (s23) at (2,-2) {$q'_2$};
\node (t1) at (3,0) {$\ast$};
\node (t2) at (3,-1) {$\ast$};
\node (t3) at (3,-2) {$\bigl[ q'_2\bigl( q'_1(u) \bigr) \bigr]_k$};
...
Could this be done in a more transparent manner, such as by using pairs of labeled periodic Dyck or Motzkin paths?
