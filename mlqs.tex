...
To compute $\swt{135452}$, we need to examine MLQs of type $(1,1,1,1)$.
...
the sum $\sum_{(q_1, \dots, q_{\ell-1})} \prod_{i=1}^{\ell-1} \wt(q_i)$,
ranging over all $\sigma$-twisted MLQs such that $u = q_{\ell-1}(\cdots q_1(1\dotsm 1) \cdots )$,
...
Furthermore, we shall show that taking the dual configuration satisfies the braid relations (although we will not need this for our proof of Theorem~\ref{thm:permutation}).
...
If $r = s$, then the entire interval $[n]$ is also balanced.
...
\darij{Needs details, but probably not here.}
