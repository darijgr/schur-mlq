...
We visualize them as points on a line which ``wraps around'' cyclically; thus, for example, the sites weakly to the right of a site $i$ are $i, i+1, \ldots, n-1, n, 1, 2, 3, \ldots$ (in this order).
...
\Darij{I don't understand these last 3 lines.}
...
This is only well-defined if these $k$ smallest letters are determined uniquely, \textit{i.e.}, if $k \in \set{ p_j(\mm) \mid j \geq 0 }$, where $\mm$ is the type of $w$.
...
Let $k \in \set{ p_j(\mm) \mid j \geq 0 }$.
...
satisfies
\[
k \in \set{ p_j(\mm) \mid j \geq 0 } \subseteq \set{ p_j(\mm) \mid j \geq 0 } \cup \set{\abs{q}} = \set{ p_j(\nn) \mid j \geq 0 }.
\]
...
Next, define the \defn{near standardization} of $u$ to be the word $\std{u} := \std{u}_1 \cdots \std{u}_{n}$ obtained as follows.
Let $k$ be minimal such that $\lvert \{ i \in \ive{n} \mid u_i < k \} \rvert = r$. (In our case, $k = \ell$, but the definition allows for more general words $u$.)
Note that $k$ may not appear in $u$.
Let $w_1, w_2, \ldots, w_r$ be the elements of $B$ in decreasing order;
thus, $B = \set{w_1 > w_2 > \cdots > w_r}$ and $w_j = b_{r+1-j}$ for each $j$.
% Let $W$ denote the set of all $i$ satisfying $u_i < k$
% Denote
% \[
...
\end{cases}
\]
That is to say, the near standardization $\std{u}$ is obtained from $u$ by
replacing the $j$-th letter that is not $k$ read right-to-left with $j$ and all letters $k$ with $r + 1$.
For example,
\begin{align*}
\text{if }\ell = 4 \text{ and} \quad u & = {\color{gray} 4}3{\color{gray} 44}33{\color{gray} 4}22{\color{gray} 4}211{\color{gray} 4},
\\ \text{then} \quad \std{u} & = {\color{gray} 9}8{\color{gray} 99}76{\color{gray} 9}54{\color{gray} 9}321{\color{gray} 9}.
\end{align*}
Note that the type $\widetilde{\mm} = (\widetilde{m}_1, \widetilde{m}_2, \ldots, \widetilde{m}_{r+1}, 0, 0, \ldots)$
of $\std{u}$ is given by $\widetilde{m}_i = 1$ for $i \leq r$ and $\widetilde{m}_{r+1} = m_k$.
%Note that we can reconstruct $u$ given some $\std{u}$ using only $v$.
Additionally, note that the near standardization is order preserving in the sense
that $1 \leq i < j \leq n$ satisfy $u_i \geq u_j$ if and only if
either $\std{u}_i > \std{u}_j$ or $\std{u}_i = \std{u}_j = r+1$.
...
\begin{remark}
The following facts are easy to see (\textit{cf.} the theory of Dyck paths):
\begin{enumerate}
 \item For any balanced interval $\mcI$, we have
       $\lvert q_1 \cap \mcI \rvert = \lvert q_2 \cap \mcI \rvert$.
 \item The empty interval is balanced, whereas the set $\ive{n}$ of all sites is not.
 \item If $A$ and $B$ are two balanced intervals, then the three intervals
       $A \cap B$, $A \setminus B$ and $B \setminus A$ are balanced,
       and $A \cup B \neq \ive{n}$; moreover, the union $A \cup B$ is also
       balanced if it is an interval.
 \item Thus, the union of all balanced intervals (i.e., the set of all balanced
       $i \in \ive{n}$) is also the disjoint union of
       all inclusion-maximal balanced intervals.
...
Then, the interval $\mcI$ of $C$ is balanced if and only if the interval $\mcI^* = \set{ n + 1 - i \mid i \in \mcI }$ of the $(n-r_1, n-r_2)$-configuration $C^* = (q_1^*, q_2^*)$ is balanced.
...
\end{remark}
\Darij{I haven't checked the last sentence of the above remark; I wouldn't be surprised if the $6$ should be a $5$. Can't be bothered to do the computation, though.
Do we actually need this remark here, or is it only used in Erik's old proof?}
...
For each $i \in \ive{k-1}$, we define a map $\fraks_i : \set{\text{MLQs}} \to \set{\text{MLQs}}$ by
\[
...
Thus, we can define an action of $\SymGp{k}$ on the set of all MLQs
by letting each simple transposition $s_i$ as $\fraks_i$,
once we have proven the following:
...
We then construct a word $\word(\qq) \in A^*$ by reading $M_{\qq}$ column by column, from top-to-bottom, left-to-right.
...
\end{align*}
Clearly, an MLQ $\qq$ is uniquely determined by $\word(\qq)$
(since $n$ is fixed).
...
\begin{enumerate}
 \item Treat all letters $i$ in $p$ as opening parentheses `(',
       ...
       and all other letters are considered frozen.
...
We let $\zeta$ be the operator that sends each word $w_1 w_2 \cdots w_n$ to $w_2 w_3 \cdots w_n w_1$.
We also let $\zeta$ act on queues by the rule $\zeta q = \set{ i - 1 \mid i \in q }$ (recall that $0 = n$ as sites);
hence, $\zeta$ acts on MLQs entrywise (since an MLQ is a tuple of queues).
Clearly, $\word(\zeta \qq) = \zeta^k \word(\qq)$ for any MLQ $\qq = (q_1, \ldots, q_k)$.
...
Thus, let $\qq = (q_1, q_2)$. We construct a sequence of parentheses as follows: For each $j = 1, 2, \dotsc, n$ (in that order), we write
\begin{itemize}
\item an opening parenthesis `(' if $j \in q_1$ and $j \notin q_2$,
\item a closing parenthesis `)' if $j \notin q_1$ and $j \in q_2$,
\item a cancelling pair of parentheses `()' if $j \in q_1$ and $j \in q_2$,
\item nothing otherwise.
\end{itemize}
Note that this yields exactly the same parentheses sequence as for $\word(\qq)$ (removing all $\circ$ letters).
Furthermore, every `(' and `)' corresponds to a contribution to $c^{\uparrow}[1,n]$ and $c^{\downarrow}[1,n]$ respectively.
Additionally, every canceling pair of parentheses from positions $j < j'$ under the standard matching algorithm corresponds to the endpoints of a balanced interval $\inter[j,j']$.
Since paired parentheses (resp.\ balanced intervals) do not change under $\sigma_1$ (resp.~$\fraks_1$), we can consider $w$ (resp.\ $\qq$) such that there are no canceling parentheses (resp.\ balanced intervals that are subintervals of $\inter[1,n]$).

Let $w = \word(\qq) = w_0 w_1 \cdots w_{n-1}$ with $a$ opening parentheses and $b$ closing parentheses in its reduced signature.
By~\cite[Prop.~5.6.1]{Loth}, we can cyclically shift $w$ (that is, apply some power of $\zeta$ to $w$)
such that either only opening parentheses `(' or only closing parentheses `)` remain, but not both; this cyclic shift commutes with $\sigma_1$.
In other words, we have $\sigma_1 \zeta^a w = \zeta^a \sigma_1 w$.
By the definition of balanced intervals being cyclic, we have $\zeta^a \fraks_1 \qq = \fraks_1 \zeta^a \qq$.
\darij{From here, blah blah. This might be a dead end.}
Hence, $\word(\zeta^a \fraks_1 \qq) = \word(\fraks_1 \zeta^a \qq)$.
However, it is clear that $\word(\zeta^a \qq) = \zeta^a w$ if and only if $\fraks_1 \zeta^a \qq = \sigma_1 \zeta^a w$.
Hence, we obtain~\eqref{pf.prop:braid.inter}.
...
\begin{remark}
Our letter $i$ corresponds to the $a_{k+1-i}$ in~\cite{Loth}
...
For $k \notin \set{p_i(\mm) \mid i \geq 0}$, we set $[w]_k = 0$.
...
Let $w, w' \in \mcW_n$ be two words of the same type.
...
Let the words $w$ and $w'$ have the type $\mm$.
...
Let $w \in \mcW_n$ be a word of type $\mm$. Let $k = p_{\alpha}(\mm)$ for some $\alpha$.
Let $q$ be a queue.
...
The notations $c^{\uparrow}$ and $c^{\downarrow}$ refer to $C$.
...
We show $i_j \in q'_2 \cap \mcI_j$ by strong induction on $j$.
Suppose $i_a \in q'_2 \cap \mcI_a$ for all $a < j$ and we have set all $q(w)_{i_a}$ during Phase~II in computing $q(w)$.
Let $\mcI_j = \inter[e,f]$.
...
Hence, in order to prove Theorem~\ref{thm:permutation}, it suffices to show that $\swt{u}_{\sigma} = \swt{u}_{\sigma s_i}$ for each $i \in \ive{\ell-2}$
(since Theorem~\ref{thm:permutation} then follows by induction on length, \textit{i.e.} the minimal number of simple transpositions needed to write $\sigma$).
Thus the proof is reduced to showing that for any configuration $C = \tup{q_1, q_2}$
and any word $u \in \mcW_n$ the dual configuration
$C' = (q'_1, q'_2)$ of $C$ satisfies $C'(u) = C(u)$.

Each word $u$ can be obtained from a standard word by a sequence of
merges (each of which sends a word $w$ to $\vee^{(k)} w$ for some
$k \in \set{ p_j(\mm) \mid j \geq 0 }$, where $\mm$ is the type of
$w$). Lemma~\ref{lemma:queue_merge_commute} shows that these merges
commute with the action of a queue (and thus of an MLQ).
Hence, it is sufficient to consider standard words $u$.
...
%=====================================================================
\section{Alternative proof sketch of Theorem~\ref{thm:permutation}}
\label{sec:altproof}

We will now sketch an alternative proof of Theorem~\ref{thm:permutation} by directly showing that $(\fraks_i \qq)(u) = \qq(u)$, where the action $\fraks_i \qq$ is as given in Section~\ref{sec:thm_proof}.
As before, the proof thus reduces down to showing for a configuration $C$ and word $u$, we have $C(u) = C'(u)$.

We first consider the case $r_1 = r_2$, in which case $C' = C$ by definition and the claim is trivial.
...
In Part~A, we reduce the problem of showing that $C(u) = C'(u)$ for \emph{all} words $u$ to the showing it for a very restricted set of words $u$.
In Part~B, we show that for this restricted set of words $u$, we indeed have $C(u) = C'(u)$.
...
%%%%%%%%%%
\subsection*{Part B: finishing the proof}
...
We assume $r_1 > r_2$ as $r_1 < r_2$ follows by Lemma~\ref{le:dual}.
Note that $u_0$ falls into one of the following three cases.
Recall that $m_1(u)$ denotes the number of occurrences of $1$ in $u$ (and $u_0$).
\begin{enumerate}
...
Therefore, we have $\swt{u}_{\sigma} = \swt{u}_{\sigma s_i}$, and hence, Theorem~\ref{thm:permutation} by induction on length.
\qed
