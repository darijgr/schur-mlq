...
Let $\qq = \tup{q_1, q_2, \dotsc, q_{\ell-1}}$ be an MLQ with $\ell-1$ queues.
Then, $u = \qq(1^n)$ has type $\mm$ and is weakly decreasing up to level $\ell-1$ if and only if the MLQ $\qq$ has type $\mm$ and is interlacing.
\end{lemma}
...
Thus it is remains to show that $u = \qq(1^n)$ is weakly decreasing up to level $\ell - 1$ if and only if $\qq$ is interlacing.
