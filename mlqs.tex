...
$q^{(j)} \succeq r^{(j)} \gg q^{(j+1)}$ for all $j \in \ive{h-1}$.
...
The construction of $q(u)$ shows that each of the letters $(q(u))_j$ with $j \notin q$ is larger than each of the letters $(q(u))_j$ with $j \in q$.
(This is true for any queue $q$ and word $u$.)
Hence, the letters $(q(u))_j$ with $j \notin q$ are the $n - \abs{q}$ largest letters of $q(u)$.
...
the set $r^{(1)}$ consists of the $\abs{r^{(1)}}$ largest
elements of $r$;
the set $r^{(2)}$ consists of the $\abs{r^{(2)}}$ next-largest
elements of $r$;
and so on.
...
In order to better understand Phase~II, let us give a
mental model for it.
...
Hence, the construction of $q(u)$ yields $(q(u))_{j_k} = u_{i_k} = j$
...
satisfies $j_k \in q^{(j)}$ and $(q(u))_{j_k} = j$.
Hence, we have found at least $\abs{r^{(j)}}$ many sites $t \in q^{(j)}$
satisfying $(q(u))_t = j$ (namely, all the sites $j_k$ for $k \in \ive{n}$
...
sites $t \in q^{(j)}$.
Therefore, all the sites $t \in q^{(j)}$ satisfy $(q(u))_t = j$.
...
would yield $(q(u))_t = j \leq h-1 < h$, contradicting $(q(u))_t = h$);
thus, they have to belong to
\[
q \setminus \tup{q^{(1)} \cup q^{(2)} \cup \cdots \cup q^{(h-1)}} = q^{(h)}
\]
(since the sets $q^{(1)}, q^{(2)}, \ldots, q^{(h)}$ partition $q$).
Thus, we know that there are $\abs{q^{(h)}}$ many positions $t \in q^{(h)}$
satisfying $(q(u))_t = h$.
These positions must cover all of $q^{(h)}$
(since there are only $\abs{q^{(h)}}$ many positions $t \in q^{(h)}$).
Hence, every position $t \in q^{(h)}$ must satisfy $(q(u))_t = h$.
...
\left( q_h\left(  u^{\left[  h-1\right]  }\right) \right)_t =
\begin{cases}
...
$u^{\left[  h\right]} = q_h\left(  u^{\left[  h-1\right]  }\right)$.
...
those letters of $u$ that are $\leq \ell-1$.
