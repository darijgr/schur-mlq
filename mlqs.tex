...
% Ideally: Use ``\href{https://wiki.haskell.org/Maybe}{maybe}-queues'',
% so that $S_k \tup{w}$ is defined to be $\mathsf{Nothing}$
% instead of being left undefined.
% The problem is: how to define dual configuration if there
% are $\mathsf{Nothing}$s, in such a way that braid still holds?

Good news: If $w$ has distinct letters, then
$S_k \tup{w}$ is ALWAYS defined. So we can use
standardization, if we have it handy.}

\begin{proof}
\Darij{TODO. (This is where the serious work is.)}

MAIN IDEA:
When $k$ is small (i.e., the smallest $k$ letters of $w$
all get digested in Phase II), this is basically about
re-interpreting the Phase II movements as a way to construct
dual configurations.
Essentially: A letter falls down and scans to the right for
the smallest available spot in $q$; this creates balanced
intervals (how?).

When $k$ is large, maybe something like this still works,
