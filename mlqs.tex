...
For each permutation $\tau \in \SymGp{r}$, we let $v_\tau$ be the $r$-tuple
$\tup{\tau_1, \tau_2, \ldots, \tau_r}$ (that is, the one-line notation of $\tau$),
and we define the word $u_\tau \in \mcW_n$ by $(u_\tau)_i = \tau_j$
if $i = b_j$ for some $j$, otherwise $(u_\tau)_i = r+1$.
(Thus, $u_\tau$ is the word $u$ as given in Theorem~\ref{thm:determinant_form} with $\ell = r+1$ and $v = v_\tau$.)
To ease notation, we shall identify any $\tau \in \SymGp{r}$ with the corresponding $u_\tau \in \mcW_n$.

In~\cite[Conj.~3.10]{AasLin17}, a formula for the spectral weight $\swt{\sigma_S}$ at $x_1 = \cdots = x_n = 1$ is conjectured.
We now prove a spectral weight version of this conjecture.
...
\end{align*}
\travis{I think this was suppose to be a $-1$ in the $h_k$ subscripts as per Theorem~\ref{thm:determinant_form}.}
where $\gamma_j = j - \lvert \{t \in T \mid t < j \} \rvert$ and the second equality is by Theorem~\ref{thm:determinant_form}.
By using the well-known formula for M\"obius inversion on the boolean lattice, the claim follows.
\end{proof}
...
Therefore, by taking Corollary~\ref{cor:special_weight_lacunar} specialized at $x_1 = \cdots = x_n = 1$, we obtain~\cite[Conj.~3.10]{AasLin17} (which is a generalization of~\cite[Conj.~3.9]{AasLin17}).
