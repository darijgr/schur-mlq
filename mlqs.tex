...
We call a $k$-tuple of vertices of the lattice a \defn{$k$-vertex}.
\begin{verlong}
...
\[
\xcoord(A') \leq\xcoord(A), \qquad \ycoord(A') \geq \ycoord(A),\qquad \xcoord(B') \leq \xcoord(B), \qquad \ycoord(B') \geq \ycoord(  B).
\]
...
\leq\xcoord\left(  B'\right)  $ and $\ycoord%
\left(  A\right)  \leq\ycoord\left(  B'\right)  $.
Similarly, the existence of a path $p'$ from $A'$ to $B$
yields $\xcoord\left(  A'\right)  \leq\xcoord%
\left(  B\right)  $ and $\ycoord\left(  A'\right)
\leq\ycoord\left(  B\right)  $.
...
we obtain $\ycoord\left(  A'\right)  =\ycoord%
\left(  A\right)  $. Thus, $\ycoord\left(  A'\right)
...
A'\right)  \leq\xcoord\left(  \underbrace{A}_{=B'%
}\right)  =\xcoord\left(  B'\right)  \leq\xcoord%
\left(  B\right)  $).
...
would have $\ycoord\left(  A'\right)  <\ycoord%
...
\[
x=\xcoord\left(  A'\right)  ,\qquad y=\ycoord%
\left(  A\right)  ,\qquad x=\xcoord\left(  B\right)  ,\qquad
...
}\right)  $; hence, $\xcoord\left(  h_{M+1}\left(
...
We now shall state a crucial result, which follows from the Lindstr\"om--Gessel--Viennot lemma and exists in the folklore\travis{This is poorly phrased, but otherwise Darij, the footnote as currently stated makes it seems like we are just avoiding a proof. We should try to make this better.}:\footnote{
...
$\xcoord(A_j) \leq \xcoord(A_i)$, $\ycoord(A_j) \geq \ycoord(A_i)$, $\xcoord(B_{\sigma(i)}) \leq \xcoord(B_{\sigma(j)})$, and $\ycoord(B_{\sigma(i)}) \geq \ycoord(B_{\sigma(j)})$.
...
Then,
\[
\sum_{T \in \SST(\lambda, s)} \wt(T) = \left(  \prod_{i=1}^{k} x_{s_i} \right)  \det\left(  h_{\lambda_j-j+i-1}(  x_1,x_2,\ldots,x_{s_j})  \right)_{1\leq i\leq k,\ 1\leq j\leq k} .
\]
\end{thm}
...
In other words, there exists some $q \in [i_1, i_1+1, \dotsc, i_2]$ \travis{What does this mean? Is this suppose to be a set?} such that $(q,j) \notin \diag{\lambda}$.
...
&  = \left(  \prod_{i=1}^k x_{s_i} \right)  \det\left( h_{\lambda_j-j+i-1}(x_1, x_2, \dotsc, x_{s_j}) \right)_{1\leq i\leq k,\ 1\leq j\leq k} .
\end{align*}
\end{proof}
...
We have $A \succeq B$ if and only if there exists $\phi \colon B \to A$ a bijection satisfying $\phi(b) \geq b$ for each $b \in B$.
\end{lemma}
...
%Forget that we fixed $\qq$. % I don't understand what purpose this sentence serves -- Travis
...
\[
u^{\left[  i\right]} := (q_1, q_2, \dotsc, q_i)(1 \dotsm 1)  = q_i\bigl( \cdots q_2\bigl( q_1(1 \dotsm 1) \bigr) \cdots \bigr),
\]
Note that $u^{\left[  i\right]  } \in \mcW_n$ and has type $\tup{m_1, m_2, \dotsc, m_i, n-p_i(\mm), 0, 0, \ldots}$.
...
Then
\[
\swt{u} = \left(  \prod_{b \in B} x_b \right)  \det \left(  h_{\gamma_j+i-j-1}(x_1,\dotsc,x_{b_j}) \right)_{1 \leq i \leq r,\ 1 \leq j \leq r}.
\]
\end{thm}
...
&  = \left(  \prod_{i=1}^k x_{b_i} \right) \det\biggl( h_{\lambda_j-j+i-1}\left(x_1,x_2,\ldots,x_{b_j}\right)  \biggr)_{1\leq i\leq k,\ 1\leq j\leq k} ,
%& \\ & = \left( \prod_{b \in B} x_b \right)  \det\left(  h_{\gamma_j+i-j-1}(x_1,\dotsc,x_{b_j}) \right)_{1\leq i\leq r,\ 1\leq j\leq r} ,
\end{align*}
where the last equality is from Corollary~\ref{cor:determinant_form.bij1c} (applied to $B$ and $b_i$ instead of $S$ and $s_i$).
% Hence the claim follows since $\gamma_j = \lambda_j$.
\end{proof}
