...
\Travis{The definitions in the following paragraph are currently unused.
Moreover, the terminology does not feel like it both naturally arose and can be used in a natural way.}

For future reference, let us introduce notations for some of the
behavior happening in the above algorithm.
Phase I of the algorithm consists of $n - \abs{q}$ \defn{steps};
each of these steps corresponds to one value of $i$,
and consists of finding the first $j \notin q$
weakly to the left of $i$ such that $v_j$ is not set,
and setting $v_j = u_i + 1$.
We shall refer to these values of $i$, $j$, $u_i$ and $v_j$ as the
\defn{in-site}, \defn{out-site}, \defn{in-letter} and
\defn{out-letter} (respectively) of this step.
Furthermore, we imagine that $j$ is found by a recursive
process, in which we start at the site $i$ and keep moving
further and further left (cyclically) until we first hit
a site $j \notin q$ such that $v_j$ is not set;
this process will be called the \defn{search process} (in step $i$).
We say that there is \defn{wraparound} (in step $i$) if this
search process involves moving from $1$ to $n$.
Analogous notations will be used for Phase II
(which, of course, has $\abs{q}$ steps).
...
This same permutation works for the construction of $q(f_k(u))$
since $(f_k(u))_a \leq (f_k(u))_b$ whenever $u_a \leq u_b$.
Thus, the construction of $v' := q(f_k(u))$ proceeds exactly as the
construction of $v := q(u)$, except that the letters used to build
the word $v'$ are those of $f_k(u)$ instead of those of $u$.
...
So either
\begin{itemize}
\item site $i$ and $j$ are set in the same phase or
\item site $i$ is set in Phase~II and $j$ in Phase~I.
\end{itemize}
...
\begin{itemize}
\item site $i$ and $j$ are set in the same phase or
\item site $i$ is set in Phase~I and $j$ in Phase~II.
\end{itemize}
Let $k = q(u)_j$ if site $j$ is set during Phase~II and $k = q(u)_j - 1$ otherwise.
Therefore, if site $j$ is set during Phase~II, we have $q\bigl( f_k(u) \bigr)_i \geq 2$ and $q\bigl( f_k(u) \bigr)_j = 1$.
...
It is clear that $\vee^{(k)} X(1 \dotsm 1) = 1 \dotsm 1$,
...
\begin{proof}
  ...
  $\wt\bigl( \theta(\qq, X) \bigr) = \wt(\qq) \wt(X)$.
...
If $w$ is a word and $k$ is a positive integer, then
\[
w^{-1}(k) := \set{i \in \ive{n} \mid w_i = k}.
\]
\travis{Change to $w^{-1}|_k$ or $w^{-1}[k]$? Not the current as it is a little too much like an inverse function of $w$.}

If $q$ is a queue and $p \in \bigl[ \abs{q} \bigr]$, then \defn{$\max_p q$} means the $p$-th largest element of $q$.

\begin{lemma}
\label{lem:no_wrap}
Let $k$ be a positive integer.
Let $u \in \mcW_n$ be a word such that $u_i < k$ for all $i \in \ive{n}$.
Let $v \in \mcW_n$ be a $k$-frow.
Let $r = \ive{n} \setminus u^{-1}(k-1)$.
Let $q$ be a queue such that $\abs{q} > \abs{r}$.
Then, $q(u) = v$ if and only if the following statements hold:
\begin{enumerate}
 \item We have $q = \ive{n} \setminus v^{-1}(k)$.
 \item We have $\abs{v^{-1}(h)} = \abs{u^{-1}(h)}$ for all $h < k$.
 \item The word $u$ is a $(k-1)$-frow.
 \item For each $p \in \ive{\abs{r}}$, we have
       $\max_p (q) \geq \max_p (r)$.
 \item If $i \in r$ and $j \in q$ satisfy $u_i > v_j$, then
       $i > j$.
\end{enumerate}
\end{lemma}

\begin{proof}
Consider the algorithm for computing $q(u)$.
The condition $\abs{q} > \abs{r}$
shows that all letters of $q(u)$ that are $< k$
are set in Phase II of the algorithm,
whereas all letters that are $= k$ are set in Phase I.
Furthermore, it shows that at least one letter of $q(u)$ is $= k-1$.

$\Longrightarrow$: Assume that $q(u) = v$.
Hence, the preceding paragraph shows that all letters of $v$ that are $< k$
are set in Phase~II of the algorithm,
whereas all letters that are $= k$ are set in Phase~I.
Thus, $v^{-1}(k) = \ive{n} \setminus q$, so that
$q = \ive{n} \setminus v^{-1}(k)$.
This proves Statement (1).
Statement (2) also follows from the fact that
that all letters of $v$ that are $< k$
are set in Phase~II of the algorithm XXXXXXXX

We claim that there is no ``wrap-around'' in Phase II of the algorithm
until the steps when we process the $i$ for which $u_i = k-1$
(i.e., every time
that we seek the first $j$ weakly to the right of $i$ such that $j \in q$ and $v_j$
is not set, we find such a $j$ before having to wrap from $n$ back to $1$,
unless $u_i = k-1$).
(Indeed, assume the contrary.
Consider the first step in Phase II in which wrap-around occurs.
This step sets $v_j$ to $u_i$,
where $i$ is some site with $u_i < k$, and
where $j$ is the smallest element of $q$ for which $v_j$ has not been set before.



XXX
%Consider the $t$ in Remark~\ref{rmk:t-splitting}.
%Then, $t = \ell-1$ (or, more precisely, we can pick $t = \ell-1$).
The condition on $\abs{q}$ shows that all letters of $q(u)$ that are $< \ell$
are set in Phase II of the algorithm,
whereas all letters that are $= \ell$ are set in Phase I.
Furthermore, it shows that at least one letter of $q(u)$ is $= \ell - 1$.
...
First, let $\qq = \tup{q_1, \ldots, q_{\ell-1}}$ be any MLQ appearing in the sum $\swt{u}$.
Thus, $\qq (1 \dotsm 1) = u$, and the type of $\qq$ is the type $\mm$ of $u$.
In particular, $q_{\ell-1} = B$.
Hence, $\wt(q_{\ell-1}) = \prod_{i\in B} x_i$.
Hence, the factor $\prod_{i\in B} x_i$ appears in each addend of $\swt{u}$;
we shall disregard this factor in the following.

For each $i \in \ive{\ell}$, we set
$u^{(i)} := q_{i-1}\bigl( \cdots q_1(1 \cdots 1) \cdots \bigr)$.
This is a word with $i$ classes, and exactly $\abs{q_i}$ of its letters are $< i$.
We claim that these letters are weakly decreasing.

This is proven by reverse induction over $i$:
The case $i = r$ follows from the construction of $u^{(r)} = u$.
To get from $i$ to $i-1$, XXX
...
Pairs of integers shall be represented as points in the plane in matrix coordinates; that is, the pair $\tup{i, j}$ moves further down as $i$ increases and further right as $j$ increases.

TODO: Sure about further down as $i$ increases? May want to undo the reflection $i \mapsto r+1-i$.

Define a directed graph with vertices $\tup{i, j} \in \ive{r} \times \ive{n}$ and edges
\[
(r+1-i,j) \to (r+1-i,j+1)
\]
...
We choose the $r$ sources $\iota_i = (r+1-i, 1)$ and terminals $\tau_i = (r-i+\gamma_i,b_{r+1-i})$ for $i \in \ive{r}$.

First, we label an element $k \in q_i \in \qq$ by $q_i\bigl( \cdots q_1(1 \cdots 1) \cdots \bigr)_k$.
We construct a disjoint $r$-path $\tup{p_1, \ldots, p_r}$ recursively:
For each $i$, if $p_1, \ldots, p_{i-1}$ are defined,
then we the largest\footnote{We do not consider the queues as lying on a cycle here but instead as elements of the set $\ive{n}$.} unused elements $(a_1, \dotsc, a_h)$ in $\qq$ labeled by $v_{r-i}$ and constructing a path
\newcommand{\patharrow}{ \xrightarrow{\hspace{10pt}} }
\[
\iota_i \patharrow (\overline{\imath} + 1, a_1) \patharrow (\overline{\imath} + 2, a_1) \patharrow (\overline{\imath} + 2, a_2) \patharrow \cdots \patharrow (\overline{\imath} + \gamma_i - 1, a_h) = \tau_i,
\]
where $\overline{\imath} = r + 1 - i$.
Conversely, given a disjoint $r$-path, we construct the MLQ by considering the north-east corners and labelling the paths according to $v$.
...
\end{tikzpicture}
\]
\caption{An example of the bijection with $n = 9$, $r = 6$, $\ell = 4$, $v = 332221$, and $B = \set{1,2,4,6,7,9}$ between MLQs and disjoint $r$-paths in the proof of Theorem~\ref{thm:determinant_form}.}
