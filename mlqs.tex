...
This can be easily seen by considering the parentheses as an infinite shifted-periodic Motzkin path, where a ``nothing'' corresponds to a flat step.
...
\Travis{I am okay with getting rid of the relation to Motzkin paths, but then we should say absolutely nothing. I really think this is clear to the reader after half a second of thought.}
...
If a site $j \in \ive{n}$ contributes a matched parenthesis or two matched parentheses or no parenthesis at all, we say $j$ is \defn{balanced}; otherwise we say $j$ is \defn{unbalanced}.
We say $\mcI \subseteq \ive{n}$ is a \defn{balanced set} if each $i \in \mcI$ is balanced.
...
Thus we assume the claim holds for $p-1$.
...
Hence, $\mcI$ remains a balanced interval after freezing $(_{i_p}$ and $)_{i_p}$.
