...
Fix a positive integer $n$. For a positive integer $k$, let $\ive{k}$ denote the set $\set{1, 2, \ldots, k}$ and $\SymGp{k}$ denote the symmetric group on $\ive{k}$.
Let $s_i \in \SymGp{k}$ be the simple transposition of $i$ and $i+1$.
...
We refer the reader to \travis{Darij, please insert appropriate reference(s).} for details on the LGV Lemma.
...
Any non-intersecting path system from the sources to the terminals gives in a straightforward way an MLQ with the corresponding weight whose bottom row is labelled $u$, and conversely \travis{To be finished?}.
...
Furthermore let $S \subseteq \ive{r-1}$ be such that $i\in S$ implies $i+1 \notin S$, and define the permutation $\sigma_S = \left( \prod_{i \in S} s_i \right) w_0$,\footnote{The elements $\{s_i \mid i \in S\}$ all commute, so the product is well-defined.} where $s_i, w_0 \in \SymGp{r}$.
...
Building on work by Ferrari--Martin~\cite{FM06}, the paper~\cite{AAMP} introduced opposite operators $\Psi_i \colon \mcW_{S\cup\{i\}} \to \mcW_S$ given by $\Psi(u) = \sum_q q(u)$, and showed that $\Psi M = M \Psi$.
Furthermore they conjectured that $\Psi_S \Psi_T = \Psi_T \Psi_S$, where it is called the \defn{commutativity conjecture}.

Note that for the commutativity conjecture, it is sufficient to show that $\Psi_i \Psi_j = \Psi_j\Psi_i$.
...
Note that $\widetilde{\Psi}_i = \Psi_i$ when we specialize $x_1 = \cdots = x_n = 1$, giving the connection between our MLQs and the multi-species TASEP as follows.
...
Note however that queues gives us \emph{a} random process with this property: for a word $u \in \mcW_{\mm}$, a move in the chain is given by
\begin{enumerate}
\item picking a random queue $q$
\item going to the state $q(u) \in \mcW_{\mm}$.
\end{enumerate}
