...
To ease notation, we shall identify any $\tau \in \SymGp{r}$ with the corresponding $u(v_{\tau}) \in \mcW_n$ from~\eqref{eq:weakly_decreasing_construction}.
...
\end{align*}
where $\gamma_j = j - \abs{ \{t \in T \mid t < j \} }$.
\end{cor}

\begin{proof}
By repeated applications of Theorem~\ref{thm:merge}, we have the first equality.
...
\]
by using the well-known formula for M\"obius inversion on the boolean lattice.
