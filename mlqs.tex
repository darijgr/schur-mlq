...
\begin{vershort}
We compute $u = q_{\ell}(u')$ by choosing the permutation such that we pick all letters $h$ in $u'$ from left to right.
Therefore, from the interlacing relation, we have $q_{\ell}^{(h)} = \set{p \mid u_p = h}$ for all $h \in \ive{\ell}$ and $q_{\ell}^{(h)} \gg q_{\ell}^{(h+1)}$.
