...
We will give the proof in Section~\ref{sec:thm_proof}.
...
\Travis{I'm considering upgrading this to a theorem considering there is a bit more involved with the other than simply applying Theorem~\ref{thm:permutation}.}
...
\begin{proof}
  ...
  Note that for
  \[
  u = \widetilde{\qq}(1 \dotsm 1) = \qq\bigl( X(1 \dotsm 1) \bigr),
  \]
  we have
  \[
  v = \qq(1 \dotsm 1) = \merge{i} u
  \]
  since $X$ is a $p_i(\mm)$-queue\travis{This probably still needs a bit more detail}.
...
\travis{We might want to change notation to $c^{\uparrow}[i,j]$ and $c^{\downarrow}[i,j]$ so as to avoid confusion with open/closed intervals.}
...
Recall the combinatorial $R$-matrix acting on $B^{r,s}$ from~\cite{NY97,Shimozono02}, which is the unique crystal isomorphism $R \colon B^{r,s} \otimes B^{r',s'} \to B^{r',s'} \otimes B^{r,s}$.
In~\cite{KMO15}, a bijection $\Xi_r$ was given relating $r$-queues with an element of the KR crystal $B^{r,1}$ by considering $q = \{b_1 < \cdots < b_r\}$  as a single-column Young tableau of height $r$.
They extended this to a bijection $\Xi$ between multiline queues of type $\mm$ with $\ell+1$ classes and $B^{p_1,1} \otimes B^{p_2,1} \otimes \dotsm \otimes B^{p_{\ell},1}$ by
\[
\Xi(\qq) = \Xi_{p_1}(q_1) \otimes \Xi_{p_2}(q_2) \otimes \dotsm \otimes \Xi_{p_{\ell}}(q_{\ell}).
\]
Thus, by comparing the description of the combinatorial $R$-matrix for $B^{r,1}$ and $B^{r',1}$ from~\cite{NY97}, we obtain that taking the dual configuration is equivalent to applying the combinatorial $R$-matrix under $\Xi$.

\begin{prop}
Let $C$ be an $(r, r')$-configuration. Then
\[
