...
\address[E. Aas]{Department of Mathematics, Pennsylvania State University, McAllister Building, State College, PA 16801, USA}
\email{eaas@kth.se}
...
However, the applications of the ASEP (and its variations) have since spread to other areas, such as
\begin{itemize}
...
\item anisotropic conductors known as solid electrolytes~\cite{CL99},
...
We show that this does not change the MLQ as a function on words.
...
We note that our weighting scheme can be extended to the multiline process used to determine the steady state distribution of the totally asymmetric zero range process (TARZP) on a ring, where multiple particles can occupy the same site.
...
Thus, we expect that a similar description of a $\sigma$-twisted multiline process can be defined such that the weighting is invariant under the action of the combinatorial $R$-matrix.
...
\item[Phase~II]
   For each of the remaining $r$ sites $i \in \ive{n}$ in increasing order (in our ordering), match the site with value $u_i$ with the first site $j$ weakly to the right (cyclically) of $i$ such that $j \in q$ that is unmatched and set $v_j = u_i$.
\end{description}
...
\begin{verlong}
(The result of the algorithm will not depend on this choice, as we will show in Lemma~\ref{lemma:order_indep} below.)
\end{verlong}
...
\begin{itemize}
 \item If $k < \abs{q}$, then this switch interchanges two consecutive
       ...
       (namely, to $h$).
 \item If $k > \abs{q}$, then a similar argument works (this time, we
       are interchanging two consecutive steps in Phase~I, and both
       letters $v_j$ will be set to $h+1$).
...
\end{tikzpicture}
\]
\caption{The bijection between MLQs and disjoint $r$-paths for Example~\ref{ex:NILP_bijection} as described in the proof of Theorem~\ref{thm:determinant_form}.}
...
It is easy to see that $\Phi_i M_S = M_{S\cup \{i\}} \Phi_i$, where $\Phi_i$ denotes the linear map $V_S \to V_{S\cup\{i\}}$ that is given by $\Phi_i$ on the basis $\mcW_S$. 
...
\begin{tikzpicture}[scale=0.8,baseline=-30]
% q_1
\foreach \x in {9,12,13} {
    \node[blue] at (\x,0) {$1$};
    \draw (\x,0) circle (0.3);
}
\foreach \x in {1,...,8,10,11,14,15}
    \draw (\x-0.3, -0.3) rectangle (\x+0.3, +0.3);
% q_2
\foreach \x in {7,8} {
...
As usual, we consider $C$ as a function on words by $C(u) := q_2\bigl(q_1(u)\bigr)$, and we define the weight of $C$ by $\wt(C) := \wt(q_1) \wt(q_2)$.
...
Then we can assign to $f$ a word $\omega_k f \in \mcW_n$ as follows:
...
\]
from the MLQ $\qq_D = (q_1, q_2, \dotsc, q_{\ell})$ to obtain the correct labelings, where $\ell + \widetilde{\ell}$ is the largest label appearing in the Kohnert labeling.
...
We have thus proven that for each $\ell \in \inter[a,b]$, we have $c^\uparrow[a,\ell] \geq c^\downarrow[a,\ell]$. In other words, the interval $\inter[a,b]$ is top-heavy, qed.]
