...
\begin{proof}
First, note that if $q$ is an $r$-queue, then $q'$ is an $(n-r)$-queue and that $q'$ is given by reversing $[n] \setminus q$.
For a fixed $\ell$, let $\ell + 1 - k$ be the dual letter of $k$.
Moreover, $v$ can be described as reversing $u$ and taking the dual letters.
Hence, the splitting class of $q'(v)$ is the dual letter of $t$.
Therefore, Phase~I (resp.~II) of $q(u)$ corresponds to Phase~II (resp.~I) of $q'(v)$ by reversing the resulting word and taking dual letters.
Similarly, for the Phase~III of $q(u)$ and $q'(v)$, and the claim follows.
\end{proof}
...
Note that that $q(\merge{i} u) = \merge{i} q(u)$ for all $i < t$ and $q(\merge{i} u) = \merge{i+1} q(u)$ for all $i \geq t$ since equal entries can be processed in any order by $q$.
Note that we can define $f_k$ as a sequence of merges on $u$, and so we have $q\bigl( f_k(u) \bigr)_i \leq q\bigl( f_k(u) \bigr)_j$ for all $k \in \ZZ_{\geq 1}$.
...
\begin{proof}
  ...
  Next, assume\darij{as the induction hypothesis?} that $v = \qq(1 \dotsm 1) = \merge{i} u$, where $\qq = (q_1, \dotsc, q_k)$ is an MLQ of type $\merge{i}\mm$ with $\ell - 1$ classes and $q_k$ is an $r$-queue\darij{why do we need this last part? we aren't working from the back of the MLQ, are we?}.
...
Any non-intersecting path system from the sources to the terminals gives in a straightforward way an MLQ with the corresponding weight whose bottom row is labelled $u$, and conversely; see Figure \ref{fig:lattice_path_bijection}. To show that all non-intersecting path systems connects $\iota_i$ to $\tau_i$ for each $i$, we may argue by induction on $i$.
...
\begin{proof}
  ...
  Let $B$ be a balanced block, and let $B^c := [\ell] \setminus B$ denote the compliment of $B$.
  \travis{Erik, is this correct or is this suppose to mean something different?}
  Note that for $B \subseteq \inter(i,j)$, the labelling of $C(u)$ for $k \notin B$ \travis{Where is $k$ used here?} is determined entirely by $q_1\cap B^c$, $q_2\cap B^c$, and $u_{\lvert B^c \rvert}$.
