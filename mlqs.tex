...
This can be easily seen by considering the parentheses as Motzkin paths, where a skipped letter corresponds to a flat step.
The unmatched positions correspond to up steps in the Motzkin path such that the path never returns to the same level.
...
$q'_2 = [q(u)]_k$.
\begin{comment}
Hence, we can apply the $k < \abs{q}$ case (proven above)
% of Proposition~\ref{prop:SL.dual}
to $n-k$, $u^*$ and $q^*$ instead of $k$, $u$ and $q$; we obtain
$\tup{ [u^*]_{n-k} , q^* }' = \tup{ q^\dagger , [q^*(u^*)]_{n-k} }$ for some queue $q^\dagger$.
In view of $q^*(u^*) = (q(u))^*$ (which follows from Lemma~\ref{le:dual}), this rewrites as 
$\tup{ [u^*]_{n-k} , q^* }' = \tup{ q^\dagger , [(q(u))^*]_{n-k} }$.
Since $[u^*]_{n-k} = ([u]_k)^*$ and $[(q(u))^*]_{n-k} = ([q(u)]_k)^*$, this simplifies as
$\tup{ ([u]_k)^* , q^* }' = \tup{ q^\dagger , ([q(u)]_k)^* }$.
Comparing this with
$\tup{ ([u]_k)^* , q^* }' = \bigl( (q'_1)^*, (q'_2)^* \bigr)$
(a consequence of~\eqref{eq.rmk:balanced-dual-let.dual}),
we obtain $\bigl( (q'_1)^*, (q'_2)^* \bigr) = \tup{ q^\dagger , ([q(u)]_k)^* }$,
so that $(q'_2)^* = ([q(u)]_k)^*$ and therefore $q'_2 = [q(u)]_k$.
\end{comment}
