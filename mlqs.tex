...
= \abs{r}
\]
(since~\eqref{eq.lem:determinant_form.interl-B.rj} show that the sets $r^{(1)}, r^{(2)}, \ldots, r^{(h-1)}$ partition $r$).
But~\eqref{eq.lem:determinant_form.interl-B.qj} show that the sets $q^{(1)}, q^{(2)}, \ldots, q^{(h)}$ partition $q$; thus,
\begin{vershort}
\[
 \abs{q}
