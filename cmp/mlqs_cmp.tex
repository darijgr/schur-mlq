\documentclass[cmp,envcountsect,envcountsame]{svjour}

\let\intertext\relax  % needs to remove a conflict
\usepackage{amsmath}
...
\titlerunning{MLQs with spectral parameters}

\author{Erik Aas\inst{1} \and Darij Grinberg\inst{2} \and Travis Scrimshaw\inst{3}\thanks{TS was partially supported by the Australian Research Council DP170102648 and the National Science Foundation RTG grant DMS-1148634.}}
\institute{Department of Mathematics, Pennsylvania State University, McAllister Building, State College, PA 116802, USA. \\ \email{eaas@kth.se}
\and School of Mathematics, University of Minnesota, 206 Church St. SE, Minneapolis, MN 55455. \\ \email{darijgrinberg@gmail.com} %\urladdr{http://www.cip.ifi.lmu.de/~grinberg/}
\and School of Mathematics and Physics, The University of Queensland, St.\ Lucia, QLD 4072, Australia. \\ \email{tcscrims@gmail.com} %\urladdr{https://people.smp.uq.edu.au/TravisScrimshaw/}
}
\authorrunning{E.~Aas \and D.~Grinberg \and T.~Scrimshaw}

\date{Received: date / Accepted: date}
% The correct dates will be entered by Springer
%
% Add name of the expert who has communicated your paper
\communicated{Alexei Borodin}
...
We construct a perfect matching on the sites in $u$ and $v$ as follows.
\begin{description}
\item[Phase~I]
   For each of the last $n - r$ sites $i \in \ive{n}$ in decreasing order (in our ordering), match the first site $j$ weakly to the left (cyclically) of $i$ such that $j \notin q$ that is unmatched and set $v_j = u_i + 1$.
\item[Phase~II]
   For each of the remaining $r$ sites $i \in \ive{n}$ in increasing order (in our ordering), match the site with value $u_i$ with the first site $j$ weakly to the right (cyclically) of $i$ such that $j \in q$ that is unmatched and set $v_j = u_i + 1$.
\end{description}
There is a choice inherent in ordering the sites (since some letters of $u$ can be equal), but we will later see that this choice does not affect the word $v$ obtained.

\begin{example}
\label{ex:first_queue}
We consider the $4$-queue $q = \{1, 4, 8, 9\}$, and let $u = 346613321$.
We order the sites as $598167243$.
...
Choose a permutation $\tup{i_1, i_2, \ldots, i_n}$ of $\tup{1, 2, \ldots, n}$ such that $u_{i_1} \leq u_{i_2} \leq \cdots \leq u_{i_n}$.
(This corresponds to the ordering of the sites that had to be chosen in the previous formulation of the algorithm.)
...
\begin{definition}
For $\sigma \in \SymGp{\ell-1}$, a \defn{$\sigma$-twisted multiline queue (MLQ) of type $\mm = \tup{m_1, m_2, \ldots, m_\ell, 0, \ldots}$}, with $\ell$ classes, is a sequence of queues $\qq = (q_1, \dotsc, q_{\ell-1})$ such that $q_i$ is a $p_{\sigma(i)}(\mm)$-queue and $m_{\ell} = n - p_{\ell-1}(\mm)$ (that is, $p_\ell(\mm) = n$).
...
Consider $u = 2312$, a word of type $\mm = \tup{1,2,1,0,0,\ldots}$.
We have the following MLQs $\qq$ (of type $\mm$) such that $\qq(1111) = 2312$:
\[
...
Next, we let $\sigma = s_1$,
and we construct the corresponding $\sigma$-twisted MLQs $\qq$ (of type $\mm$)
\[
\begin{tikzpicture}[baseline=-30]
  \def\ll{0.75}   % level scaling
...
Consequently, the construction of the queue $q(\vee^{(k)} u)$ proceeds exactly like
...
\label{cor:merges}
  ...
  $m_t \neq 0$ and $m_{t+1} \neq 0$.
  Let $v$ be a packed word of type $\bigvee_T \mm$.
  Then,
  \[
...
\begin{example}
Let $\mm = (1,1,1,1,1,0,0,\ldots)$ and $T = \{1,3\}$.
Then we have $\bigvee_T \mm = (2,2,1,\ldots)$, and we take $v = 12123$.
...
24135.
\]
Note that $v = \vee_1 u'$ for $u' \in \{13234, 23134\}$, both of these possible $u'$ having type $(1, 1, 2, 1, 0, 0, \ldots)$.
...
$\bigvee_T u' = \bigvee_T w_0$ are precisely the words of the form $\sigma_S$ for $S \subseteq T$.
...
Let $\tup{A_1, A_2, \dotsc, A_k}$ and $\tup{B_1, B_2, \dotsc, B_k}$ be two $k$-vertices, then a \defn{non-intersecting lattice path tuple (NILP)} from $\tup{A_1, A_2, \dotsc, A_k}$ to $\tup{B_1, B_2, \dotsc, B_k}$ shall mean a $k$-tuple $\tup{p_1, p_2, \dotsc, p_k}$ of paths such that
\begin{itemize}
...
Next, let $s = \tup{s_1, s_2, \dotsc, s_k}$ be a strictly increasing sequence of elements of $\ive{n}$.
Then,
\[
...
Following~\cite[\S5.5]{Loth}, for each $i \in \set{1, 2, \ldots, k-1}$, define an operator
$\sigma_i \colon A^* \to A^*$.
...
From the injectivity of the map $\word \colon \set{\text{MLQs}} \to A^*$,
...
\begin{proof}
The assumption $\abs{q_1} \leq \abs{q_2}$ implies all closing parentheses in $\SP(q_1, q_2)$ are matched.
...
\begin{proposition} \label{prop:SL.dual}
Let $u \in \mcW_n$ be a word of type $\mm$.
Let $k = p_{\alpha}(\mm)$ for some $\alpha$.
Let $q$ be a queue.
...
In order to compute $\swt{135452}$, we need to examine MLQs of type $(1,1,1,1, 2, 0, \ldots)$.
...
\begin{enumerate}
\item Set $q_0 = \emptyset$, and let $f_0 \colon q_0 \to \ive{0}$ be the trivial map.
