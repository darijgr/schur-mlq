...
\Travis{Do we want $N$ or $n$? We should make a decision.}
...
\begin{equation}
\label{eq:amplitude}
  [u] := \sum_{(q_1, \dotsc, q_{\ell-1})} \prod_{i=1}^{\ell-1} \prod_{j \in q_i} x_j,
\end{equation}
\Travis{This creates a big notational conflict with $[n]$. We need to change this.}
where the sum is over all MLQs $(q_1, \dotsc, q_{\ell-1})$ of type $\mm$ such that $u = (q_{\ell-1} \circ \cdots \circ q_1)(1\dotsm 1)$.
...
We will also need the \defn{elementary symmetric function} and \defn{complete homogeneous symmetric function} on the indeterminates $x_1, x_2, \ldots$:
\[
e_k(x_1, \ldots) = \sum_{i_1 < \cdots < i_k} x_{i_1} \dotsm x_{i_k},
\qquad\qquad
h_k(x_1, \ldots) = \sum_{i_1 \leq \cdots \leq i_k} x_{i_1} \dotsm x_{i_k},
\]
respectively.
We define $e_d(x_1, \ldots) = 0$ and $h_d(x_1, \ldots) = 0$ for $d < 0$.
...
\label{thm:determinant_form}
  ...
  Then
  \[
  [u] = \det(h_{i-j-1+\gamma_j}(x_1, \dots, x_{b_j}))_{1\leq i,j\leq n},
  \]
