...
In~\cite{AAMP} a more algebraic approach is used for computing the stationary probabilities.\travis{This sentence, if we keep it, belongs in the introduction.}
...
Recall that $\mcW_{\mm}$ denotes the set of words of type $\mm$.

Now, let $V_S = \RR^{\mcW_{\mm_S}}$ \travis{This makes my eyes hurt.} and let $M_S \colon V_S \to V_S$ be the transition matrix of the TASEP on $\mcW_{\mbf{m}_S}$.
...
We note, however that the Markov chain on $\mcW_{\mm}$ with transitions $u \to q(u)$ for $r$-queues $q$ (for some fixed $r$) has stationary probabilities given by our weighted amplitudes $u$. 
