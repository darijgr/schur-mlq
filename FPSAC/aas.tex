...
We will also consider our process to be discrete in time, where our transition map interchanges a pair $u_i > u_{i+1}$ to $u_{i+1} u_i$ and is done at a uniform rate.

The steady state of the TASEP is known in terms of another process using multiline queues and applying the Ferrari--Martin algorithm~\cite{FM06}.
In~\cite{KMO15,KMO16}, the Ferrari--Martin algorithm was reformulated in terms of the combinatorial $R$-matrix~\cite{NY97,Shimozono02} and using Kirillov--Reshetikhin crystals.
...
A word $u$ or type $\mm$ is \defn{standard} if there exists an $\ell$ such that $m_i \neq 0$ for $1 \leq i \leq \ell$ and $m_i = 0$ for $i > \ell$.
...
\begin{dfn}
A (ordinary) \defn{multiline queue} (MLQ) of type $\mm$ is a sequence of queues $q_1, \dotsc, q_{\ell-1}$ such that $q_i$ is a $p_i(\mm)$-queue.
\end{dfn}
...
For a standard word $u$ of type $\mm$ with $\ell$ classes, we define the \defn{amplitude} or \defn{spectral weight} as
\begin{equation}
\label{eq:amplitude}
...
\label{thm:permutation}
  Let $u$ be a standard word of type $\mm$ with $\ell$ classes.
...
\begin{cor}
  Let $\mm$ be a standard type and $\mm' = (m_1, \dotsc, m_{i-1}, m_i + m_{i+1}, m_{i+2}, \ldots)$.
...
\label{thm:determinant_form}
  ...
  Let $v_1 v_2 \dotsm v_k$ \travis{Do not use $\dots$ (dots command)! It can get it wrong as it does here!} be a weakly decreasing (non-cyclic) standard word of length $k$ with $r-1$ classes.
...
In~\cite{AasLin17}, a formula for the amplitude $\swt{u((\prod_{i\in S} s_i)w_0)}$ is conjectured, where $w_0 = k (k-1) \dotsm 1$ is the reverse permutation on $[k]$.

Let $\varphi(S) = \swt{u(( \prod_{i\in S} s_i ) w_0)}$, and $\psi(T) = \sum_{S \subseteq T} \varphi(S)$.
By Theorem~\ref{thm:permutation}, we hav
\[
\psi(T) = \left(\prod_{i\in S} e_{m_1 + \dotsm + m_i}(x_1, \dotsc, x_n) \right) \swt{ \left( \prod_{j \in T} \merge{n-j}{} \right) w_0},
\]
\travis{If we keep $\wedge_j$, then we can use $\bigwedge_T$.}
\travis{Does order matter here?}
By M\"obius inversion we have $\varphi(S) = \sum_{T\subseteq S} (-1)^{|S|-|T|} \psi(T)$.
By Theorem~\ref{thm:determinant_form}, $\swt{w_T} \prod_{i \in T} e_i(n) \det(\dots)$ \erik{FIX}.
