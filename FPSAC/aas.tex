...
We will also consider the process to be discrete in time, where the transition map interchanges a pair $u_i u_{i+1}$ with $u_i > u_{i+1}$ to $u_{i+1} u_i$.
...
\Travis{We need to talk about Yang--Baxter here because we need this to really be an $S_m$ action for confluence.}
...
Let $V_S$ be the vector space over $\RR$ with basis $\set{\epsilon_w \mid w \in \mcW_S}$, and let $M_S \colon V_S \to V_S$ be the transition matrix of the TASEP on $\mcW_S$.
For $i,S$ such that $i\notin S$, we define $\Psi_i \colon V_{S \cup \{i\}} \to V_S$ by letting $\Psi_i(\epsilon_u) = \sum_{q} \epsilon_{q(u)}$, where the sum is taken over all $i$-queues.
