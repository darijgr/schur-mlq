...
\address{\addressmark{1}Department of Mathematics, Pennsylvania State University, McAllister Building, State College, PA 16802, USA
...
We will also consider the process to be discrete in time.
...
Moreover, it connects the TASEP with five-vertex models, corner transfer matrices, 3D integrable lattice models, and the tetrahedron equation of~\cite{Zam80}, yielding a new matrix product formula for the steady state distribution.
...
This allows us to define the spectral weight of a word $u$ to be the sum of the weights of all ordinary MLQs $\qq$ such that $u = \qq(1^n)$.
...
This action is given by applying a combinatorial $R$-matrix to an MLQ,\footnote{This operation has also previously appeared in other contexts by Danilov and Koshevoy~\cite{DanilovKoshevoy} (see also~\cite[Ch.~4]{Gorodentsev2}).} and we show that does not change the MLQ as a function on words.
...
Taken together with $x_1 = \cdots = x_n = 1$ proves~\cite[Conj.~3.10]{AasLin17}.
...
For the remainder of this section, we fix an $(r,s)$-configuration $C = (q_1, q_2)$.
...
The following notation will be useful later on: for a word $u \in \mcW_n$ and $(r,s)$-configuration $C = (q_1, q_2)$, we let $T(j)$ be the pair $(u_j, s_j)$ where $s_j = \bigcirc$ if $j \in q_1$ and $s_j = \square$ if $j\notin q_1$.
...
\begin{remark}
The BB and, when $r > s$, BU reductions always have $q_1(u) = q_1(u_{i \leftrightarrow j})$. 
\end{remark}

\begin{lemma}[UB reduction]
\label{le:UB}
...
The \defn{totally asymmetric simple exclusion process} (TASEP) is a Markov chain on $\mcW_{n}$ with transitions $u_i u_{i+1} \to u_{i+1} u_i$ with rate $1$ for $u_i < u_{i+1}$.
...
For example when $n = 4$, we have $\mm_{\emptyset} = (1,1,1,1)$ and $\mm_{\{1,2,3\}} = (4)$.
...
Let $M_S \colon V_S \to V_S$ be the transition matrix of the TASEP on $\mcW_S$.
For $i,S$ such that $i\notin S$, we define $\Psi_i \colon V_{S \cup \{i\}} \to V_S$ by letting $\Psi_i(\epsilon_u) = \sum_{q} \epsilon_{q(u)}$, where the sum is taken over all $i$-queues $q$.

In~\cite{AAMP}, it is shown that $M_S \Psi_i = \Psi_i M_{S\cup\{i\}}$.
Let $\pi_S \in V_S$ be the stationary distribution of $M_S$: the unique vector with non-negative entries summing to $1$ satisfying $\pi_S = M_S \pi_S$.
Thus, the vector $\pi_S$ can be computed from $\pi_{S\cup\{i\}}$ by $\pi_S = \Psi_i \pi_{S\cup\{i\}}$.
This allows us to compute every stationary distribution from the trivial vector $\pi_{[n]} = 1 \in \RR$.

It is conjectured in~\cite{AAMP} that $\Psi_i \Psi_j = \Psi_j\Psi_i$, where it is called the \defn{commutativity conjecture}.
...
Thus, our proof of Theorem~\ref{thm:permutation} shows that $\widetilde{\Psi}_i \widetilde{\Psi}_j = \widetilde{\Psi}_j \widetilde{\Psi}_i$ for the weighted operators $\widetilde{\Psi}_i$ given by $\widetilde{\Psi}_i(\epsilon_u) = \sum_q \wt(q) \epsilon_{q(u)}$.
Note that $\widetilde{\Psi}_i = \Psi_i$ when we specialize $x_1 = \cdots = x_n = 1$.
...
However, we note that the Markov chain on $\mcW_n$ with transitions $u \to q(u)$ for $r$-queues $q$ (for some fixed $r$) has stationary probabilities given by our spectral weights of $u$. 
