...
\begin{example}
Suppose $n = 8$. Consider the $(6,2)$-configuration $C = (\{1,2,3,4,5,8\}, \{7,8\})$ with $u$ on the left and $u_{1\leftrightarrow8}$ on the right:
\[
\begin{tikzpicture}[baseline=25]
  \def\sc{0.75}   % Change this to adjust the x-scaling
  \foreach \i in {1,5} { \node at (\i*\sc,2.7){$1$}; }
  \foreach \i in {2,3,4,6,7,8} { \node at (\i*\sc,2.7){$2$}; }
  \foreach \i in {1,2,3,4,5,8} { \draw (\i*\sc,2) circle (0.3); }
  \foreach \i in {6,7} { \draw (\i*\sc-.3,2-.3) rectangle +(0.6,+0.6); }
  \foreach \i in {1,5} { \node at (\i*\sc,2) {$1$}; }
  \foreach \i in {2,3,4,8} { \node at (\i*\sc,2) {$2$}; }
  \foreach \i in {6,7} { \node at (\i*\sc,2) {$3$}; }
  \foreach \i in {7,8} { \draw (\i*\sc,1) circle (0.3); }
  \foreach \i in {1,2,3,4,5,6} { \draw (\i*\sc-.3,1-.3) rectangle +(0.6,+0.6); }
  \foreach \i in {5,6} { \node at (\i*\sc,1) {$4$}; }
  \foreach \i in {7,8} { \node at (\i*\sc,1) {$1$}; }
