...
\\ \addressmark{3}School of Mathematics and Physics, The University of Queensland, St. Lucia, QLD 4072, Australia}
...
In Section~\ref{sec:tasep}, we describe the connection between our work and the TASEP.
...
A word $u$ of type $\mm$ is \defn{packed} if there exists an $\ell$ such that $m_i \neq 0$ for $1 \leq i \leq \ell$ and $m_i = 0$ for $i > \ell$. We will also call the type $\mm$ itself packed in this case.
...
Note that if one of the previous lemmas applies to $C$ for some $u,i,j$, then it applies to $C'$ with the same $u, i, j$.
