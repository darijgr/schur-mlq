...
One potential application is that our weighting could be used to describe the steady state distribution for the inhomogeneous TASEP~\cite{AL14,AM13}.
Furthermore, we expect our weighting scheme can be extended to the totally asymmetric zero range process (TARZP), where multiple particles can occupy the same site~\cite{KMO16TARZP,KMO16TARZPII}.
...
\label{thm:determinant_form}
  ...
  Then
  \[
  \swt{u} = \prod_{i\in B} x_i \det\bigl( h_{i-j-1+\gamma_j}(b_j) \bigr)_{1\leq i,j\leq r},
  \]
...
\[
  \psi(T) = \left(\prod_{i\in S} e_{p_i(\mm)}(n) \right) \swt{ \bigvee_T w_0} = \prod_{i\in B} x_i \left(\prod_{i\in S} e_{p_i(\mm)}(n) \right) \det(h_{i-j+1+\gamma_j}(b_j))_{1\leq i,j \leq r}.
\]
...
Taken together, and letting $x_i=1$ for all $i$, this proves~\cite[Conj.~3.10]{AasLin17}.
...
This allows for computing every stationary distribution from the trivial vector $\pi_{[n]} = 1 \in \RR$.

It is conjectured in~\cite{AAMP} that $\Psi_i \Psi_j = \Psi_j\Psi_i$, which is called the \defn{commutativity conjecture}.
...
Thus, our proof of Theorem~\ref{thm:permutation} shows that $\Psi_i \Psi_j = \Psi_j \Psi_i$ for the weighted operators $\Psi_i$ given by $\Psi_i(\epsilon_u) = \sum_q \wt(q) \epsilon_{q(u)}$.
