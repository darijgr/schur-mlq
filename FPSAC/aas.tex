...
To this end, we introduce an action of the symmetric group on MLQs which, under the FM algorithm, corresponds to the natural action by letters on words.
We show this by proving that applying a combinatorial $R$-matrix to an MLQ\footnote{This operation has also previously appeared in other contexts by Danilov and Koshevoy~\cite{DanilovKoshevoy} (see also~\cite[Ch.~4]{Gorodentsev2}).} does not change the MLQ as a function on words.
