...
Corollary \ref{co:merge} states that $\swt{13234} e_3(\xx) = \swt{13245} + \swt{14235}$, as we can check directly in this case.
\end{example}
...
In~\cite{AasLin17}, a formula for the amplitude $\swt{u((\prod_{i\in S} s_i)w_0)}$ is conjectured, where $w_0 = k (k-1) \dotsm 1$ is the reverse permutation on $[k]$ (note that the elements $(s_i : i \in S)$ commute pairwise, so $\prod_{i\in S} s_i$ well defined).
...
By Theorem~\ref{thm:determinant_form}, $\swt{\left(\prod_{j\in T} \merge{n-j}{} \right) w_0 } = \det(h_{i-j+1+\gamma_j}(b_j))_{1\leq i,j\leq k}$ where $\gamma_i$ is \erik{FILL IN}

By M\"obius inversion we have $\varphi(S) = \sum_{T\subseteq S} (-1)^{|S|-|T|} \psi(T)$.
By Theorem~\ref{thm:determinant_form}, $\swt{w_T} \prod_{i \in T} e_i(n) \det(\dots)$.
