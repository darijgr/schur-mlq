...
Our main result gives a proof of the commutativity conjecture of~\cite{AAMP} when we specialize our weighting to a single indeterminate\travis{Specialize to $1$?}. \erik{mention algebraic approach of AAMP}
...
We will consider the indices of letters in a word to be taken modulo $n$ (that is, $u_{k+m} = u_k$ for all $m$).
...
The \defn{totally asymmetric simple exclusion process} (TASEP) is a Markov chain on $\ive{n}$ with transitions $ji \to ij$ with rate $1$ for $i < j$.
Tthese moves preserve the type of the words, so each irreducible part of the chain corresponds to a type $\mm$.
In~\cite{AAMP} a more algebraic approach is used for computing the stationary probabilities. Clearly, there are $2^{n-1}$ packed types of length $n$.
...
Let $V_S$ be the vector space whose basis vectors are indexed by the words in $\mcW_S$, and let $M_S \colon V_S \to V_S$ be the transition matrix of the TASEP on $\mcW_S$.
For $i,S$ such that $i\notin S$, we define $\Psi_i \colon V_{S \cup \{i\}} \to V_S$ by letting $\Psi_i(e_u) = \sum_{q} e_{q(u)}$, where we sum over all $i$-queues and $e_w$ is the basis vector in $V_{S\cup\{i\}}$ or $V_S$ corresponding to the word $w$.

In~\cite{AAMP}, it is shown that $M_S \Psi_i = \Psi_i M_{S\cup\{i\}}$.
Therefore, if $\pi_S \in V_S$ is the unique vector with positive entries summing to $1$, satisfying $\pi_S = M_S \pi_S$, the vector $\pi_S$ can be computed from $\pi_{S\cup\{i\}}$ by $\pi_S = \Psi_i \pi_{S\cup\{i\}}$.
...
We have not found any process similar to the TASEP using only ``local'' moves for which our weighted amplitudes $u$ are the stationary probabilities.
We note, however that the Markov chain on $\mcW_n$ with transitions $u \to q(u)$ for $r$-queues $q$ (for some fixed $r$) has stationary probabilities given by our weighted amplitudes $u$. 
...
Oooohh Spooky!!!! It comes back from beyond the comments.

\Travis{The big example below will find its way into the article somehow. Will experiment.}
