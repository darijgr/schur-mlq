...
%TCIDATA{LastRevised=Friday, August 03, 2018 17:04:38}
...
\ell-1\right]  }=\mathbf{q}\left(  1\cdots1\right)  $.

We need another piece of notation: If $s\in\mathcal{W}_{n}$ and $t\in
\mathbb{N}$, then we say that $s$ is \defn{weakly decreasing up to level $t$}
if and only if every two sites $i<j$ in $\left[  n\right]  $ satisfying
$s_{j}\leq t$ must satisfy $s_{i}\geq s_{j}$. Equivalently, $s$ is weakly
decreasing up to level $t$ if and only if $s$ becomes weakly decreasing if all
letters larger than $t$ are removed. Our construction of $u$ shows that $u$ is
weakly decreasing up to level $\ell-1$.
...
word $u^{\left[  i\right]  }$ is weakly increasing up to level $i$.
\end{statement}
...
For the induction step, TODO XXX.]

\begin{statement}
\textit{Claim 2:} Let $i\in\left[  \ell-1\right]  $ and $j\in\left[  i\right]
$. Then, $q_{i}^{\left(  j\right)  }$ is the set of all $t\in\left[  n\right]
$ satisfying $\left(  u^{\left[  i\right]  }\right)  _{t}=j$.
\end{statement}

[\textit{Proof of Claim 2:} Explain how this follows from Claim 1 and the type
of $u^{\left[  i\right]  }$. TODO XXX]

\begin{statement}
\textit{Claim 3:} Let $i\in\left[  \ell-1\right]  $. Then, the construction of
$q_{i}\left(  u^{\left[  i-1\right]  }\right)  $ involves no wrapping-around.
TODO clarify.
\end{statement}

[\textit{Proof of Claim 3:} TODO XXX]

\begin{statement}
\textit{Claim 4:} Let $i\in\left[  \ell-1\right]  $ and $j\in\left[
i-1\right]  $. Then, $q_{i}^{\left(  j\right)  }\succeq q_{i-1}^{\left(
j\right)  }\gg q_{i}^{\left(  j+1\right)  }$.
\end{statement}

[\textit{Proof of Claim 4:} Descending induction on $j$. Use Claim 1 and Claim
2 too. TODO XXX Fix a permutation $\left(  i_{1},i_{2},\ldots,i_{n}\right)  $
of $\left(  1,2,\ldots,n\right)  $ such that $u_{i_{1}}\leq u_{i_{2}}%
\leq\cdots\leq u_{i_{n}}$. XXX Careful with the letter $i$]

\begin{statement}
\textit{Claim 5:} We have $q_{\ell-1}=B$. 
\end{statement}

[\textit{Proof of Claim 5:} TODO XXX]

Now, the MLQ $\mathbf{q}$ is interlacing (by Claim 4) and satisfies
$q_{\ell-1}=B$ (by Claim 5). This proves the $\Longrightarrow$ direction of
...
[\textit{Proof of Claim 11:} Induction on $i$. TODO XXX]

Applying Claim 11 to $i=\ell-1$, we conclude that TODO XXX
